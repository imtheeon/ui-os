...
async function main() {
  ...
  ok("manager enqueued data_quality_agent+schema_detection_agent+document_classifier+schema_evolution_agent+column_profiler+data_dictionary_agent+missing_data_agent+headcount_analytics_agent+productivity_agent+growth_rate_agent+data_privacy_agent+data_quality+compliance_agent+onboarding_agent+clarification_agent+multi_period+audit_summarizer+kpi_extractor+sql_analyst+anomaly_detector+categorizer+data_cleaner+unit_normalizer+duplicate_detector+outlier_explanation_agent+time_series_decomp_agent+failure_risk_agent+run_rate_agent+spend_analysis_agent+investor_memo_agent+okr_tracker_agent+swot_agent+query_builder_agent+esg_reporting_agent+seasonality_agent+benchmark_agent+professional_services_agent+headcount_analysis_agent+competitive_benchmarking_agent+data_reshape_agent+date_normalization_agent+string_normalization_agent+currency_normalization_agent+join_quality_agent+data_validation_rules_agent+distribution_agent+correlation_agent+regression_agent+hypothesis_testing_agent+pareto_agent+clustering_agent+funnel_analysis_agent+retention_analysis_agent+ab_test_agent+nps_analysis_agent+feature_adoption_agent+customer_health_score_agent+expansion_opportunity_agent+attrition_analysis_agent+headcount_planning_agent+demand_forecasting_agent+capacity_planning_agent+sla_analysis_agent+metric_tree_agent+text_analytics_agent+data_freshness_agent+narrative_diff_agent+alert_threshold_agent+data_annotation_agent+due_diligence_checklist_agent+market_sizing_agent+invoice_extraction_agent+bigquery_query_agent+reconciler+invoice_matcher+cash_flow_agent+tax_categorizer+budget_analyst+saas_metrics_agent+burn_rate_agent+cohort_agent+ar_aging_agent+ap_agent+bank_recon_agent+ratio_analysis_agent+cash_positioning_agent+debt_schedule_agent+equity_waterfall_agent+investor_metrics_agent+payment_terms_agent+working_capital_optimization_agent+profitability_agent+working_capital_agent+break_even_agent+cogs_analysis_agent+revenue_recognition_agent+churn_risk_agent+customer_segmentation_agent+payroll_analytics_agent+vendor_concentration_agent+clv_agent+sensitivity_analysis_agent+sales_pipeline_agent+pricing_optimization_agent+contract_analysis_agent+marketing_roi_agent+fraud_detection_agent+concentration_risk_agent+scenario_agent+liquidity_risk_agent+covenant_tracking_agent+transaction_classifier+expense_policy_agent+subscription_tracker+commission_calculator+overtime_analysis_agent+unit_economics_agent+valuation_agent+cap_table_agent+lease_analysis_agent+asset_register_agent+price_volume_mix_agent+bridge_analysis_agent+discount_analysis_agent+maverick_spend_agent+collections_priority_agent+bad_debt_provision_agent+credit_scoring_agent+fx_exposure_agent+consolidation_agent+nonprofit_agent+healthcare_agent+legal_billing_agent+hospitality_agent+construction_agent+revenue_quality_agent+cohort_analysis_agent+variance_analysis_agent+cash_flow_forecast_agent+expense_forecast_agent+debt_covenant_agent+tax_provision_agent+collections_agent+board_narrative_agent+investor_update_agent+vendor_risk+trend_detector+period_comparator+health_scorer+email_drafter+recommender+pattern_memory+accountant+forecaster+quota_attainment_agent+win_loss_agent+forecast_accuracy_agent+attribution_agent+price_elasticity_agent+compensation_analysis_agent+financial_statement_normalizer_agent+three_statement_model_agent+quality_of_earnings_agent+deferred_revenue_agent+earnings_quality_agent+intercompany_elimination_agent+stock_based_compensation_agent+gaap_compliance_agent+tax_rate_reconciliation_agent+report_generator+orchestrator_agent+confidence_reviewer_agent+exec_summarizer+insight_synthesis_agent+conflict_detection_agent+alert_agent+client_reporter+narrator+meeting_prepper+board_deck_builder+viz_recommender+chart_config_agent+kpi_card_agent+dashboard_spec_agent+validator+analyst+action_priority_agent", captured.length === 187);
  ...
  console.log("== funnel analysis agent ==");
  ok("analyze_funnel accepts good", validateProposal("analyze_funnel", {
    funnel_name: "Sales Pipeline Funnel",
    funnel_stages: [
      { stage_name: "Leads", entity_count: 1240, conversion_rate_from_previous: 100, drop_off_count: 0 },
      { stage_name: "Qualified", entity_count: 682, conversion_rate_from_previous: 55, drop_off_count: 558 },
    ],
    overall_conversion_rate: 7.0, biggest_drop_off_stage: "Qualified",
    drop_off_analysis: [], segment_breakdowns: [], optimization_recommendations: ["Tighten lead qualification"],
  }).ok);
  ok("analyze_funnel rejects funnel_stages with fewer than 2 items", !validateProposal("analyze_funnel", {
    funnel_name: "Funnel",
    funnel_stages: [{ stage_name: "Leads", entity_count: 100, conversion_rate_from_previous: 100, drop_off_count: 0 }],
    overall_conversion_rate: 50, biggest_drop_off_stage: "Leads",
    drop_off_analysis: [], segment_breakdowns: [], optimization_recommendations: ["x"],
  }).ok);
  ok("analyze_funnel rejects overall_conversion_rate > 100", !validateProposal("analyze_funnel", {
    funnel_name: "Funnel",
    funnel_stages: [
      { stage_name: "A", entity_count: 100, conversion_rate_from_previous: 100, drop_off_count: 0 },
      { stage_name: "B", entity_count: 50, conversion_rate_from_previous: 50, drop_off_count: 50 },
    ],
    overall_conversion_rate: 150, biggest_drop_off_stage: "B",
    drop_off_analysis: [], segment_breakdowns: [], optimization_recommendations: ["x"],
  }).ok);
  ok("analyze_funnel rejects empty funnel_name", !validateProposal("analyze_funnel", {
    funnel_name: "",
    funnel_stages: [
      { stage_name: "A", entity_count: 100, conversion_rate_from_previous: 100, drop_off_count: 0 },
      { stage_name: "B", entity_count: 50, conversion_rate_from_previous: 50, drop_off_count: 50 },
    ],
    overall_conversion_rate: 50, biggest_drop_off_stage: "B",
    drop_off_analysis: [], segment_breakdowns: [], optimization_recommendations: ["x"],
  }).ok);
  ok("funnel_analysis_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("funnel_analysis_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentFna } = await import("./lib/run-agent");
  const { stubBrain: sbFna } = await import("./lib/agent-brain");
  const { approveAction: approveFna, listPending: listFna } = await import("./lib/actions-service");
  const orgFna = await makeOrg("pro");
  const payloadFna = await makePayload(orgFna);
  const rFna = await runAgentFna({ orgId: orgFna, payloadId: payloadFna, role: "funnel_analysis_agent" }, { db, brain: sbFna });
  ok("funnel_analysis_agent run produced an analysis", rFna.ok && rFna.proposalCount === 1);
  const pendFna = await listFna(orgFna, { db });
  ok("stub proposal passes validateProposal and returns analyze_funnel", pendFna.length === 1 && pendFna[0].kind === "analyze_funnel");
  const apprFna = await approveFna(orgFna, pendFna[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes funnel_analysis_runs", apprFna.ok && apprFna.recordTable === "funnel_analysis_runs", JSON.stringify(apprFna));
  const { data: fnaRows } = await db.from("funnel_analysis_runs").select("org_id").eq("org_id", orgFna);
  ok("funnel analysis record org-stamped", fnaRows?.length === 1 && fnaRows[0].org_id === orgFna);
  const { routePayload: routeFna } = await import("./lib/manager");
  const routeCheckFnaFin = await routeFna({ orgId: orgFna, payloadId: payloadFna }, { db, enqueue: () => {} });
  const { data: plainPayloadFna } = await db.from("inbound_payloads").insert({
    org_id: orgFna, source: "upload", storage_path: `${orgFna}/fna/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckFnaNonFin = await routeFna({ orgId: orgFna, payloadId: plainPayloadFna!.id }, { db, enqueue: () => {} });
  ok("funnel_analysis_agent routes on BOTH the financial and non-financial route",
    routeCheckFnaFin.ok && routeCheckFnaFin.plan.includes("funnel_analysis_agent") &&
    routeCheckFnaNonFin.ok && routeCheckFnaNonFin.plan.includes("funnel_analysis_agent"));
  await db.from("organizations").delete().eq("id", orgFna);

  console.log("== retention analysis agent ==");
  ok("analyze_retention accepts good", validateProposal("analyze_retention", {
    cohort_type: "monthly",
    cohorts: [{ cohort_label: "Jan 2024", initial_count: 124, retention_by_period: [100, 78, 64] }],
    average_retention_by_period: [{ period: "Month 0", avg_retention_pct: 100 }],
    retention_curve_shape: "steep_initial_drop", critical_drop_off_period: "Month 1",
    churn_drivers: [], retention_benchmarks: [], improvement_recommendations: ["Redesign onboarding"],
  }).ok);
  ok("analyze_retention rejects bad cohort_type", !validateProposal("analyze_retention", {
    cohort_type: "daily",
    cohorts: [{ cohort_label: "Jan", initial_count: 100, retention_by_period: [100] }],
    average_retention_by_period: [], retention_curve_shape: "flat_high", critical_drop_off_period: "Month 1",
    churn_drivers: [], retention_benchmarks: [], improvement_recommendations: ["x"],
  }).ok);
  ok("analyze_retention rejects bad retention_curve_shape", !validateProposal("analyze_retention", {
    cohort_type: "monthly",
    cohorts: [{ cohort_label: "Jan", initial_count: 100, retention_by_period: [100] }],
    average_retention_by_period: [], retention_curve_shape: "sine_wave", critical_drop_off_period: "Month 1",
    churn_drivers: [], retention_benchmarks: [], improvement_recommendations: ["x"],
  }).ok);
  ok("analyze_retention filters out cohorts item with initial_count < 1", (() => {
    const r = validateProposal("analyze_retention", {
      cohort_type: "monthly",
      cohorts: [
        { cohort_label: "Jan", initial_count: 100, retention_by_period: [100] },
        { cohort_label: "Feb", initial_count: 0, retention_by_period: [100] },
      ],
      average_retention_by_period: [], retention_curve_shape: "flat_high", critical_drop_off_period: "Month 1",
      churn_drivers: [], retention_benchmarks: [], improvement_recommendations: ["x"],
    });
  ...
  ok("retention_analysis_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("retention_analysis_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentRtn } = await import("./lib/run-agent");
  const { stubBrain: sbRtn } = await import("./lib/agent-brain");
  const { approveAction: approveRtn, listPending: listRtn } = await import("./lib/actions-service");
  const orgRtn = await makeOrg("pro");
  const payloadRtn = await makePayload(orgRtn);
  const rRtn = await runAgentRtn({ orgId: orgRtn, payloadId: payloadRtn, role: "retention_analysis_agent" }, { db, brain: sbRtn });
  ok("retention_analysis_agent run produced an analysis", rRtn.ok && rRtn.proposalCount === 1);
  const pendRtn = await listRtn(orgRtn, { db });
  ok("stub proposal passes validateProposal and returns analyze_retention", pendRtn.length === 1 && pendRtn[0].kind === "analyze_retention");
  const apprRtn = await approveRtn(orgRtn, pendRtn[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes retention_analysis_runs", apprRtn.ok && apprRtn.recordTable === "retention_analysis_runs", JSON.stringify(apprRtn));
  const { data: rtnRows } = await db.from("retention_analysis_runs").select("org_id").eq("org_id", orgRtn);
  ok("retention analysis record org-stamped", rtnRows?.length === 1 && rtnRows[0].org_id === orgRtn);
  const { routePayload: routeRtn } = await import("./lib/manager");
  const routeCheckRtnFin = await routeRtn({ orgId: orgRtn, payloadId: payloadRtn }, { db, enqueue: () => {} });
  const { data: plainPayloadRtn } = await db.from("inbound_payloads").insert({
    org_id: orgRtn, source: "upload", storage_path: `${orgRtn}/rtn/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckRtnNonFin = await routeRtn({ orgId: orgRtn, payloadId: plainPayloadRtn!.id }, { db, enqueue: () => {} });
  ok("retention_analysis_agent routes on BOTH the financial and non-financial route",
    routeCheckRtnFin.ok && routeCheckRtnFin.plan.includes("retention_analysis_agent") &&
    routeCheckRtnNonFin.ok && routeCheckRtnNonFin.plan.includes("retention_analysis_agent"));
  await db.from("organizations").delete().eq("id", orgRtn);

  console.log("== ab test agent ==");
  ok("analyze_ab_test accepts good", validateProposal("analyze_ab_test", {
    test_name: "Checkout Flow Redesign", hypothesis: "Simplified checkout increases conversion",
    variants: [
      { variant_name: "Control", sample_size: 4821, primary_metric_value: 0.124, conversion_rate: 12.4 },
      { variant_name: "Variant A", sample_size: 4795, primary_metric_value: 0.157, conversion_rate: 15.7 },
    ],
    primary_metric: "Purchase Completion Rate",
    statistical_result: { p_value: 0.008, confidence_level: 0.95, statistically_significant: true, test_type: "two-proportion z-test" },
    winner: "variant_a", effect_size: "medium", sample_size_adequacy: "adequate",
    segment_effects: [], recommendation: "Ship Variant A", caveats: [],
  }).ok);
  ok("analyze_ab_test rejects variants with fewer than 2 items", !validateProposal("analyze_ab_test", {
    test_name: "Test", hypothesis: "H",
    variants: [{ variant_name: "Control", sample_size: 100, primary_metric_value: 0.1, conversion_rate: 10 }],
    primary_metric: "Metric",
    statistical_result: { p_value: 0.05, confidence_level: 0.95, statistically_significant: false, test_type: "t-test" },
    winner: "no_winner", effect_size: "negligible", sample_size_adequacy: "insufficient",
    segment_effects: [], recommendation: "R", caveats: [],
  }).ok);
  ok("analyze_ab_test rejects bad winner", !validateProposal("analyze_ab_test", {
    test_name: "Test", hypothesis: "H",
    variants: [
      { variant_name: "Control", sample_size: 100, primary_metric_value: 0.1, conversion_rate: 10 },
      { variant_name: "A", sample_size: 100, primary_metric_value: 0.12, conversion_rate: 12 },
    ],
    primary_metric: "Metric",
    statistical_result: { p_value: 0.05, confidence_level: 0.95, statistically_significant: false, test_type: "t-test" },
    winner: "variant_z", effect_size: "small", sample_size_adequacy: "adequate",
    segment_effects: [], recommendation: "R", caveats: [],
  }).ok);
  ok("analyze_ab_test rejects statistical_result with p_value > 1", !validateProposal("analyze_ab_test", {
    test_name: "Test", hypothesis: "H",
    variants: [
      { variant_name: "Control", sample_size: 100, primary_metric_value: 0.1, conversion_rate: 10 },
      { variant_name: "A", sample_size: 100, primary_metric_value: 0.12, conversion_rate: 12 },
    ...
    primary_metric: "Metric",
    statistical_result: { p_value: 1.5, confidence_level: 0.95, statistically_significant: false, test_type: "t-test" },
    winner: "no_winner", effect_size: "small", sample_size_adequacy: "adequate",
    segment_effects: [], recommendation: "R", caveats: [],
  }).ok);
  ok("ab_test_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("ab_test_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentAbt } = await import("./lib/run-agent");
  const { stubBrain: sbAbt } = await import("./lib/agent-brain");
  const { approveAction: approveAbt, listPending: listAbt } = await import("./lib/actions-service");
  const orgAbt = await makeOrg("pro");
  const payloadAbt = await makePayload(orgAbt);
  const rAbt = await runAgentAbt({ orgId: orgAbt, payloadId: payloadAbt, role: "ab_test_agent" }, { db, brain: sbAbt });
  ok("ab_test_agent run produced an analysis", rAbt.ok && rAbt.proposalCount === 1);
  const pendAbt = await listAbt(orgAbt, { db });
  ok("stub proposal passes validateProposal and returns analyze_ab_test", pendAbt.length === 1 && pendAbt[0].kind === "analyze_ab_test");
  const apprAbt = await approveAbt(orgAbt, pendAbt[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes ab_test_runs", apprAbt.ok && apprAbt.recordTable === "ab_test_runs", JSON.stringify(apprAbt));
  const { data: abtRows } = await db.from("ab_test_runs").select("org_id").eq("org_id", orgAbt);
  ok("ab test record org-stamped", abtRows?.length === 1 && abtRows[0].org_id === orgAbt);
  const { routePayload: routeAbt } = await import("./lib/manager");
  const routeCheckAbtFin = await routeAbt({ orgId: orgAbt, payloadId: payloadAbt }, { db, enqueue: () => {} });
  const { data: plainPayloadAbt } = await db.from("inbound_payloads").insert({
    org_id: orgAbt, source: "upload", storage_path: `${orgAbt}/abt/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckAbtNonFin = await routeAbt({ orgId: orgAbt, payloadId: plainPayloadAbt!.id }, { db, enqueue: () => {} });
  ok("ab_test_agent routes on BOTH the financial and non-financial route",
    routeCheckAbtFin.ok && routeCheckAbtFin.plan.includes("ab_test_agent") &&
    routeCheckAbtNonFin.ok && routeCheckAbtNonFin.plan.includes("ab_test_agent"));
  await db.from("organizations").delete().eq("id", orgAbt);

  console.log("== nps analysis agent ==");
  ok("analyze_nps accepts good", validateProposal("analyze_nps", {
    nps_score: 34, promoter_count: 187, passive_count: 143, detractor_count: 82, total_responses: 412,
    promoter_pct: 45.4, passive_pct: 34.7, detractor_pct: 19.9,
    top_promoter_themes: [], top_detractor_themes: [], segment_nps: [],
    trend_assessment: "improving", action_priorities: ["Fix report speed"],
  }).ok);
  ok("analyze_nps rejects nps_score > 100", !validateProposal("analyze_nps", {
    nps_score: 150, promoter_count: 1, passive_count: 1, detractor_count: 1, total_responses: 3,
    promoter_pct: 33, passive_pct: 33, detractor_pct: 34,
    top_promoter_themes: [], top_detractor_themes: [], segment_nps: [],
    trend_assessment: "stable", action_priorities: ["x"],
  }).ok);
  ok("analyze_nps rejects nps_score < -100", !validateProposal("analyze_nps", {
    nps_score: -150, promoter_count: 1, passive_count: 1, detractor_count: 1, total_responses: 3,
    promoter_pct: 33, passive_pct: 33, detractor_pct: 34,
  ...
  ok("analyze_nps rejects total_responses < 1", !validateProposal("analyze_nps", {
    nps_score: 0, promoter_count: 0, passive_count: 0, detractor_count: 0, total_responses: 0,
    promoter_pct: 0, passive_pct: 0, detractor_pct: 0,
    top_promoter_themes: [], top_detractor_themes: [], segment_nps: [],
    trend_assessment: "stable", action_priorities: ["x"],
  }).ok);
  ok("nps_analysis_agent → haiku model",
    (await import("./lib/agent-brain")).modelForRole("nps_analysis_agent") === "claude-haiku-4-5-20251001");

  const { runAgent: runAgentNps } = await import("./lib/run-agent");
  const { stubBrain: sbNps } = await import("./lib/agent-brain");
  const { approveAction: approveNps, listPending: listNps } = await import("./lib/actions-service");
  const orgNps = await makeOrg("pro");
  const payloadNps = await makePayload(orgNps);
  const rNps = await runAgentNps({ orgId: orgNps, payloadId: payloadNps, role: "nps_analysis_agent" }, { db, brain: sbNps });
  ok("nps_analysis_agent run produced an analysis", rNps.ok && rNps.proposalCount === 1);
  const pendNps = await listNps(orgNps, { db });
  ok("stub proposal passes validateProposal and returns analyze_nps", pendNps.length === 1 && pendNps[0].kind === "analyze_nps");
  const apprNps = await approveNps(orgNps, pendNps[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes nps_analysis_runs", apprNps.ok && apprNps.recordTable === "nps_analysis_runs", JSON.stringify(apprNps));
  const { data: npsRows } = await db.from("nps_analysis_runs").select("org_id").eq("org_id", orgNps);
  ok("nps analysis record org-stamped", npsRows?.length === 1 && npsRows[0].org_id === orgNps);
  const { routePayload: routeNps } = await import("./lib/manager");
  const routeCheckNpsFin = await routeNps({ orgId: orgNps, payloadId: payloadNps }, { db, enqueue: () => {} });
  const { data: plainPayloadNps } = await db.from("inbound_payloads").insert({
    org_id: orgNps, source: "upload", storage_path: `${orgNps}/nps/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckNpsNonFin = await routeNps({ orgId: orgNps, payloadId: plainPayloadNps!.id }, { db, enqueue: () => {} });
  ok("nps_analysis_agent routes on BOTH the financial and non-financial route",
    routeCheckNpsFin.ok && routeCheckNpsFin.plan.includes("nps_analysis_agent") &&
    routeCheckNpsNonFin.ok && routeCheckNpsNonFin.plan.includes("nps_analysis_agent"));
  await db.from("organizations").delete().eq("id", orgNps);

  console.log("== feature adoption agent ==");
  ok("analyze_feature_adoption accepts good", validateProposal("analyze_feature_adoption", {
    features_analyzed: [{ feature_name: "Dashboard", users_who_used: 847, total_users: 847, adoption_rate_pct: 100, avg_uses_per_user: 22.4 }],
    adoption_summary: { total_features: 1, avg_adoption_rate_pct: 100, median_adoption_rate_pct: 100 },
    power_features: [], underutilized_features: [], adoption_by_segment: [], correlation_with_retention: [],
    recommendations: ["Ship bulk edit"],
  }).ok);
  ok("analyze_feature_adoption rejects empty features_analyzed array", !validateProposal("analyze_feature_adoption", {
    features_analyzed: [],
    adoption_summary: { total_features: 0, avg_adoption_rate_pct: 0, median_adoption_rate_pct: 0 },
    power_features: [], underutilized_features: [], adoption_by_segment: [], correlation_with_retention: [],
    recommendations: ["x"],
  }).ok);
  ok("analyze_feature_adoption filters out features_analyzed item with total_users < 1", (() => {
    const r = validateProposal("analyze_feature_adoption", {
      features_analyzed: [
        { feature_name: "Dashboard", users_who_used: 100, total_users: 100, adoption_rate_pct: 100, avg_uses_per_user: 5 },
        { feature_name: "Bad", users_who_used: 0, total_users: 0, adoption_rate_pct: 0, avg_uses_per_user: 0 },
      ],
      adoption_summary: { total_features: 2, avg_adoption_rate_pct: 50, median_adoption_rate_pct: 50 },
      power_features: [], underutilized_features: [], adoption_by_segment: [], correlation_with_retention: [],
      recommendations: ["x"],
    });
    return r.ok && (r.payload.features_analyzed as unknown[]).length === 1;
  })());
  ok("analyze_feature_adoption filters out correlation_with_retention item with bad retention_signal_strength", (() => {
    const r = validateProposal("analyze_feature_adoption", {
      features_analyzed: [{ feature_name: "Dashboard", users_who_used: 100, total_users: 100, adoption_rate_pct: 100, avg_uses_per_user: 5 }],
      adoption_summary: { total_features: 1, avg_adoption_rate_pct: 100, median_adoption_rate_pct: 100 },
      power_features: [], underutilized_features: [], adoption_by_segment: [],
      correlation_with_retention: [
        { feature_name: "A", retained_users_adoption_pct: 80, churned_users_adoption_pct: 30, retention_signal_strength: "strong" },
        { feature_name: "B", retained_users_adoption_pct: 80, churned_users_adoption_pct: 30, retention_signal_strength: "extreme" },
      ],
      recommendations: ["x"],
    });
    return r.ok && (r.payload.correlation_with_retention as unknown[]).length === 1;
  })());
  ok("feature_adoption_agent → haiku model",
    (await import("./lib/agent-brain")).modelForRole("feature_adoption_agent") === "claude-haiku-4-5-20251001");

  const { runAgent: runAgentFad } = await import("./lib/run-agent");
  const { stubBrain: sbFad } = await import("./lib/agent-brain");
  const { approveAction: approveFad, listPending: listFad } = await import("./lib/actions-service");
  const orgFad = await makeOrg("pro");
  const payloadFad = await makePayload(orgFad);
  const rFad = await runAgentFad({ orgId: orgFad, payloadId: payloadFad, role: "feature_adoption_agent" }, { db, brain: sbFad });
  ok("feature_adoption_agent run produced an analysis", rFad.ok && rFad.proposalCount === 1);
  const pendFad = await listFad(orgFad, { db });
  ok("stub proposal passes validateProposal and returns analyze_feature_adoption", pendFad.length === 1 && pendFad[0].kind === "analyze_feature_adoption");
  const apprFad = await approveFad(orgFad, pendFad[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes feature_adoption_runs", apprFad.ok && apprFad.recordTable === "feature_adoption_runs", JSON.stringify(apprFad));
  const { data: fadRows } = await db.from("feature_adoption_runs").select("org_id").eq("org_id", orgFad);
  ok("feature adoption record org-stamped", fadRows?.length === 1 && fadRows[0].org_id === orgFad);
  const { routePayload: routeFad } = await import("./lib/manager");
  const routeCheckFadFin = await routeFad({ orgId: orgFad, payloadId: payloadFad }, { db, enqueue: () => {} });
  const { data: plainPayloadFad } = await db.from("inbound_payloads").insert({
    org_id: orgFad, source: "upload", storage_path: `${orgFad}/fad/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckFadNonFin = await routeFad({ orgId: orgFad, payloadId: plainPayloadFad!.id }, { db, enqueue: () => {} });
  ok("feature_adoption_agent routes on BOTH the financial and non-financial route",
    routeCheckFadFin.ok && routeCheckFadFin.plan.includes("feature_adoption_agent") &&
    routeCheckFadNonFin.ok && routeCheckFadNonFin.plan.includes("feature_adoption_agent"));
  await db.from("organizations").delete().eq("id", orgFad);

  console.log("== customer health score agent ==");
  ok("score_customer_health accepts good", validateProposal("score_customer_health", {
    scored_customers: [{ customer_name: "Acme Corp", health_score: 87, health_category: "green", top_risk_factor: "None" }],
    score_components: [{ component_name: "Product Engagement", weight_pct: 25, description: "Feature usage" }],
    health_distribution: { green_count: 412, yellow_count: 298, red_count: 137, avg_score: 61.4 },
    high_risk_customers: [], champion_customers: [], model_confidence: "medium",
    recommended_playbooks: [{ playbook_name: "Red Alert", trigger_condition: "Score < 45", target_segment: "All red" }],
  }).ok);
  ok("score_customer_health rejects empty scored_customers array", !validateProposal("score_customer_health", {
    scored_customers: [],
    score_components: [{ component_name: "X", weight_pct: 100, description: "d" }],
    health_distribution: { green_count: 0, yellow_count: 0, red_count: 0, avg_score: 0 },
    high_risk_customers: [], champion_customers: [], model_confidence: "medium",
    recommended_playbooks: [{ playbook_name: "P", trigger_condition: "T", target_segment: "S" }],
  }).ok);
  ok("score_customer_health filters out scored_customers item with health_score > 100", (() => {
    const r = validateProposal("score_customer_health", {
      scored_customers: [
        { customer_name: "Acme Corp", health_score: 87, health_category: "green", top_risk_factor: "None" },
        { customer_name: "Bad Corp", health_score: 150, health_category: "green", top_risk_factor: "None" },
      ],
      score_components: [{ component_name: "X", weight_pct: 100, description: "d" }],
      health_distribution: { green_count: 1, yellow_count: 0, red_count: 0, avg_score: 87 },
      high_risk_customers: [], champion_customers: [], model_confidence: "medium",
      recommended_playbooks: [{ playbook_name: "P", trigger_condition: "T", target_segment: "S" }],
    });
    return r.ok && (r.payload.scored_customers as unknown[]).length === 1;
  })());
  ok("score_customer_health rejects bad model_confidence", !validateProposal("score_customer_health", {
    scored_customers: [{ customer_name: "Acme Corp", health_score: 87, health_category: "green", top_risk_factor: "None" }],
    score_components: [{ component_name: "X", weight_pct: 100, description: "d" }],
    health_distribution: { green_count: 1, yellow_count: 0, red_count: 0, avg_score: 87 },
    high_risk_customers: [], champion_customers: [], model_confidence: "extreme",
    recommended_playbooks: [{ playbook_name: "P", trigger_condition: "T", target_segment: "S" }],
  }).ok);
  ok("customer_health_score_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("customer_health_score_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentChs } = await import("./lib/run-agent");
  const { stubBrain: sbChs } = await import("./lib/agent-brain");
  const { approveAction: approveChs, listPending: listChs } = await import("./lib/actions-service");
  const orgChs = await makeOrg("pro");
  const payloadChs = await makePayload(orgChs);
  const rChs = await runAgentChs({ orgId: orgChs, payloadId: payloadChs, role: "customer_health_score_agent" }, { db, brain: sbChs });
  ok("customer_health_score_agent run produced an analysis", rChs.ok && rChs.proposalCount === 1);
  const pendChs = await listChs(orgChs, { db });
  ok("stub proposal passes validateProposal and returns score_customer_health", pendChs.length === 1 && pendChs[0].kind === "score_customer_health");
  const apprChs = await approveChs(orgChs, pendChs[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes customer_health_score_runs", apprChs.ok && apprChs.recordTable === "customer_health_score_runs", JSON.stringify(apprChs));
  const { data: chsRows } = await db.from("customer_health_score_runs").select("org_id").eq("org_id", orgChs);
  ok("customer health score record org-stamped", chsRows?.length === 1 && chsRows[0].org_id === orgChs);
  const { routePayload: routeChs } = await import("./lib/manager");
  const routeCheckChsFin = await routeChs({ orgId: orgChs, payloadId: payloadChs }, { db, enqueue: () => {} });
  const { data: plainPayloadChs } = await db.from("inbound_payloads").insert({
    org_id: orgChs, source: "upload", storage_path: `${orgChs}/chs/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckChsNonFin = await routeChs({ orgId: orgChs, payloadId: plainPayloadChs!.id }, { db, enqueue: () => {} });
  ok("customer_health_score_agent routes on BOTH the financial and non-financial route",
    routeCheckChsFin.ok && routeCheckChsFin.plan.includes("customer_health_score_agent") &&
    routeCheckChsNonFin.ok && routeCheckChsNonFin.plan.includes("customer_health_score_agent"));
  await db.from("organizations").delete().eq("id", orgChs);

  console.log("== quota attainment agent ==");
  ok("analyze_quota_attainment accepts good", validateProposal("analyze_quota_attainment", {
    period: "Q1 2024", team_attainment_pct: 84.2,
    rep_attainments: [{ rep_name: "Sarah Chen", quota: 250000, actual: 312000, attainment_pct: 124.8 }],
    attainment_distribution: { pct_above_100: 28.4, pct_75_to_100: 41.2, pct_50_to_75: 18.6, pct_below_50: 11.8 },
    at_risk_reps: [], top_performers: [], quota_setting_assessment: "balanced",
    recommendations: ["Pipeline audit"],
  }).ok);
  ok("analyze_quota_attainment rejects empty period", !validateProposal("analyze_quota_attainment", {
    period: "", team_attainment_pct: 50,
    rep_attainments: [{ rep_name: "A", quota: 100, actual: 50, attainment_pct: 50 }],
    attainment_distribution: { pct_above_100: 0, pct_75_to_100: 0, pct_50_to_75: 100, pct_below_50: 0 },
    at_risk_reps: [], top_performers: [], quota_setting_assessment: "balanced", recommendations: ["x"],
  }).ok);
  ok("analyze_quota_attainment rejects empty rep_attainments array", !validateProposal("analyze_quota_attainment", {
    period: "Q1", team_attainment_pct: 50,
    rep_attainments: [],
    attainment_distribution: { pct_above_100: 0, pct_75_to_100: 0, pct_50_to_75: 100, pct_below_50: 0 },
    at_risk_reps: [], top_performers: [], quota_setting_assessment: "balanced", recommendations: ["x"],
  }).ok);
  ok("analyze_quota_attainment rejects bad quota_setting_assessment", !validateProposal("analyze_quota_attainment", {
    period: "Q1", team_attainment_pct: 50,
    rep_attainments: [{ rep_name: "A", quota: 100, actual: 50, attainment_pct: 50 }],
    attainment_distribution: { pct_above_100: 0, pct_75_to_100: 0, pct_50_to_75: 100, pct_below_50: 0 },
    at_risk_reps: [], top_performers: [], quota_setting_assessment: "excessive", recommendations: ["x"],
  }).ok);
  ok("quota_attainment_agent → haiku model",
    (await import("./lib/agent-brain")).modelForRole("quota_attainment_agent") === "claude-haiku-4-5-20251001");

  const { runAgent: runAgentQat } = await import("./lib/run-agent");
  const { stubBrain: sbQat } = await import("./lib/agent-brain");
  const { approveAction: approveQat, listPending: listQat } = await import("./lib/actions-service");
  const orgQat = await makeOrg("pro");
  const payloadQat = await makePayload(orgQat);
  const rQat = await runAgentQat({ orgId: orgQat, payloadId: payloadQat, role: "quota_attainment_agent" }, { db, brain: sbQat });
  ok("quota_attainment_agent run produced an analysis", rQat.ok && rQat.proposalCount === 1);
  const pendQat = await listQat(orgQat, { db });
  ok("stub proposal passes validateProposal and returns analyze_quota_attainment", pendQat.length === 1 && pendQat[0].kind === "analyze_quota_attainment");
  const apprQat = await approveQat(orgQat, pendQat[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes quota_attainment_runs", apprQat.ok && apprQat.recordTable === "quota_attainment_runs", JSON.stringify(apprQat));
  const { data: qatRows } = await db.from("quota_attainment_runs").select("org_id").eq("org_id", orgQat);
  ok("quota attainment record org-stamped", qatRows?.length === 1 && qatRows[0].org_id === orgQat);
  const { routePayload: routeQat } = await import("./lib/manager");
  const routeCheckQat = await routeQat({ orgId: orgQat, payloadId: payloadQat }, { db, enqueue: () => {} });
  ok("quota_attainment_agent routes on the financial route", routeCheckQat.ok && routeCheckQat.plan.includes("quota_attainment_agent"));
  await db.from("organizations").delete().eq("id", orgQat);

  console.log("== win loss agent ==");
  ok("analyze_win_loss accepts good", validateProposal("analyze_win_loss", {
    period: "Q1 2024", total_opportunities: 284, won_count: 119, lost_count: 165, win_rate_pct: 41.9,
    win_factors: [{ factor: "Executive sponsor engaged", mention_frequency_pct: 78, impact: "high" }],
    loss_factors: [{ factor: "Price constraint", mention_frequency_pct: 52, impact: "high" }],
    competitor_analysis: [], deal_characteristic_analysis: [], win_rate_by_segment: [],
    recommendations: ["Mandate exec sponsor by Stage 2"],
  }).ok);
  ok("analyze_win_loss rejects win_rate_pct > 100", !validateProposal("analyze_win_loss", {
    period: "Q1", total_opportunities: 10, won_count: 5, lost_count: 5, win_rate_pct: 150,
    win_factors: [{ factor: "X", mention_frequency_pct: 50, impact: "medium" }],
    loss_factors: [{ factor: "Y", mention_frequency_pct: 50, impact: "medium" }],
    competitor_analysis: [], deal_characteristic_analysis: [], win_rate_by_segment: [], recommendations: ["x"],
  }).ok);
  ok("analyze_win_loss rejects empty win_factors array", !validateProposal("analyze_win_loss", {
    period: "Q1", total_opportunities: 10, won_count: 5, lost_count: 5, win_rate_pct: 50,
    win_factors: [],
    loss_factors: [{ factor: "Y", mention_frequency_pct: 50, impact: "medium" }],
    competitor_analysis: [], deal_characteristic_analysis: [], win_rate_by_segment: [], recommendations: ["x"],
  }).ok);
  ok("analyze_win_loss filters out win_factors item with bad impact", (() => {
    const r = validateProposal("analyze_win_loss", {
      period: "Q1", total_opportunities: 10, won_count: 5, lost_count: 5, win_rate_pct: 50,
      win_factors: [
        { factor: "X", mention_frequency_pct: 50, impact: "medium" },
        { factor: "Z", mention_frequency_pct: 50, impact: "extreme" },
      ],
      loss_factors: [{ factor: "Y", mention_frequency_pct: 50, impact: "medium" }],
      competitor_analysis: [], deal_characteristic_analysis: [], win_rate_by_segment: [], recommendations: ["x"],
    });
    return r.ok && (r.payload.win_factors as unknown[]).length === 1;
  })());
  ok("win_loss_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("win_loss_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentWls } = await import("./lib/run-agent");
  const { stubBrain: sbWls } = await import("./lib/agent-brain");
  const { approveAction: approveWls, listPending: listWls } = await import("./lib/actions-service");
  const orgWls = await makeOrg("pro");
  const payloadWls = await makePayload(orgWls);
  const rWls = await runAgentWls({ orgId: orgWls, payloadId: payloadWls, role: "win_loss_agent" }, { db, brain: sbWls });
  ok("win_loss_agent run produced an analysis", rWls.ok && rWls.proposalCount === 1);
  const pendWls = await listWls(orgWls, { db });
  ok("stub proposal passes validateProposal and returns analyze_win_loss", pendWls.length === 1 && pendWls[0].kind === "analyze_win_loss");
  const apprWls = await approveWls(orgWls, pendWls[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes win_loss_runs", apprWls.ok && apprWls.recordTable === "win_loss_runs", JSON.stringify(apprWls));
  const { data: wlsRows } = await db.from("win_loss_runs").select("org_id").eq("org_id", orgWls);
  ok("win loss record org-stamped", wlsRows?.length === 1 && wlsRows[0].org_id === orgWls);
  const { routePayload: routeWls } = await import("./lib/manager");
  const routeCheckWls = await routeWls({ orgId: orgWls, payloadId: payloadWls }, { db, enqueue: () => {} });
  ok("win_loss_agent routes on the financial route", routeCheckWls.ok && routeCheckWls.plan.includes("win_loss_agent"));
  await db.from("organizations").delete().eq("id", orgWls);

  console.log("== forecast accuracy agent ==");
  ok("analyze_forecast_accuracy accepts good", validateProposal("analyze_forecast_accuracy", {
    period: "Q1 2024",
    forecast_vs_actual: [{ label: "January", forecasted: 420000, actual: 365000, accuracy_pct: 86.9, variance: -55000 }],
    overall_accuracy_pct: 86.9, bias_direction: "over_forecast", mape: 13.3,
    period_accuracy_trend: [{ period: "Q1 2024", mape: 13.3 }],
    rep_accuracy_breakdown: [{ rep_name: "Sarah Chen", mape: 5.1, bias: "Conservative" }],
    pipeline_quality_flags: ["Flag one"],
    recommendations: ["Coach rep on forecasting"],
  }).ok);
  ok("analyze_forecast_accuracy rejects empty period", !validateProposal("analyze_forecast_accuracy", {
    period: "",
    forecast_vs_actual: [{ label: "January", forecasted: 420000, actual: 365000, accuracy_pct: 86.9, variance: -55000 }],
    overall_accuracy_pct: 86.9, bias_direction: "over_forecast", mape: 13.3,
    period_accuracy_trend: [], rep_accuracy_breakdown: [], pipeline_quality_flags: [], recommendations: ["x"],
  }).ok);
  ok("analyze_forecast_accuracy rejects bad bias_direction", !validateProposal("analyze_forecast_accuracy", {
    period: "Q1 2024",
    forecast_vs_actual: [{ label: "January", forecasted: 420000, actual: 365000, accuracy_pct: 86.9, variance: -55000 }],
    overall_accuracy_pct: 86.9, bias_direction: "sideways", mape: 13.3,
    period_accuracy_trend: [], rep_accuracy_breakdown: [], pipeline_quality_flags: [], recommendations: ["x"],
  }).ok);
  ok("analyze_forecast_accuracy filters out forecast_vs_actual item with empty label", (() => {
    const r = validateProposal("analyze_forecast_accuracy", {
      period: "Q1 2024",
      forecast_vs_actual: [
        { label: "January", forecasted: 420000, actual: 365000, accuracy_pct: 86.9, variance: -55000 },
        { label: "", forecasted: 100, actual: 90, accuracy_pct: 90, variance: -10 },
      ],
      overall_accuracy_pct: 86.9, bias_direction: "over_forecast", mape: 13.3,
      period_accuracy_trend: [], rep_accuracy_breakdown: [], pipeline_quality_flags: [], recommendations: ["x"],
    });
    return r.ok && (r.payload.forecast_vs_actual as unknown[]).length === 1;
  })());
  ok("forecast_accuracy_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("forecast_accuracy_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentFca } = await import("./lib/run-agent");
  const { stubBrain: sbFca } = await import("./lib/agent-brain");
  const { approveAction: approveFca, listPending: listFca } = await import("./lib/actions-service");
  const orgFca = await makeOrg("pro");
  const payloadFca = await makePayload(orgFca);
  const rFca = await runAgentFca({ orgId: orgFca, payloadId: payloadFca, role: "forecast_accuracy_agent" }, { db, brain: sbFca });
  ok("forecast_accuracy_agent run produced an analysis", rFca.ok && rFca.proposalCount === 1);
  const pendFca = await listFca(orgFca, { db });
  ok("stub proposal passes validateProposal and returns analyze_forecast_accuracy", pendFca.length === 1 && pendFca[0].kind === "analyze_forecast_accuracy");
  const apprFca = await approveFca(orgFca, pendFca[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes forecast_accuracy_runs", apprFca.ok && apprFca.recordTable === "forecast_accuracy_runs", JSON.stringify(apprFca));
  const { data: fcaRows } = await db.from("forecast_accuracy_runs").select("org_id").eq("org_id", orgFca);
  ok("forecast accuracy record org-stamped", fcaRows?.length === 1 && fcaRows[0].org_id === orgFca);
  const { routePayload: routeFca } = await import("./lib/manager");
  const routeCheckFca = await routeFca({ orgId: orgFca, payloadId: payloadFca }, { db, enqueue: () => {} });
  ok("forecast_accuracy_agent routes on the financial route", routeCheckFca.ok && routeCheckFca.plan.includes("forecast_accuracy_agent"));
  await db.from("organizations").delete().eq("id", orgFca);

  console.log("== attribution agent ==");
  ok("analyze_attribution accepts good", validateProposal("analyze_attribution", {
    attribution_model: "linear", total_revenue_attributed: 1245000,
    channel_attribution: [{ channel: "Organic Search", attributed_revenue: 398400, attributed_pct: 32.0, conversion_count: 48, avg_deal_size: 8300 }],
    top_converting_paths: [], assisted_vs_direct: { direct_revenue_pct: 22.4, assisted_revenue_pct: 77.6, multi_touch_deal_pct: 81.3 },
    channel_efficiency: [], recommendations: ["Double organic search investment"],
  }).ok);
  ok("analyze_attribution rejects bad attribution_model", !validateProposal("analyze_attribution", {
    attribution_model: "gut_feeling", total_revenue_attributed: 1245000,
    channel_attribution: [{ channel: "Organic Search", attributed_revenue: 398400, attributed_pct: 32.0, conversion_count: 48, avg_deal_size: 8300 }],
    top_converting_paths: [], assisted_vs_direct: { direct_revenue_pct: 22.4, assisted_revenue_pct: 77.6, multi_touch_deal_pct: 81.3 },
    channel_efficiency: [], recommendations: ["x"],
  }).ok);
  ok("analyze_attribution rejects empty channel_attribution array", !validateProposal("analyze_attribution", {
    attribution_model: "linear", total_revenue_attributed: 1245000,
    channel_attribution: [],
    top_converting_paths: [], assisted_vs_direct: { direct_revenue_pct: 22.4, assisted_revenue_pct: 77.6, multi_touch_deal_pct: 81.3 },
    channel_efficiency: [], recommendations: ["x"],
  }).ok);
  ok("analyze_attribution filters out channel_attribution item with empty channel", (() => {
    const r = validateProposal("analyze_attribution", {
      attribution_model: "linear", total_revenue_attributed: 1245000,
      channel_attribution: [
        { channel: "Organic Search", attributed_revenue: 398400, attributed_pct: 32.0, conversion_count: 48, avg_deal_size: 8300 },
        { channel: "", attributed_revenue: 100, attributed_pct: 1, conversion_count: 1, avg_deal_size: 100 },
      ],
      top_converting_paths: [], assisted_vs_direct: { direct_revenue_pct: 22.4, assisted_revenue_pct: 77.6, multi_touch_deal_pct: 81.3 },
      channel_efficiency: [], recommendations: ["x"],
    });
    return r.ok && (r.payload.channel_attribution as unknown[]).length === 1;
  })());
  ok("attribution_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("attribution_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentAtn } = await import("./lib/run-agent");
  const { stubBrain: sbAtn } = await import("./lib/agent-brain");
  const { approveAction: approveAtn, listPending: listAtn } = await import("./lib/actions-service");
  const orgAtn = await makeOrg("pro");
  const payloadAtn = await makePayload(orgAtn);
  const rAtn = await runAgentAtn({ orgId: orgAtn, payloadId: payloadAtn, role: "attribution_agent" }, { db, brain: sbAtn });
  ok("attribution_agent run produced an analysis", rAtn.ok && rAtn.proposalCount === 1);
  const pendAtn = await listAtn(orgAtn, { db });
  ok("stub proposal passes validateProposal and returns analyze_attribution", pendAtn.length === 1 && pendAtn[0].kind === "analyze_attribution");
  const apprAtn = await approveAtn(orgAtn, pendAtn[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes attribution_runs", apprAtn.ok && apprAtn.recordTable === "attribution_runs", JSON.stringify(apprAtn));
  const { data: atnRows } = await db.from("attribution_runs").select("org_id").eq("org_id", orgAtn);
  ok("attribution record org-stamped", atnRows?.length === 1 && atnRows[0].org_id === orgAtn);
  const { routePayload: routeAtn } = await import("./lib/manager");
  const routeCheckAtn = await routeAtn({ orgId: orgAtn, payloadId: payloadAtn }, { db, enqueue: () => {} });
  ok("attribution_agent routes on the financial route", routeCheckAtn.ok && routeCheckAtn.plan.includes("attribution_agent"));
  await db.from("organizations").delete().eq("id", orgAtn);

  console.log("== price elasticity agent ==");
  ok("analyze_price_elasticity accepts good", validateProposal("analyze_price_elasticity", {
    product_analyzed: "Pro Plan Monthly Subscription",
    price_points: [
      { price: 49, demand_units: 1420, revenue: 69580, time_period: "Q1 2023" },
      { price: 59, demand_units: 1298, revenue: 76582, time_period: "Q2 2023" },
    ],
    elasticity_coefficient: -1.18, elasticity_classification: "elastic",
    revenue_maximizing_price: 64, current_price: 79, price_increase_headroom_pct: -18.99,
    segment_elasticity: [], competitive_context: "Three comparable products price between $49-89/month.",
    recommendations: ["Consider price reduction"],
  }).ok);
  ok("analyze_price_elasticity rejects empty product_analyzed", !validateProposal("analyze_price_elasticity", {
    product_analyzed: "",
    price_points: [
      { price: 49, demand_units: 1420, revenue: 69580, time_period: "Q1 2023" },
      { price: 59, demand_units: 1298, revenue: 76582, time_period: "Q2 2023" },
    ...
    elasticity_coefficient: -1.18, elasticity_classification: "elastic",
    revenue_maximizing_price: 64, current_price: 79, price_increase_headroom_pct: -18.99,
    segment_elasticity: [], competitive_context: "x", recommendations: ["x"],
  }).ok);
  ok("analyze_price_elasticity rejects fewer than 2 price_points", !validateProposal("analyze_price_elasticity", {
    product_analyzed: "Pro Plan",
    price_points: [{ price: 49, demand_units: 1420, revenue: 69580, time_period: "Q1 2023" }],
    elasticity_coefficient: -1.18, elasticity_classification: "elastic",
    revenue_maximizing_price: 64, current_price: 79, price_increase_headroom_pct: -18.99,
    segment_elasticity: [], competitive_context: "x", recommendations: ["x"],
  }).ok);
  ok("analyze_price_elasticity rejects bad elasticity_classification", !validateProposal("analyze_price_elasticity", {
    product_analyzed: "Pro Plan",
    price_points: [
      { price: 49, demand_units: 1420, revenue: 69580, time_period: "Q1 2023" },
      { price: 59, demand_units: 1298, revenue: 76582, time_period: "Q2 2023" },
    ],
    elasticity_coefficient: -1.18, elasticity_classification: "very_bendy",
    revenue_maximizing_price: 64, current_price: 79, price_increase_headroom_pct: -18.99,
    segment_elasticity: [], competitive_context: "x", recommendations: ["x"],
  }).ok);
  ok("price_elasticity_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("price_elasticity_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentPel } = await import("./lib/run-agent");
  const { stubBrain: sbPel } = await import("./lib/agent-brain");
  const { approveAction: approvePel, listPending: listPel } = await import("./lib/actions-service");
  const orgPel = await makeOrg("pro");
  const payloadPel = await makePayload(orgPel);
  const rPel = await runAgentPel({ orgId: orgPel, payloadId: payloadPel, role: "price_elasticity_agent" }, { db, brain: sbPel });
  ok("price_elasticity_agent run produced an analysis", rPel.ok && rPel.proposalCount === 1);
  const pendPel = await listPel(orgPel, { db });
  ok("stub proposal passes validateProposal and returns analyze_price_elasticity", pendPel.length === 1 && pendPel[0].kind === "analyze_price_elasticity");
  const apprPel = await approvePel(orgPel, pendPel[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes price_elasticity_runs", apprPel.ok && apprPel.recordTable === "price_elasticity_runs", JSON.stringify(apprPel));
  const { data: pelRows } = await db.from("price_elasticity_runs").select("org_id").eq("org_id", orgPel);
  ok("price elasticity record org-stamped", pelRows?.length === 1 && pelRows[0].org_id === orgPel);
  const { routePayload: routePel } = await import("./lib/manager");
  const routeCheckPel = await routePel({ orgId: orgPel, payloadId: payloadPel }, { db, enqueue: () => {} });
  ok("price_elasticity_agent routes on the financial route", routeCheckPel.ok && routeCheckPel.plan.includes("price_elasticity_agent"));
  await db.from("organizations").delete().eq("id", orgPel);

  console.log("== expansion opportunity agent ==");
  ok("analyze_expansion_opportunity accepts good", validateProposal("analyze_expansion_opportunity", {
    total_expansion_potential: 847000,
    opportunity_accounts: [{ account_name: "Acme Corp", current_arr: 120000, expansion_potential: 48000, expansion_type: "upsell", readiness_score: 88, recommended_play: "Upgrade to Enterprise tier" }],
    expansion_types: [{ type: "upsell", opportunity_count: 23, total_potential: 312000 }],
    readiness_distribution: { hot_count: 28, warm_count: 54, cold_count: 35 },
    prioritized_plays: [{ play_name: "Power User Upgrade Campaign", target_segment: "Pro users on 9+ features", expected_arr_lift: 180000, effort: "low" }],
    expansion_risk_flags: ["Beta Industries has an outstanding AR balance"],
  }).ok);
  ok("analyze_expansion_opportunity rejects empty opportunity_accounts array", !validateProposal("analyze_expansion_opportunity", {
    total_expansion_potential: 847000,
    opportunity_accounts: [],
    expansion_types: [], readiness_distribution: { hot_count: 28, warm_count: 54, cold_count: 35 },
    prioritized_plays: [{ play_name: "x", target_segment: "y", expected_arr_lift: 1000, effort: "low" }],
    expansion_risk_flags: [],
  }).ok);
  ok("analyze_expansion_opportunity filters out opportunity_accounts item with bad expansion_type", (() => {
    const r = validateProposal("analyze_expansion_opportunity", {
      total_expansion_potential: 847000,
      opportunity_accounts: [
        { account_name: "Acme Corp", current_arr: 120000, expansion_potential: 48000, expansion_type: "upsell", readiness_score: 88, recommended_play: "x" },
        { account_name: "Bad Co", current_arr: 1000, expansion_potential: 100, expansion_type: "downsell", readiness_score: 50, recommended_play: "y" },
      ],
      expansion_types: [], readiness_distribution: { hot_count: 28, warm_count: 54, cold_count: 35 },
      prioritized_plays: [{ play_name: "x", target_segment: "y", expected_arr_lift: 1000, effort: "low" }],
      expansion_risk_flags: [],
    });
    return r.ok && (r.payload.opportunity_accounts as unknown[]).length === 1;
  })());
  ok("analyze_expansion_opportunity rejects empty prioritized_plays array", !validateProposal("analyze_expansion_opportunity", {
    total_expansion_potential: 847000,
    opportunity_accounts: [{ account_name: "Acme Corp", current_arr: 120000, expansion_potential: 48000, expansion_type: "upsell", readiness_score: 88, recommended_play: "x" }],
    expansion_types: [], readiness_distribution: { hot_count: 28, warm_count: 54, cold_count: 35 },
    prioritized_plays: [],
    expansion_risk_flags: [],
  }).ok);
  ok("expansion_opportunity_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("expansion_opportunity_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentExo } = await import("./lib/run-agent");
  const { stubBrain: sbExo } = await import("./lib/agent-brain");
  const { approveAction: approveExo, listPending: listExo } = await import("./lib/actions-service");
  const orgExo = await makeOrg("pro");
  const payloadExo = await makePayload(orgExo);
  const rExo = await runAgentExo({ orgId: orgExo, payloadId: payloadExo, role: "expansion_opportunity_agent" }, { db, brain: sbExo });
  ok("expansion_opportunity_agent run produced an analysis", rExo.ok && rExo.proposalCount === 1);
  const pendExo = await listExo(orgExo, { db });
  ok("stub proposal passes validateProposal and returns analyze_expansion_opportunity", pendExo.length === 1 && pendExo[0].kind === "analyze_expansion_opportunity");
  const apprExo = await approveExo(orgExo, pendExo[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes expansion_opportunity_runs", apprExo.ok && apprExo.recordTable === "expansion_opportunity_runs", JSON.stringify(apprExo));
  const { data: exoRows } = await db.from("expansion_opportunity_runs").select("org_id").eq("org_id", orgExo);
  ok("expansion opportunity record org-stamped", exoRows?.length === 1 && exoRows[0].org_id === orgExo);
  const { routePayload: routeExo } = await import("./lib/manager");
  const routeCheckExoFin = await routeExo({ orgId: orgExo, payloadId: payloadExo }, { db, enqueue: () => {} });
  const { data: plainPayloadExo } = await db.from("inbound_payloads").insert({
    org_id: orgExo, source: "upload", storage_path: `${orgExo}/exo/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckExoNonFin = await routeExo({ orgId: orgExo, payloadId: plainPayloadExo!.id }, { db, enqueue: () => {} });
  ok("expansion_opportunity_agent routes on BOTH the financial and non-financial route",
    routeCheckExoFin.ok && routeCheckExoFin.plan.includes("expansion_opportunity_agent") &&
    routeCheckExoNonFin.ok && routeCheckExoNonFin.plan.includes("expansion_opportunity_agent"));
  await db.from("organizations").delete().eq("id", orgExo);


  console.log("== attrition analysis agent ==");
  ok("analyze_attrition accepts good", validateProposal("analyze_attrition", {
    period: "Q1 2024", total_headcount: 248, voluntary_exits: 14, involuntary_exits: 3,
    voluntary_attrition_rate_pct: 5.6, annualized_attrition_rate_pct: 22.6,
    attrition_by_department: [{ department: "Engineering", headcount: 82, exits: 9, attrition_rate_pct: 11.0 }],
    attrition_by_tenure: [{ tenure_band: "0-6 months", headcount: 44, exits: 6, attrition_rate_pct: 13.6 }],
    attrition_by_level: [{ level: "IC", headcount: 148, exits: 11, attrition_rate_pct: 7.4 }],
    flight_risk_flags: [{ flag: "Engineering attrition elevated", affected_count: 82, severity: "high" }],
    cost_of_attrition: { total_estimated_cost: 1750000, cost_per_exit: 125000, methodology_note: "x" },
    retention_recommendations: ["Conduct stay interviews"],
  }).ok);
  ok("analyze_attrition rejects voluntary_attrition_rate_pct > 100", !validateProposal("analyze_attrition", {
    period: "Q1 2024", total_headcount: 248, voluntary_exits: 14, involuntary_exits: 3,
    voluntary_attrition_rate_pct: 150, annualized_attrition_rate_pct: 22.6,
    attrition_by_department: [], attrition_by_tenure: [], attrition_by_level: [], flight_risk_flags: [],
    cost_of_attrition: { total_estimated_cost: 1750000, cost_per_exit: 125000, methodology_note: "x" },
    retention_recommendations: ["x"],
  }).ok);
  ok("analyze_attrition rejects empty period", !validateProposal("analyze_attrition", {
    period: "", total_headcount: 248, voluntary_exits: 14, involuntary_exits: 3,
    voluntary_attrition_rate_pct: 5.6, annualized_attrition_rate_pct: 22.6,
    attrition_by_department: [], attrition_by_tenure: [], attrition_by_level: [], flight_risk_flags: [],
    cost_of_attrition: { total_estimated_cost: 1750000, cost_per_exit: 125000, methodology_note: "x" },
    retention_recommendations: ["x"],
  }).ok);
  ok("analyze_attrition filters out flight_risk_flags item with bad severity", (() => {
    const r = validateProposal("analyze_attrition", {
      period: "Q1 2024", total_headcount: 248, voluntary_exits: 14, involuntary_exits: 3,
      voluntary_attrition_rate_pct: 5.6, annualized_attrition_rate_pct: 22.6,
      attrition_by_department: [], attrition_by_tenure: [], attrition_by_level: [],
      flight_risk_flags: [
        { flag: "Good flag", affected_count: 82, severity: "high" },
        { flag: "Bad flag", affected_count: 10, severity: "extreme" },
      ],
      cost_of_attrition: { total_estimated_cost: 1750000, cost_per_exit: 125000, methodology_note: "x" },
      retention_recommendations: ["x"],
    });
    return r.ok && (r.payload.flight_risk_flags as unknown[]).length === 1;
  })());
  ok("attrition_analysis_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("attrition_analysis_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentAtr } = await import("./lib/run-agent");
  const { stubBrain: sbAtr } = await import("./lib/agent-brain");
  const { approveAction: approveAtr, listPending: listAtr } = await import("./lib/actions-service");
  const orgAtr = await makeOrg("pro");
  const payloadAtr = await makePayload(orgAtr);
  const rAtr = await runAgentAtr({ orgId: orgAtr, payloadId: payloadAtr, role: "attrition_analysis_agent" }, { db, brain: sbAtr });
  ok("attrition_analysis_agent run produced an analysis", rAtr.ok && rAtr.proposalCount === 1);
  const pendAtr = await listAtr(orgAtr, { db });
  ok("stub proposal passes validateProposal and returns analyze_attrition", pendAtr.length === 1 && pendAtr[0].kind === "analyze_attrition");
  const apprAtr = await approveAtr(orgAtr, pendAtr[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes attrition_analysis_runs", apprAtr.ok && apprAtr.recordTable === "attrition_analysis_runs", JSON.stringify(apprAtr));
  const { data: atrRows } = await db.from("attrition_analysis_runs").select("org_id").eq("org_id", orgAtr);
  ok("attrition analysis record org-stamped", atrRows?.length === 1 && atrRows[0].org_id === orgAtr);
  const { routePayload: routeAtr } = await import("./lib/manager");
  const routeCheckAtrFin = await routeAtr({ orgId: orgAtr, payloadId: payloadAtr }, { db, enqueue: () => {} });
  const { data: plainPayloadAtr } = await db.from("inbound_payloads").insert({
    org_id: orgAtr, source: "upload", storage_path: `${orgAtr}/atr/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckAtrNonFin = await routeAtr({ orgId: orgAtr, payloadId: plainPayloadAtr!.id }, { db, enqueue: () => {} });
  ok("attrition_analysis_agent routes on BOTH the financial and non-financial route",
    routeCheckAtrFin.ok && routeCheckAtrFin.plan.includes("attrition_analysis_agent") &&
    routeCheckAtrNonFin.ok && routeCheckAtrNonFin.plan.includes("attrition_analysis_agent"));
  await db.from("organizations").delete().eq("id", orgAtr);

  console.log("== headcount planning agent ==");
  ok("plan_headcount accepts good", validateProposal("plan_headcount", {
    planning_period: "H1 2024", current_headcount: 248, planned_headcount: 292, net_change: 44,
    department_plans: [{ department: "Engineering", current: 82, planned: 102, open_roles: 8, priority: "critical" }],
    hiring_timeline: [{ month: "Feb 2024", planned_hires: 8, cumulative_headcount: 256 }],
    ...
    capacity_gaps: [{ area: "Backend Engineering", gap_description: "3 unfilled roles", hire_count_needed: 3, urgency: "immediate" }],
    hiring_risks: ["Market conditions"],
  }).ok);
  ok("plan_headcount rejects empty department_plans array", !validateProposal("plan_headcount", {
    planning_period: "H1 2024", current_headcount: 248, planned_headcount: 292, net_change: 44,
    department_plans: [], hiring_timeline: [],
    cost_impact: { current_annual_cost: 38688000, planned_annual_cost: 45552000, incremental_cost: 6864000, avg_fully_loaded_cost: 156000 },
    capacity_gaps: [], hiring_risks: [],
  }).ok);
  ok("plan_headcount rejects empty planning_period", !validateProposal("plan_headcount", {
    planning_period: "", current_headcount: 248, planned_headcount: 292, net_change: 44,
    department_plans: [{ department: "Engineering", current: 82, planned: 102, open_roles: 8, priority: "critical" }],
    hiring_timeline: [],
    cost_impact: { current_annual_cost: 38688000, planned_annual_cost: 45552000, incremental_cost: 6864000, avg_fully_loaded_cost: 156000 },
    capacity_gaps: [], hiring_risks: [],
  }).ok);
  ok("plan_headcount filters out department_plans item with bad priority", (() => {
    const r = validateProposal("plan_headcount", {
      planning_period: "H1 2024", current_headcount: 248, planned_headcount: 292, net_change: 44,
      department_plans: [
        { department: "Engineering", current: 82, planned: 102, open_roles: 8, priority: "critical" },
        { department: "Sales", current: 51, planned: 68, open_roles: 5, priority: "urgent" },
      ],
      hiring_timeline: [],
      cost_impact: { current_annual_cost: 38688000, planned_annual_cost: 45552000, incremental_cost: 6864000, avg_fully_loaded_cost: 156000 },
      capacity_gaps: [], hiring_risks: [],
    });
    return r.ok && (r.payload.department_plans as unknown[]).length === 1;
  })());
  ok("headcount_planning_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("headcount_planning_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentHcp } = await import("./lib/run-agent");
  const { stubBrain: sbHcp } = await import("./lib/agent-brain");
  const { approveAction: approveHcp, listPending: listHcp } = await import("./lib/actions-service");
  const orgHcp = await makeOrg("pro");
  const payloadHcp = await makePayload(orgHcp);
  const rHcp = await runAgentHcp({ orgId: orgHcp, payloadId: payloadHcp, role: "headcount_planning_agent" }, { db, brain: sbHcp });
  ok("headcount_planning_agent run produced an analysis", rHcp.ok && rHcp.proposalCount === 1);
  const pendHcp = await listHcp(orgHcp, { db });
  ok("stub proposal passes validateProposal and returns plan_headcount", pendHcp.length === 1 && pendHcp[0].kind === "plan_headcount");
  const apprHcp = await approveHcp(orgHcp, pendHcp[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes headcount_planning_runs", apprHcp.ok && apprHcp.recordTable === "headcount_planning_runs", JSON.stringify(apprHcp));
  const { data: hcpRows } = await db.from("headcount_planning_runs").select("org_id").eq("org_id", orgHcp);
  ok("headcount planning record org-stamped", hcpRows?.length === 1 && hcpRows[0].org_id === orgHcp);
  const { routePayload: routeHcp } = await import("./lib/manager");
  const routeCheckHcpFin = await routeHcp({ orgId: orgHcp, payloadId: payloadHcp }, { db, enqueue: () => {} });
  const { data: plainPayloadHcp } = await db.from("inbound_payloads").insert({
    org_id: orgHcp, source: "upload", storage_path: `${orgHcp}/hcp/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckHcpNonFin = await routeHcp({ orgId: orgHcp, payloadId: plainPayloadHcp!.id }, { db, enqueue: () => {} });
  ok("headcount_planning_agent routes on BOTH the financial and non-financial route",
    routeCheckHcpFin.ok && routeCheckHcpFin.plan.includes("headcount_planning_agent") &&
    routeCheckHcpNonFin.ok && routeCheckHcpNonFin.plan.includes("headcount_planning_agent"));
  await db.from("organizations").delete().eq("id", orgHcp);

  console.log("== demand forecasting agent ==");
  ok("forecast_demand accepts good", validateProposal("forecast_demand", {
    metric_forecasted: "MRR", forecast_horizon: "Next 6 months",
    historical_data_summary: { period_count: 18, avg_value: 62400, growth_rate_pct: 7.8, data_quality: "sufficient" },
    forecast_periods: [{ period: "Feb 2024", forecast_value: 91800, low_estimate: 87200, high_estimate: 96400, notes: "x" }],
    methodology: "exponential_smoothing", seasonality_detected: false, trend_direction: "steady_growth",
    forecast_confidence: "medium", key_assumptions: ["Growth continues"],
    risk_scenarios: [{ scenario_name: "Delay", probability: "medium", impact_on_forecast_pct: -18, description: "x" }],
  }).ok);
  ok("forecast_demand rejects empty metric_forecasted", !validateProposal("forecast_demand", {
    metric_forecasted: "", forecast_horizon: "Next 6 months",
    historical_data_summary: { period_count: 18, avg_value: 62400, growth_rate_pct: 7.8, data_quality: "sufficient" },
    forecast_periods: [{ period: "Feb 2024", forecast_value: 91800, low_estimate: 87200, high_estimate: 96400, notes: "x" }],
    methodology: "exponential_smoothing", seasonality_detected: false, trend_direction: "steady_growth",
    forecast_confidence: "medium", key_assumptions: ["x"], risk_scenarios: [],
  }).ok);
  ok("forecast_demand rejects bad methodology", !validateProposal("forecast_demand", {
    metric_forecasted: "MRR", forecast_horizon: "Next 6 months",
    historical_data_summary: { period_count: 18, avg_value: 62400, growth_rate_pct: 7.8, data_quality: "sufficient" },
    forecast_periods: [{ period: "Feb 2024", forecast_value: 91800, low_estimate: 87200, high_estimate: 96400, notes: "x" }],
    methodology: "guesswork", seasonality_detected: false, trend_direction: "steady_growth",
    forecast_confidence: "medium", key_assumptions: ["x"], risk_scenarios: [],
  }).ok);
  ok("forecast_demand rejects bad trend_direction", !validateProposal("forecast_demand", {
    metric_forecasted: "MRR", forecast_horizon: "Next 6 months",
    historical_data_summary: { period_count: 18, avg_value: 62400, growth_rate_pct: 7.8, data_quality: "sufficient" },
    forecast_periods: [{ period: "Feb 2024", forecast_value: 91800, low_estimate: 87200, high_estimate: 96400, notes: "x" }],
    methodology: "exponential_smoothing", seasonality_detected: false, trend_direction: "wobbling",
    forecast_confidence: "medium", key_assumptions: ["x"], risk_scenarios: [],
  }).ok);
  ok("demand_forecasting_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("demand_forecasting_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentDfc } = await import("./lib/run-agent");
  const { stubBrain: sbDfc } = await import("./lib/agent-brain");
  const { approveAction: approveDfc, listPending: listDfc } = await import("./lib/actions-service");
  const orgDfc = await makeOrg("pro");
  const payloadDfc = await makePayload(orgDfc);
  const rDfc = await runAgentDfc({ orgId: orgDfc, payloadId: payloadDfc, role: "demand_forecasting_agent" }, { db, brain: sbDfc });
  ok("demand_forecasting_agent run produced an analysis", rDfc.ok && rDfc.proposalCount === 1);
  const pendDfc = await listDfc(orgDfc, { db });
  ok("stub proposal passes validateProposal and returns forecast_demand", pendDfc.length === 1 && pendDfc[0].kind === "forecast_demand");
  const apprDfc = await approveDfc(orgDfc, pendDfc[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes demand_forecasting_runs", apprDfc.ok && apprDfc.recordTable === "demand_forecasting_runs", JSON.stringify(apprDfc));
  const { data: dfcRows } = await db.from("demand_forecasting_runs").select("org_id").eq("org_id", orgDfc);
  ok("demand forecasting record org-stamped", dfcRows?.length === 1 && dfcRows[0].org_id === orgDfc);
  const { routePayload: routeDfc } = await import("./lib/manager");
  const routeCheckDfcFin = await routeDfc({ orgId: orgDfc, payloadId: payloadDfc }, { db, enqueue: () => {} });
  const { data: plainPayloadDfc } = await db.from("inbound_payloads").insert({
    org_id: orgDfc, source: "upload", storage_path: `${orgDfc}/dfc/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckDfcNonFin = await routeDfc({ orgId: orgDfc, payloadId: plainPayloadDfc!.id }, { db, enqueue: () => {} });
  ok("demand_forecasting_agent routes on BOTH the financial and non-financial route",
    routeCheckDfcFin.ok && routeCheckDfcFin.plan.includes("demand_forecasting_agent") &&
    routeCheckDfcNonFin.ok && routeCheckDfcNonFin.plan.includes("demand_forecasting_agent"));
  await db.from("organizations").delete().eq("id", orgDfc);

  console.log("== capacity planning agent ==");
  ok("plan_capacity accepts good", validateProposal("plan_capacity", {
    capacity_dimension: "CSM accounts", planning_horizon: "Q2-Q3 2024",
    current_capacity: { total_capacity: 700, utilized_capacity: 612, utilization_pct: 87.4, unit: "accounts" },
    demand_forecast: [{ period: "Apr 2024", expected_demand: 680, capacity_required: 680, surplus_or_deficit: 20 }],
    capacity_gaps: [{ period: "May 2024", gap_size: 24, gap_type: "shortage", severity: "low" }],
    utilization_analysis: { optimal_utilization_pct: 75, current_vs_optimal: "x", peak_utilization_pct: 92.4, trough_utilization_pct: 78.1 },
    constraint_analysis: [{ constraint: "CSM headcount", current_limit: "10 CSMs", expansion_feasibility: "moderate" }],
    investment_options: [{ option_name: "Hire 2 CSMs", capacity_added: 140, cost_estimate: 280000, lead_time: "90 days", tradeoffs: "x" }],
    recommendations: ["Begin hiring"],
  }).ok);
  ok("plan_capacity rejects empty capacity_dimension", !validateProposal("plan_capacity", {
    capacity_dimension: "", planning_horizon: "Q2-Q3 2024",
    current_capacity: { total_capacity: 700, utilized_capacity: 612, utilization_pct: 87.4, unit: "accounts" },
    demand_forecast: [{ period: "Apr 2024", expected_demand: 680, capacity_required: 680, surplus_or_deficit: 20 }],
    capacity_gaps: [], utilization_analysis: { optimal_utilization_pct: 75, current_vs_optimal: "x", peak_utilization_pct: 92.4, trough_utilization_pct: 78.1 },
    constraint_analysis: [], investment_options: [], recommendations: ["x"],
  }).ok);
  ok("plan_capacity rejects current_capacity with utilization_pct > 100", !validateProposal("plan_capacity", {
    capacity_dimension: "CSM accounts", planning_horizon: "Q2-Q3 2024",
    current_capacity: { total_capacity: 700, utilized_capacity: 612, utilization_pct: 150, unit: "accounts" },
    demand_forecast: [{ period: "Apr 2024", expected_demand: 680, capacity_required: 680, surplus_or_deficit: 20 }],
    capacity_gaps: [], utilization_analysis: { optimal_utilization_pct: 75, current_vs_optimal: "x", peak_utilization_pct: 92.4, trough_utilization_pct: 78.1 },
    constraint_analysis: [], investment_options: [], recommendations: ["x"],
  }).ok);
  ok("plan_capacity filters out capacity_gaps item with bad gap_type", (() => {
    const r = validateProposal("plan_capacity", {
      capacity_dimension: "CSM accounts", planning_horizon: "Q2-Q3 2024",
      current_capacity: { total_capacity: 700, utilized_capacity: 612, utilization_pct: 87.4, unit: "accounts" },
      demand_forecast: [{ period: "Apr 2024", expected_demand: 680, capacity_required: 680, surplus_or_deficit: 20 }],
      capacity_gaps: [
        { period: "May 2024", gap_size: 24, gap_type: "shortage", severity: "low" },
        { period: "Jun 2024", gap_size: 10, gap_type: "overflow", severity: "low" },
      ],
      utilization_analysis: { optimal_utilization_pct: 75, current_vs_optimal: "x", peak_utilization_pct: 92.4, trough_utilization_pct: 78.1 },
      constraint_analysis: [], investment_options: [], recommendations: ["x"],
    });
    return r.ok && (r.payload.capacity_gaps as unknown[]).length === 1;
  })());
  ok("capacity_planning_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("capacity_planning_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentCpc } = await import("./lib/run-agent");
  const { stubBrain: sbCpc } = await import("./lib/agent-brain");
  const { approveAction: approveCpc, listPending: listCpc } = await import("./lib/actions-service");
  const orgCpc = await makeOrg("pro");
  const payloadCpc = await makePayload(orgCpc);
  const rCpc = await runAgentCpc({ orgId: orgCpc, payloadId: payloadCpc, role: "capacity_planning_agent" }, { db, brain: sbCpc });
  ok("capacity_planning_agent run produced an analysis", rCpc.ok && rCpc.proposalCount === 1);
  const pendCpc = await listCpc(orgCpc, { db });
  ok("stub proposal passes validateProposal and returns plan_capacity", pendCpc.length === 1 && pendCpc[0].kind === "plan_capacity");
  const apprCpc = await approveCpc(orgCpc, pendCpc[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes capacity_planning_runs", apprCpc.ok && apprCpc.recordTable === "capacity_planning_runs", JSON.stringify(apprCpc));
  const { data: cpcRows } = await db.from("capacity_planning_runs").select("org_id").eq("org_id", orgCpc);
  ok("capacity planning record org-stamped", cpcRows?.length === 1 && cpcRows[0].org_id === orgCpc);
  const { routePayload: routeCpc } = await import("./lib/manager");
  const routeCheckCpcFin = await routeCpc({ orgId: orgCpc, payloadId: payloadCpc }, { db, enqueue: () => {} });
  const { data: plainPayloadCpc } = await db.from("inbound_payloads").insert({
    org_id: orgCpc, source: "upload", storage_path: `${orgCpc}/cpc/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckCpcNonFin = await routeCpc({ orgId: orgCpc, payloadId: plainPayloadCpc!.id }, { db, enqueue: () => {} });
  ok("capacity_planning_agent routes on BOTH the financial and non-financial route",
    routeCheckCpcFin.ok && routeCheckCpcFin.plan.includes("capacity_planning_agent") &&
    routeCheckCpcNonFin.ok && routeCheckCpcNonFin.plan.includes("capacity_planning_agent"));
  await db.from("organizations").delete().eq("id", orgCpc);

  console.log("== sla analysis agent ==");
  ok("analyze_sla accepts good", validateProposal("analyze_sla", {
    period: "March 2024",
    sla_metrics: [{ metric_name: "P1 First Response Time", target_value: 1, actual_value: 0.8, compliance_pct: 94.2, unit: "hours", breach_count: 3 }],
    overall_compliance_pct: 86.2,
    breach_summary: { total_breaches: 41, critical_breaches: 11, breach_rate_pct: 13.8 },
    worst_performing_categories: [{ category: "P1 Time to Resolution", compliance_pct: 71.4, breach_count: 8, root_cause_hypothesis: "x" }],
    trend_assessment: "declining",
    customer_impact_assessment: [{ customer_tier: "Enterprise", breaches_experienced: 18, escalation_risk: "high" }],
    improvement_actions: ["Establish dedicated P1 team"],
  }).ok);
  ok("analyze_sla rejects overall_compliance_pct > 100", !validateProposal("analyze_sla", {
    period: "March 2024",
    sla_metrics: [{ metric_name: "P1 First Response Time", target_value: 1, actual_value: 0.8, compliance_pct: 94.2, unit: "hours", breach_count: 3 }],
    overall_compliance_pct: 150,
    breach_summary: { total_breaches: 41, critical_breaches: 11, breach_rate_pct: 13.8 },
    worst_performing_categories: [], trend_assessment: "declining", customer_impact_assessment: [], improvement_actions: ["x"],
  }).ok);
  ok("analyze_sla rejects bad trend_assessment", !validateProposal("analyze_sla", {
    period: "March 2024",
    sla_metrics: [{ metric_name: "P1 First Response Time", target_value: 1, actual_value: 0.8, compliance_pct: 94.2, unit: "hours", breach_count: 3 }],
    overall_compliance_pct: 86.2,
    breach_summary: { total_breaches: 41, critical_breaches: 11, breach_rate_pct: 13.8 },
    worst_performing_categories: [], trend_assessment: "chaotic", customer_impact_assessment: [], improvement_actions: ["x"],
  }).ok);
  ok("analyze_sla filters out sla_metrics item with empty metric_name", (() => {
    const r = validateProposal("analyze_sla", {
      period: "March 2024",
      sla_metrics: [
        { metric_name: "P1 First Response Time", target_value: 1, actual_value: 0.8, compliance_pct: 94.2, unit: "hours", breach_count: 3 },
        { metric_name: "", target_value: 4, actual_value: 3.1, compliance_pct: 91.7, unit: "hours", breach_count: 12 },
      ],
      overall_compliance_pct: 86.2,
      breach_summary: { total_breaches: 41, critical_breaches: 11, breach_rate_pct: 13.8 },
      worst_performing_categories: [], trend_assessment: "declining", customer_impact_assessment: [], improvement_actions: ["x"],
    });
    return r.ok && (r.payload.sla_metrics as unknown[]).length === 1;
  })());
  ok("sla_analysis_agent → haiku model",
    (await import("./lib/agent-brain")).modelForRole("sla_analysis_agent") === "claude-haiku-4-5-20251001");

  const { runAgent: runAgentSla } = await import("./lib/run-agent");
  const { stubBrain: sbSla } = await import("./lib/agent-brain");
  const { approveAction: approveSla, listPending: listSla } = await import("./lib/actions-service");
  const orgSla = await makeOrg("pro");
  const payloadSla = await makePayload(orgSla);
  const rSla = await runAgentSla({ orgId: orgSla, payloadId: payloadSla, role: "sla_analysis_agent" }, { db, brain: sbSla });
  ok("sla_analysis_agent run produced an analysis", rSla.ok && rSla.proposalCount === 1);
  const pendSla = await listSla(orgSla, { db });
  ok("stub proposal passes validateProposal and returns analyze_sla", pendSla.length === 1 && pendSla[0].kind === "analyze_sla");
  const apprSla = await approveSla(orgSla, pendSla[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes sla_analysis_runs", apprSla.ok && apprSla.recordTable === "sla_analysis_runs", JSON.stringify(apprSla));
  const { data: slaRows } = await db.from("sla_analysis_runs").select("org_id").eq("org_id", orgSla);
  ok("sla analysis record org-stamped", slaRows?.length === 1 && slaRows[0].org_id === orgSla);
  const { routePayload: routeSla } = await import("./lib/manager");
  const routeCheckSlaFin = await routeSla({ orgId: orgSla, payloadId: payloadSla }, { db, enqueue: () => {} });
  const { data: plainPayloadSla } = await db.from("inbound_payloads").insert({
    org_id: orgSla, source: "upload", storage_path: `${orgSla}/sla/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckSlaNonFin = await routeSla({ orgId: orgSla, payloadId: plainPayloadSla!.id }, { db, enqueue: () => {} });
  ok("sla_analysis_agent routes on BOTH the financial and non-financial route",
    routeCheckSlaFin.ok && routeCheckSlaFin.plan.includes("sla_analysis_agent") &&
    routeCheckSlaNonFin.ok && routeCheckSlaNonFin.plan.includes("sla_analysis_agent"));
  await db.from("organizations").delete().eq("id", orgSla);

  console.log("== compensation analysis agent ==");
  ok("analyze_compensation accepts good", validateProposal("analyze_compensation", {
    total_compensation_cost: 38688000, avg_total_comp: 156000,
    compensation_by_level: [{ level: "L3 IC", headcount: 68, avg_base: 95000, avg_total_comp: 118000, salary_range_min: 78000, salary_range_max: 128000 }],
    compensation_by_department: [{ department: "Engineering", headcount: 82, avg_base: 152000, total_dept_cost: 17696000 }],
    pay_equity_flags: [{ flag_type: "Below-band employees", description: "x", affected_count: 14, severity: "high" }],
    market_benchmarking: [{ level: "L3 IC", internal_avg: 95000, market_p50: 102000, market_p75: 118000, position: "below_p50" }],
    ...
    recommendations: ["Prioritize L3 audit"],
  }).ok);
  ok("analyze_compensation rejects empty compensation_by_level array", !validateProposal("analyze_compensation", {
    total_compensation_cost: 38688000, avg_total_comp: 156000,
    compensation_by_level: [], compensation_by_department: [], pay_equity_flags: [], market_benchmarking: [],
    comp_ratio_distribution: { below_80_count: 14, ratio_80_to_100_count: 112, ratio_100_to_120_count: 52, above_120_count: 8 },
    budget_impact: { annual_base_budget: 29760000, annual_total_comp_budget: 38688000, headcount_budget_pct: 72.4 },
    recommendations: ["x"],
  }).ok);
  ok("analyze_compensation rejects budget_impact with headcount_budget_pct > 100", !validateProposal("analyze_compensation", {
    total_compensation_cost: 38688000, avg_total_comp: 156000,
    compensation_by_level: [{ level: "L3 IC", headcount: 68, avg_base: 95000, avg_total_comp: 118000, salary_range_min: 78000, salary_range_max: 128000 }],
    compensation_by_department: [], pay_equity_flags: [], market_benchmarking: [],
    comp_ratio_distribution: { below_80_count: 14, ratio_80_to_100_count: 112, ratio_100_to_120_count: 52, above_120_count: 8 },
    budget_impact: { annual_base_budget: 29760000, annual_total_comp_budget: 38688000, headcount_budget_pct: 150 },
    recommendations: ["x"],
  }).ok);
  ok("analyze_compensation filters out pay_equity_flags item with bad severity", (() => {
    const r = validateProposal("analyze_compensation", {
      total_compensation_cost: 38688000, avg_total_comp: 156000,
      compensation_by_level: [{ level: "L3 IC", headcount: 68, avg_base: 95000, avg_total_comp: 118000, salary_range_min: 78000, salary_range_max: 128000 }],
      compensation_by_department: [],
      pay_equity_flags: [
        { flag_type: "Below-band employees", description: "x", affected_count: 14, severity: "high" },
        { flag_type: "Weird flag", description: "y", affected_count: 2, severity: "catastrophic" },
      ],
      market_benchmarking: [],
      comp_ratio_distribution: { below_80_count: 14, ratio_80_to_100_count: 112, ratio_100_to_120_count: 52, above_120_count: 8 },
      budget_impact: { annual_base_budget: 29760000, annual_total_comp_budget: 38688000, headcount_budget_pct: 72.4 },
    ...
    return r.ok && (r.payload.pay_equity_flags as unknown[]).length === 1;
  })());
  ok("compensation_analysis_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("compensation_analysis_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentCmp } = await import("./lib/run-agent");
  const { stubBrain: sbCmp } = await import("./lib/agent-brain");
  const { approveAction: approveCmp, listPending: listCmp } = await import("./lib/actions-service");
  const orgCmp = await makeOrg("pro");
  const payloadCmp = await makePayload(orgCmp);
  const rCmp = await runAgentCmp({ orgId: orgCmp, payloadId: payloadCmp, role: "compensation_analysis_agent" }, { db, brain: sbCmp });
  ok("compensation_analysis_agent run produced an analysis", rCmp.ok && rCmp.proposalCount === 1);
  const pendCmp = await listCmp(orgCmp, { db });
  ok("stub proposal passes validateProposal and returns analyze_compensation", pendCmp.length === 1 && pendCmp[0].kind === "analyze_compensation");
  const apprCmp = await approveCmp(orgCmp, pendCmp[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes compensation_analysis_runs", apprCmp.ok && apprCmp.recordTable === "compensation_analysis_runs", JSON.stringify(apprCmp));
  const { data: cmpRows } = await db.from("compensation_analysis_runs").select("org_id").eq("org_id", orgCmp);
  ok("compensation analysis record org-stamped", cmpRows?.length === 1 && cmpRows[0].org_id === orgCmp);
  const { routePayload: routeCmp } = await import("./lib/manager");
  ...
  console.log("== financial statement normalizer agent ==");
  ok("normalize_financial_statements accepts good", validateProposal("normalize_financial_statements", {
    statement_type: "income_statement", accounting_standard_detected: "gaap",
    periods_detected: [{ period_label: "FY 2023", period_type: "annual", is_audited: true }],
    normalized_line_items: [{ standard_label: "Net Revenue", reported_label: "Total Net Sales", value: 4820000, period_label: "FY 2023", category: "Revenue" }],
    reclassifications: [{ from_label: "Other Income", to_label: "Non-Operating Income", amount: 42000, reason: "x" }],
    normalization_issues: [{ issue: "x", severity: "high", recommendation: "y" }],
    comparability_assessment: "Comparable after adjustments.",
  }).ok);
  ok("normalize_financial_statements rejects bad statement_type", !validateProposal("normalize_financial_statements", {
    statement_type: "profit_loss", accounting_standard_detected: "gaap",
    periods_detected: [{ period_label: "FY 2023", period_type: "annual", is_audited: true }],
    normalized_line_items: [{ standard_label: "x", reported_label: "y", value: 1, period_label: "FY 2023", category: "z" }],
    reclassifications: [], normalization_issues: [], comparability_assessment: "x",
  }).ok);
  ok("normalize_financial_statements rejects bad accounting_standard_detected", !validateProposal("normalize_financial_statements", {
    statement_type: "income_statement", accounting_standard_detected: "gasp",
    periods_detected: [{ period_label: "FY 2023", period_type: "annual", is_audited: true }],
    normalized_line_items: [{ standard_label: "x", reported_label: "y", value: 1, period_label: "FY 2023", category: "z" }],
    reclassifications: [], normalization_issues: [], comparability_assessment: "x",
  }).ok);
  ok("normalize_financial_statements rejects empty normalized_line_items array", !validateProposal("normalize_financial_statements", {
    statement_type: "income_statement", accounting_standard_detected: "gaap",
    periods_detected: [{ period_label: "FY 2023", period_type: "annual", is_audited: true }],
    normalized_line_items: [], reclassifications: [], normalization_issues: [], comparability_assessment: "x",
  }).ok);
  ok("financial_statement_normalizer_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("financial_statement_normalizer_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentFsn } = await import("./lib/run-agent");
  const { stubBrain: sbFsn } = await import("./lib/agent-brain");
  const { approveAction: approveFsn, listPending: listFsn } = await import("./lib/actions-service");
  const orgFsn = await makeOrg("pro");
  const payloadFsn = await makePayload(orgFsn);
  const rFsn = await runAgentFsn({ orgId: orgFsn, payloadId: payloadFsn, role: "financial_statement_normalizer_agent" }, { db, brain: sbFsn });
  ok("financial_statement_normalizer_agent run produced an analysis", rFsn.ok && rFsn.proposalCount === 1);
  const pendFsn = await listFsn(orgFsn, { db });
  ok("stub proposal passes validateProposal and returns normalize_financial_statements", pendFsn.length === 1 && pendFsn[0].kind === "normalize_financial_statements");
  const apprFsn = await approveFsn(orgFsn, pendFsn[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes financial_statement_normalizer_runs", apprFsn.ok && apprFsn.recordTable === "financial_statement_normalizer_runs", JSON.stringify(apprFsn));
  const { data: fsnRows } = await db.from("financial_statement_normalizer_runs").select("org_id").eq("org_id", orgFsn);
  ok("financial statement normalizer record org-stamped", fsnRows?.length === 1 && fsnRows[0].org_id === orgFsn);
  const { routePayload: routeFsn } = await import("./lib/manager");
  const routeCheckFsn = await routeFsn({ orgId: orgFsn, payloadId: payloadFsn }, { db, enqueue: () => {} });
  ok("financial_statement_normalizer_agent routes on the financial route", routeCheckFsn.ok && routeCheckFsn.plan.includes("financial_statement_normalizer_agent"));
  await db.from("organizations").delete().eq("id", orgFsn);

  console.log("== three statement model agent ==");
  ok("build_three_statement_model accepts good", validateProposal("build_three_statement_model", {
    base_period: "FY 2023",
    ...
    model_integrity_checks: [{ check_name: "x", passed: true, detail: "y" }],
    key_assumptions: [{ assumption: "Revenue", value_or_description: "x", sensitivity: "high" }],
    model_completeness: "partial", gaps_and_caveats: ["x"],
  }).ok);
  ok("build_three_statement_model rejects empty base_period", !validateProposal("build_three_statement_model", {
    base_period: "",
    income_statement: { revenue: 4820000, cost_of_revenue: 1928000, gross_profit: 2892000, operating_expenses: 2168000, ebitda: 724000, ebit: 518000, net_income: 342000 },
    balance_sheet: { total_assets: 3847000, total_liabilities: 1920000, total_equity: 1927000, check_assets_eq_liabilities_plus_equity: true },
    cash_flow_statement: { operating_cash_flow: 481000, investing_cash_flow: -320000, financing_cash_flow: -85000, net_change_in_cash: 76000, ending_cash_balance: 847000 },
    model_integrity_checks: [], key_assumptions: [{ assumption: "x", value_or_description: "y", sensitivity: "high" }], model_completeness: "partial", gaps_and_caveats: [],
  }).ok);
  ok("build_three_statement_model rejects bad model_completeness", !validateProposal("build_three_statement_model", {
    base_period: "FY 2023",
    income_statement: { revenue: 4820000, cost_of_revenue: 1928000, gross_profit: 2892000, operating_expenses: 2168000, ebitda: 724000, ebit: 518000, net_income: 342000 },
    balance_sheet: { total_assets: 3847000, total_liabilities: 1920000, total_equity: 1927000, check_assets_eq_liabilities_plus_equity: true },
    cash_flow_statement: { operating_cash_flow: 481000, investing_cash_flow: -320000, financing_cash_flow: -85000, net_change_in_cash: 76000, ending_cash_balance: 847000 },
    model_integrity_checks: [], key_assumptions: [{ assumption: "x", value_or_description: "y", sensitivity: "high" }], model_completeness: "fictional", gaps_and_caveats: [],
  }).ok);
  ok("build_three_statement_model filters out key_assumptions item with bad sensitivity", (() => {
    const r = validateProposal("build_three_statement_model", {
      base_period: "FY 2023",
      income_statement: { revenue: 4820000, cost_of_revenue: 1928000, gross_profit: 2892000, operating_expenses: 2168000, ebitda: 724000, ebit: 518000, net_income: 342000 },
      balance_sheet: { total_assets: 3847000, total_liabilities: 1920000, total_equity: 1927000, check_assets_eq_liabilities_plus_equity: true },
      cash_flow_statement: { operating_cash_flow: 481000, investing_cash_flow: -320000, financing_cash_flow: -85000, net_change_in_cash: 76000, ending_cash_balance: 847000 },
      ...
      key_assumptions: [
        { assumption: "Revenue", value_or_description: "x", sensitivity: "high" },
        { assumption: "Weird", value_or_description: "y", sensitivity: "extreme" },
      ],
      model_completeness: "partial", gaps_and_caveats: [],
    });
    return r.ok && (r.payload.key_assumptions as unknown[]).length === 1;
  })());
  ok("three_statement_model_agent → opus model",
    (await import("./lib/agent-brain")).modelForRole("three_statement_model_agent") === "claude-opus-4-8");

  const { runAgent: runAgentTsm } = await import("./lib/run-agent");
  const { stubBrain: sbTsm } = await import("./lib/agent-brain");
  const { approveAction: approveTsm, listPending: listTsm } = await import("./lib/actions-service");
  const orgTsm = await makeOrg("pro");
  const payloadTsm = await makePayload(orgTsm);
  const rTsm = await runAgentTsm({ orgId: orgTsm, payloadId: payloadTsm, role: "three_statement_model_agent" }, { db, brain: sbTsm });
  ok("three_statement_model_agent run produced an analysis", rTsm.ok && rTsm.proposalCount === 1);
  const pendTsm = await listTsm(orgTsm, { db });
  ok("stub proposal passes validateProposal and returns build_three_statement_model", pendTsm.length === 1 && pendTsm[0].kind === "build_three_statement_model");
  const apprTsm = await approveTsm(orgTsm, pendTsm[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes three_statement_model_runs", apprTsm.ok && apprTsm.recordTable === "three_statement_model_runs", JSON.stringify(apprTsm));
  const { data: tsmRows } = await db.from("three_statement_model_runs").select("org_id").eq("org_id", orgTsm);
  ok("three statement model record org-stamped", tsmRows?.length === 1 && tsmRows[0].org_id === orgTsm);
  const { routePayload: routeTsm } = await import("./lib/manager");
  const routeCheckTsm = await routeTsm({ orgId: orgTsm, payloadId: payloadTsm }, { db, enqueue: () => {} });
  ok("three_statement_model_agent routes on the financial route", routeCheckTsm.ok && routeCheckTsm.plan.includes("three_statement_model_agent"));
  await db.from("organizations").delete().eq("id", orgTsm);

  console.log("== quality of earnings agent ==");
  ok("assess_quality_of_earnings accepts good", validateProposal("assess_quality_of_earnings", {
    reported_ebitda: 724000, adjusted_ebitda: 612000,
    adjustments: [{ description: "PPP forgiveness", amount: -42000, direction: "deduct", category: "non_recurring" }],
    recurring_revenue_pct: 78.4,
    revenue_quality_flags: [{ flag: "Customer concentration", severity: "high", detail: "x" }],
    expense_quality_flags: [{ flag: "Below-market comp", severity: "high", detail: "y" }],
    one_time_items: [{ description: "PPP", amount: 42000, income_or_expense: "income" }],
    qoe_rating: "medium",
    key_risks: [{ risk: "Concentration", materiality: "high", detail: "z" }],
    due_diligence_questions: ["What are the contract terms?"],
  }).ok);
  ok("assess_quality_of_earnings rejects recurring_revenue_pct > 100", !validateProposal("assess_quality_of_earnings", {
    reported_ebitda: 724000, adjusted_ebitda: 612000, adjustments: [], recurring_revenue_pct: 150,
    revenue_quality_flags: [], expense_quality_flags: [], one_time_items: [], qoe_rating: "medium", key_risks: [], due_diligence_questions: ["x"],
  }).ok);
  ok("assess_quality_of_earnings rejects bad qoe_rating", !validateProposal("assess_quality_of_earnings", {
    reported_ebitda: 724000, adjusted_ebitda: 612000, adjustments: [], recurring_revenue_pct: 78.4,
    revenue_quality_flags: [], expense_quality_flags: [], one_time_items: [], qoe_rating: "excellent", key_risks: [], due_diligence_questions: ["x"],
  }).ok);
  ok("assess_quality_of_earnings filters out adjustments item with bad direction", (() => {
    const r = validateProposal("assess_quality_of_earnings", {
      reported_ebitda: 724000, adjusted_ebitda: 612000,
      adjustments: [
        { description: "Good", amount: -42000, direction: "deduct", category: "non_recurring" },
        { description: "Bad", amount: 10000, direction: "sideways", category: "non_recurring" },
      ],
      recurring_revenue_pct: 78.4, revenue_quality_flags: [], expense_quality_flags: [], one_time_items: [], qoe_rating: "medium", key_risks: [], due_diligence_questions: ["x"],
    });
    return r.ok && (r.payload.adjustments as unknown[]).length === 1;
  })());
  ok("quality_of_earnings_agent → opus model",
    (await import("./lib/agent-brain")).modelForRole("quality_of_earnings_agent") === "claude-opus-4-8");

  const { runAgent: runAgentQoe } = await import("./lib/run-agent");
  const { stubBrain: sbQoe } = await import("./lib/agent-brain");
  const { approveAction: approveQoe, listPending: listQoe } = await import("./lib/actions-service");
  const orgQoe = await makeOrg("pro");
  const payloadQoe = await makePayload(orgQoe);
  const rQoe = await runAgentQoe({ orgId: orgQoe, payloadId: payloadQoe, role: "quality_of_earnings_agent" }, { db, brain: sbQoe });
  ok("quality_of_earnings_agent run produced an analysis", rQoe.ok && rQoe.proposalCount === 1);
  const pendQoe = await listQoe(orgQoe, { db });
  ok("stub proposal passes validateProposal and returns assess_quality_of_earnings", pendQoe.length === 1 && pendQoe[0].kind === "assess_quality_of_earnings");
  const apprQoe = await approveQoe(orgQoe, pendQoe[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes quality_of_earnings_runs", apprQoe.ok && apprQoe.recordTable === "quality_of_earnings_runs", JSON.stringify(apprQoe));
  const { data: qoeRows } = await db.from("quality_of_earnings_runs").select("org_id").eq("org_id", orgQoe);
  ok("quality of earnings record org-stamped", qoeRows?.length === 1 && qoeRows[0].org_id === orgQoe);
  const { routePayload: routeQoe } = await import("./lib/manager");
  const routeCheckQoe = await routeQoe({ orgId: orgQoe, payloadId: payloadQoe }, { db, enqueue: () => {} });
  ok("quality_of_earnings_agent routes on the financial route", routeCheckQoe.ok && routeCheckQoe.plan.includes("quality_of_earnings_agent"));
  await db.from("organizations").delete().eq("id", orgQoe);

  console.log("== deferred revenue agent ==");
  ok("analyze_deferred_revenue accepts good", validateProposal("analyze_deferred_revenue", {
    period: "Q1 2024", opening_deferred_revenue: 1240000, new_billings: 485000, revenue_recognized: 312000, closing_deferred_revenue: 1413000,
    waterfall_schedule: [{ period_label: "Q2 2024", expected_revenue_recognition: 318000, cumulative_recognized: 318000 }],
    cohort_analysis: [{ cohort_label: "Q4 2023", original_billings: 420000, still_deferred: 285000, pct_recognized: 32.1 }],
    revenue_visibility_score: 82, recognition_pattern: "ratable",
    anomalies: [{ anomaly: "x", severity: "medium", impact: "y" }],
    recommendations: ["Verify billings"],
  }).ok);
  ok("analyze_deferred_revenue rejects empty period", !validateProposal("analyze_deferred_revenue", {
    period: "", opening_deferred_revenue: 1240000, new_billings: 485000, revenue_recognized: 312000, closing_deferred_revenue: 1413000,
    waterfall_schedule: [{ period_label: "Q2 2024", expected_revenue_recognition: 318000, cumulative_recognized: 318000 }],
    cohort_analysis: [], revenue_visibility_score: 82, recognition_pattern: "ratable", anomalies: [], recommendations: [],
  }).ok);
  ok("analyze_deferred_revenue rejects bad recognition_pattern", !validateProposal("analyze_deferred_revenue", {
    period: "Q1 2024", opening_deferred_revenue: 1240000, new_billings: 485000, revenue_recognized: 312000, closing_deferred_revenue: 1413000,
    waterfall_schedule: [{ period_label: "Q2 2024", expected_revenue_recognition: 318000, cumulative_recognized: 318000 }],
    cohort_analysis: [], revenue_visibility_score: 82, recognition_pattern: "instant", anomalies: [], recommendations: [],
  }).ok);
  ok("analyze_deferred_revenue rejects revenue_visibility_score > 100", !validateProposal("analyze_deferred_revenue", {
    period: "Q1 2024", opening_deferred_revenue: 1240000, new_billings: 485000, revenue_recognized: 312000, closing_deferred_revenue: 1413000,
    waterfall_schedule: [{ period_label: "Q2 2024", expected_revenue_recognition: 318000, cumulative_recognized: 318000 }],
    cohort_analysis: [], revenue_visibility_score: 150, recognition_pattern: "ratable", anomalies: [], recommendations: [],
  }).ok);
  ok("deferred_revenue_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("deferred_revenue_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentDrv } = await import("./lib/run-agent");
  const { stubBrain: sbDrv } = await import("./lib/agent-brain");
  const { approveAction: approveDrv, listPending: listDrv } = await import("./lib/actions-service");
  const orgDrv = await makeOrg("pro");
  const payloadDrv = await makePayload(orgDrv);
  const rDrv = await runAgentDrv({ orgId: orgDrv, payloadId: payloadDrv, role: "deferred_revenue_agent" }, { db, brain: sbDrv });
  ok("deferred_revenue_agent run produced an analysis", rDrv.ok && rDrv.proposalCount === 1);
  const pendDrv = await listDrv(orgDrv, { db });
  ok("stub proposal passes validateProposal and returns analyze_deferred_revenue", pendDrv.length === 1 && pendDrv[0].kind === "analyze_deferred_revenue");
  const apprDrv = await approveDrv(orgDrv, pendDrv[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes deferred_revenue_runs", apprDrv.ok && apprDrv.recordTable === "deferred_revenue_runs", JSON.stringify(apprDrv));
  const { data: drvRows } = await db.from("deferred_revenue_runs").select("org_id").eq("org_id", orgDrv);
  ok("deferred revenue record org-stamped", drvRows?.length === 1 && drvRows[0].org_id === orgDrv);
  const { routePayload: routeDrv } = await import("./lib/manager");
  const routeCheckDrv = await routeDrv({ orgId: orgDrv, payloadId: payloadDrv }, { db, enqueue: () => {} });
  ok("deferred_revenue_agent routes on the financial route", routeCheckDrv.ok && routeCheckDrv.plan.includes("deferred_revenue_agent"));
  await db.from("organizations").delete().eq("id", orgDrv);

  console.log("== earnings quality agent ==");
  ok("assess_earnings_quality accepts good", validateProposal("assess_earnings_quality", {
    accrual_ratio: 0.082, cash_conversion_ratio: 0.71, earnings_persistence_assessment: "persistent",
    accruals_analysis: { total_accruals: 315000, operating_accruals: 287000, financing_accruals: 28000, accrual_direction: "increasing" },
    revenue_vs_cash_reconciliation: { reported_revenue: 4820000, cash_collected: 4512000, ar_change: 308000, gap_explanation: "x" },
    red_flags: [{ flag: "Cash conversion below threshold", severity: "medium", evidence: "y" }],
    green_flags: [{ flag: "Clean reconciliation", evidence: "z" }],
    overall_quality_rating: "medium",
    forensic_indicators: [{ indicator: "DSO trend", beneish_m_score_component: "DSR Index", interpretation: "x" }],
  }).ok);
  ok("assess_earnings_quality rejects bad earnings_persistence_assessment", !validateProposal("assess_earnings_quality", {
    accrual_ratio: 0.082, cash_conversion_ratio: 0.71, earnings_persistence_assessment: "chaotic",
    accruals_analysis: { total_accruals: 315000, operating_accruals: 287000, financing_accruals: 28000, accrual_direction: "increasing" },
    revenue_vs_cash_reconciliation: { reported_revenue: 4820000, cash_collected: 4512000, ar_change: 308000, gap_explanation: "x" },
    red_flags: [], green_flags: [], overall_quality_rating: "medium", forensic_indicators: [],
  }).ok);
  ok("assess_earnings_quality rejects bad overall_quality_rating", !validateProposal("assess_earnings_quality", {
    accrual_ratio: 0.082, cash_conversion_ratio: 0.71, earnings_persistence_assessment: "persistent",
    accruals_analysis: { total_accruals: 315000, operating_accruals: 287000, financing_accruals: 28000, accrual_direction: "increasing" },
    revenue_vs_cash_reconciliation: { reported_revenue: 4820000, cash_collected: 4512000, ar_change: 308000, gap_explanation: "x" },
    red_flags: [], green_flags: [], overall_quality_rating: "superb", forensic_indicators: [],
  }).ok);
  ok("assess_earnings_quality filters out red_flags item with bad severity", (() => {
    const r = validateProposal("assess_earnings_quality", {
      accrual_ratio: 0.082, cash_conversion_ratio: 0.71, earnings_persistence_assessment: "persistent",
      accruals_analysis: { total_accruals: 315000, operating_accruals: 287000, financing_accruals: 28000, accrual_direction: "increasing" },
      revenue_vs_cash_reconciliation: { reported_revenue: 4820000, cash_collected: 4512000, ar_change: 308000, gap_explanation: "x" },
      red_flags: [
        { flag: "Good", severity: "medium", evidence: "y" },
        { flag: "Bad", severity: "extreme", evidence: "z" },
      ],
      green_flags: [], overall_quality_rating: "medium", forensic_indicators: [],
    });
    return r.ok && (r.payload.red_flags as unknown[]).length === 1;
  })());
  ok("earnings_quality_agent → opus model",
    (await import("./lib/agent-brain")).modelForRole("earnings_quality_agent") === "claude-opus-4-8");

  const { runAgent: runAgentEqy } = await import("./lib/run-agent");
  const { stubBrain: sbEqy } = await import("./lib/agent-brain");
  const { approveAction: approveEqy, listPending: listEqy } = await import("./lib/actions-service");
  const orgEqy = await makeOrg("pro");
  const payloadEqy = await makePayload(orgEqy);
  const rEqy = await runAgentEqy({ orgId: orgEqy, payloadId: payloadEqy, role: "earnings_quality_agent" }, { db, brain: sbEqy });
  ok("earnings_quality_agent run produced an analysis", rEqy.ok && rEqy.proposalCount === 1);
  const pendEqy = await listEqy(orgEqy, { db });
  ok("stub proposal passes validateProposal and returns assess_earnings_quality", pendEqy.length === 1 && pendEqy[0].kind === "assess_earnings_quality");
  const apprEqy = await approveEqy(orgEqy, pendEqy[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes earnings_quality_runs", apprEqy.ok && apprEqy.recordTable === "earnings_quality_runs", JSON.stringify(apprEqy));
  const { data: eqyRows } = await db.from("earnings_quality_runs").select("org_id").eq("org_id", orgEqy);
  ok("earnings quality record org-stamped", eqyRows?.length === 1 && eqyRows[0].org_id === orgEqy);
  const { routePayload: routeEqy } = await import("./lib/manager");
  const routeCheckEqy = await routeEqy({ orgId: orgEqy, payloadId: payloadEqy }, { db, enqueue: () => {} });
  ok("earnings_quality_agent routes on the financial route", routeCheckEqy.ok && routeCheckEqy.plan.includes("earnings_quality_agent"));
  await db.from("organizations").delete().eq("id", orgEqy);

  console.log("== intercompany elimination agent ==");
  ok("eliminate_intercompany accepts good", validateProposal("eliminate_intercompany", {
    entities_identified: [{ entity_name: "HoldCo USA LLC", entity_type: "parent", currency: "USD" }],
    intercompany_transactions: [{ from_entity: "Services Co", to_entity: "OpCo", transaction_type: "management_fee", amount: 120000, currency: "GBP", period: "FY 2023" }],
    elimination_schedule: [{ description: "Eliminate IC fee", debit_account: "Revenue", credit_account: "Expense", amount: 120000, period: "FY 2023" }],
    ...
    unreconciled_differences: [{ description: "FX timing", amount: 15000, likely_cause: "x" }],
    elimination_completeness: "substantially_complete",
    recommendations: ["Standardize FX policy"],
  }).ok);
  ok("eliminate_intercompany rejects empty entities_identified array", !validateProposal("eliminate_intercompany", {
    entities_identified: [], intercompany_transactions: [], elimination_schedule: [],
    consolidated_summary: { consolidated_revenue: 8420000, intercompany_revenue_eliminated: 148000, net_revenue: 8272000, elimination_pct: 1.76 },
    unreconciled_differences: [], elimination_completeness: "substantially_complete", recommendations: [],
  }).ok);
  ok("eliminate_intercompany rejects bad elimination_completeness", !validateProposal("eliminate_intercompany", {
    entities_identified: [{ entity_name: "HoldCo", entity_type: "parent", currency: "USD" }],
    intercompany_transactions: [], elimination_schedule: [],
    consolidated_summary: { consolidated_revenue: 8420000, intercompany_revenue_eliminated: 148000, net_revenue: 8272000, elimination_pct: 1.76 },
    unreconciled_differences: [], elimination_completeness: "flawless", recommendations: [],
  }).ok);
  ok("eliminate_intercompany filters out intercompany_transactions item with bad transaction_type", (() => {
    const r = validateProposal("eliminate_intercompany", {
      entities_identified: [{ entity_name: "HoldCo", entity_type: "parent", currency: "USD" }],
      intercompany_transactions: [
        { from_entity: "A", to_entity: "B", transaction_type: "loan", amount: 500000, currency: "USD", period: "FY 2023" },
        { from_entity: "A", to_entity: "B", transaction_type: "gift", amount: 1000, currency: "USD", period: "FY 2023" },
      ],
      elimination_schedule: [],
      consolidated_summary: { consolidated_revenue: 8420000, intercompany_revenue_eliminated: 148000, net_revenue: 8272000, elimination_pct: 1.76 },
      unreconciled_differences: [], elimination_completeness: "substantially_complete", recommendations: [],
    });
    return r.ok && (r.payload.intercompany_transactions as unknown[]).length === 1;
  })());
  ok("intercompany_elimination_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("intercompany_elimination_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentIce } = await import("./lib/run-agent");
  const { stubBrain: sbIce } = await import("./lib/agent-brain");
  const { approveAction: approveIce, listPending: listIce } = await import("./lib/actions-service");
  const orgIce = await makeOrg("pro");
  const payloadIce = await makePayload(orgIce);
  const rIce = await runAgentIce({ orgId: orgIce, payloadId: payloadIce, role: "intercompany_elimination_agent" }, { db, brain: sbIce });
  ok("intercompany_elimination_agent run produced an analysis", rIce.ok && rIce.proposalCount === 1);
  const pendIce = await listIce(orgIce, { db });
  ok("stub proposal passes validateProposal and returns eliminate_intercompany", pendIce.length === 1 && pendIce[0].kind === "eliminate_intercompany");
  const apprIce = await approveIce(orgIce, pendIce[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes intercompany_elimination_runs", apprIce.ok && apprIce.recordTable === "intercompany_elimination_runs", JSON.stringify(apprIce));
  const { data: iceRows } = await db.from("intercompany_elimination_runs").select("org_id").eq("org_id", orgIce);
  ok("intercompany elimination record org-stamped", iceRows?.length === 1 && iceRows[0].org_id === orgIce);
  const { routePayload: routeIce } = await import("./lib/manager");
  const routeCheckIce = await routeIce({ orgId: orgIce, payloadId: payloadIce }, { db, enqueue: () => {} });
  ok("intercompany_elimination_agent routes on the financial route", routeCheckIce.ok && routeCheckIce.plan.includes("intercompany_elimination_agent"));
  await db.from("organizations").delete().eq("id", orgIce);


  console.log("== stock-based compensation agent ==");
  ok("analyze_stock_based_compensation accepts good", validateProposal("analyze_stock_based_compensation", {
    period: "FY 2023", total_sbc_expense: 2840000, sbc_as_pct_of_revenue: 14.6, sbc_as_pct_of_opex: 31.2,
    sbc_by_function: [{ function_name: "Engineering", sbc_amount: 1704000, headcount: 82, sbc_per_employee: 20780 }],
    ...
    sbc_quality_flags: [{ flag: "x", severity: "medium", detail: "y" }],
    recommendations: ["Model future SBC"],
  }).ok);
  ok("analyze_stock_based_compensation rejects sbc_as_pct_of_revenue > 100", !validateProposal("analyze_stock_based_compensation", {
    period: "FY 2023", total_sbc_expense: 2840000, sbc_as_pct_of_revenue: 150, sbc_as_pct_of_opex: 31.2,
    sbc_by_function: [], unvested_overhang: { total_unvested_shares: 4820000, total_fully_diluted_shares: 28400000, overhang_pct: 16.97, weighted_avg_remaining_vest_years: 2.8 },
    dilution_analysis: { basic_shares: 18200000, fully_diluted_shares: 28400000, dilution_pct: 35.95, options_in_the_money: 6800000 },
    cash_vs_gaap_earnings_bridge: { gaap_net_income: 342000, sbc_expense_addback: 2840000, cash_net_income: 3182000, cash_earnings_per_share_impact: 0.156 },
    sbc_quality_flags: [], recommendations: [],
  }).ok);
  ok("analyze_stock_based_compensation rejects unvested_overhang with overhang_pct > 100", !validateProposal("analyze_stock_based_compensation", {
    period: "FY 2023", total_sbc_expense: 2840000, sbc_as_pct_of_revenue: 14.6, sbc_as_pct_of_opex: 31.2,
    sbc_by_function: [], unvested_overhang: { total_unvested_shares: 4820000, total_fully_diluted_shares: 28400000, overhang_pct: 150, weighted_avg_remaining_vest_years: 2.8 },
    dilution_analysis: { basic_shares: 18200000, fully_diluted_shares: 28400000, dilution_pct: 35.95, options_in_the_money: 6800000 },
    cash_vs_gaap_earnings_bridge: { gaap_net_income: 342000, sbc_expense_addback: 2840000, cash_net_income: 3182000, cash_earnings_per_share_impact: 0.156 },
    sbc_quality_flags: [], recommendations: [],
  }).ok);
  ok("analyze_stock_based_compensation filters out sbc_quality_flags item with bad severity", (() => {
    const r = validateProposal("analyze_stock_based_compensation", {
      period: "FY 2023", total_sbc_expense: 2840000, sbc_as_pct_of_revenue: 14.6, sbc_as_pct_of_opex: 31.2,
      sbc_by_function: [],
      unvested_overhang: { total_unvested_shares: 4820000, total_fully_diluted_shares: 28400000, overhang_pct: 16.97, weighted_avg_remaining_vest_years: 2.8 },
      dilution_analysis: { basic_shares: 18200000, fully_diluted_shares: 28400000, dilution_pct: 35.95, options_in_the_money: 6800000 },
      cash_vs_gaap_earnings_bridge: { gaap_net_income: 342000, sbc_expense_addback: 2840000, cash_net_income: 3182000, cash_earnings_per_share_impact: 0.156 },
      sbc_quality_flags: [
        { flag: "Good", severity: "medium", detail: "x" },
        { flag: "Bad", severity: "extreme", detail: "y" },
      ],
      recommendations: [],
    });
    return r.ok && (r.payload.sbc_quality_flags as unknown[]).length === 1;
  })());
  ok("stock_based_compensation_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("stock_based_compensation_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentSbc } = await import("./lib/run-agent");
  const { stubBrain: sbSbc } = await import("./lib/agent-brain");
  const { approveAction: approveSbc, listPending: listSbc } = await import("./lib/actions-service");
  const orgSbc = await makeOrg("pro");
  const payloadSbc = await makePayload(orgSbc);
  const rSbc = await runAgentSbc({ orgId: orgSbc, payloadId: payloadSbc, role: "stock_based_compensation_agent" }, { db, brain: sbSbc });
  ok("stock_based_compensation_agent run produced an analysis", rSbc.ok && rSbc.proposalCount === 1);
  const pendSbc = await listSbc(orgSbc, { db });
  ok("stub proposal passes validateProposal and returns analyze_stock_based_compensation", pendSbc.length === 1 && pendSbc[0].kind === "analyze_stock_based_compensation");
  const apprSbc = await approveSbc(orgSbc, pendSbc[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes stock_based_compensation_runs", apprSbc.ok && apprSbc.recordTable === "stock_based_compensation_runs", JSON.stringify(apprSbc));
  const { data: sbcRows } = await db.from("stock_based_compensation_runs").select("org_id").eq("org_id", orgSbc);
  ok("stock-based compensation record org-stamped", sbcRows?.length === 1 && sbcRows[0].org_id === orgSbc);
  const { routePayload: routeSbc } = await import("./lib/manager");
  const routeCheckSbc = await routeSbc({ orgId: orgSbc, payloadId: payloadSbc }, { db, enqueue: () => {} });
  ok("stock_based_compensation_agent routes on the financial route", routeCheckSbc.ok && routeCheckSbc.plan.includes("stock_based_compensation_agent"));
  await db.from("organizations").delete().eq("id", orgSbc);

  console.log("== gaap compliance agent ==");
  ok("assess_gaap_compliance accepts good", validateProposal("assess_gaap_compliance", {
    standards_applicable: [{ standard_code: "ASC 606", standard_name: "Revenue", relevance_reason: "x" }],
    compliance_assessments: [{ area: "Revenue Recognition", standard_code: "ASC 606", compliance_status: "compliant", finding: "x" }],
    critical_findings: [{ finding: "x", standard_violated: "ASC 842", financial_impact: "y", urgency: "immediate" }],
    disclosure_gaps: [{ disclosure_type: "x", required_by: "y", gap_description: "z" }],
    accounting_policy_notes: [{ policy_area: "x", current_policy: "y", gaap_alignment: "aligned" }],
    overall_compliance_rating: "needs_improvement",
    remediation_plan: [{ action: "x", priority: "p1", timeline: "y", owner: "z" }],
  }).ok);
  ok("assess_gaap_compliance rejects empty compliance_assessments array", !validateProposal("assess_gaap_compliance", {
    standards_applicable: [{ standard_code: "ASC 606", standard_name: "Revenue", relevance_reason: "x" }],
    compliance_assessments: [], critical_findings: [], disclosure_gaps: [], accounting_policy_notes: [],
    overall_compliance_rating: "needs_improvement", remediation_plan: [],
  }).ok);
  ok("assess_gaap_compliance rejects bad overall_compliance_rating", !validateProposal("assess_gaap_compliance", {
    standards_applicable: [{ standard_code: "ASC 606", standard_name: "Revenue", relevance_reason: "x" }],
    compliance_assessments: [{ area: "x", standard_code: "y", compliance_status: "compliant", finding: "z" }],
    critical_findings: [], disclosure_gaps: [], accounting_policy_notes: [],
    overall_compliance_rating: "perfect", remediation_plan: [],
  }).ok);
  ok("assess_gaap_compliance filters out compliance_assessments item with bad compliance_status", (() => {
    const r = validateProposal("assess_gaap_compliance", {
      standards_applicable: [{ standard_code: "ASC 606", standard_name: "Revenue", relevance_reason: "x" }],
      compliance_assessments: [
        { area: "Good", standard_code: "ASC 606", compliance_status: "compliant", finding: "x" },
        { area: "Bad", standard_code: "ASC 606", compliance_status: "mostly_fine", finding: "y" },
      ],
      critical_findings: [], disclosure_gaps: [], accounting_policy_notes: [],
      overall_compliance_rating: "needs_improvement", remediation_plan: [],
    });
    return r.ok && (r.payload.compliance_assessments as unknown[]).length === 1;
  })());
  ok("gaap_compliance_agent → opus model",
    (await import("./lib/agent-brain")).modelForRole("gaap_compliance_agent") === "claude-opus-4-8");

  const { runAgent: runAgentGcp } = await import("./lib/run-agent");
  const { stubBrain: sbGcp } = await import("./lib/agent-brain");
  const { approveAction: approveGcp, listPending: listGcp } = await import("./lib/actions-service");
  const orgGcp = await makeOrg("pro");
  const payloadGcp = await makePayload(orgGcp);
  const rGcp = await runAgentGcp({ orgId: orgGcp, payloadId: payloadGcp, role: "gaap_compliance_agent" }, { db, brain: sbGcp });
  ok("gaap_compliance_agent run produced an analysis", rGcp.ok && rGcp.proposalCount === 1);
  const pendGcp = await listGcp(orgGcp, { db });
  ok("stub proposal passes validateProposal and returns assess_gaap_compliance", pendGcp.length === 1 && pendGcp[0].kind === "assess_gaap_compliance");
  const apprGcp = await approveGcp(orgGcp, pendGcp[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes gaap_compliance_runs", apprGcp.ok && apprGcp.recordTable === "gaap_compliance_runs", JSON.stringify(apprGcp));
  const { data: gcpRows } = await db.from("gaap_compliance_runs").select("org_id").eq("org_id", orgGcp);
  ok("gaap compliance record org-stamped", gcpRows?.length === 1 && gcpRows[0].org_id === orgGcp);
  const { routePayload: routeGcp } = await import("./lib/manager");
  const routeCheckGcp = await routeGcp({ orgId: orgGcp, payloadId: payloadGcp }, { db, enqueue: () => {} });
  ok("gaap_compliance_agent routes on the financial route", routeCheckGcp.ok && routeCheckGcp.plan.includes("gaap_compliance_agent"));
  await db.from("organizations").delete().eq("id", orgGcp);

  console.log("== tax rate reconciliation agent ==");
  ok("reconcile_tax_rate accepts good", validateProposal("reconcile_tax_rate", {
    period: "FY 2023", statutory_rate_pct: 21, effective_tax_rate_pct: 14.8, rate_difference_pct: -6.2,
    pre_tax_income: 518000, tax_provision: 76700,
    reconciling_items: [{ item: "Federal Rate", effect_on_rate_ppts: 21.0, explanation: "x" }],
    deferred_tax_analysis: { deferred_tax_asset: 284000, deferred_tax_liability: 47000, net_deferred_tax: 237000, valuation_allowance: 0, realizability_assessment: "x" },
    tax_risk_flags: [{ flag: "x", severity: "medium", potential_exposure: "y" }],
    recommendations: ["Formalize documentation"],
  }).ok);
  ok("reconcile_tax_rate rejects statutory_rate_pct > 100", !validateProposal("reconcile_tax_rate", {
    period: "FY 2023", statutory_rate_pct: 150, effective_tax_rate_pct: 14.8, rate_difference_pct: -6.2,
    pre_tax_income: 518000, tax_provision: 76700, reconciling_items: [],
    deferred_tax_analysis: { deferred_tax_asset: 284000, deferred_tax_liability: 47000, net_deferred_tax: 237000, valuation_allowance: 0, realizability_assessment: "x" },
    tax_risk_flags: [], recommendations: [],
  }).ok);
  ok("reconcile_tax_rate rejects empty period", !validateProposal("reconcile_tax_rate", {
    period: "", statutory_rate_pct: 21, effective_tax_rate_pct: 14.8, rate_difference_pct: -6.2,
    pre_tax_income: 518000, tax_provision: 76700, reconciling_items: [],
    deferred_tax_analysis: { deferred_tax_asset: 284000, deferred_tax_liability: 47000, net_deferred_tax: 237000, valuation_allowance: 0, realizability_assessment: "x" },
    tax_risk_flags: [], recommendations: [],
  }).ok);
  ok("reconcile_tax_rate filters out tax_risk_flags item with bad severity", (() => {
    const r = validateProposal("reconcile_tax_rate", {
      period: "FY 2023", statutory_rate_pct: 21, effective_tax_rate_pct: 14.8, rate_difference_pct: -6.2,
      pre_tax_income: 518000, tax_provision: 76700, reconciling_items: [],
      deferred_tax_analysis: { deferred_tax_asset: 284000, deferred_tax_liability: 47000, net_deferred_tax: 237000, valuation_allowance: 0, realizability_assessment: "x" },
      tax_risk_flags: [
        { flag: "Good", severity: "medium", potential_exposure: "x" },
        { flag: "Bad", severity: "extreme", potential_exposure: "y" },
      ],
      recommendations: [],
    });
    return r.ok && (r.payload.tax_risk_flags as unknown[]).length === 1;
  })());
  ok("tax_rate_reconciliation_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("tax_rate_reconciliation_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentTrr } = await import("./lib/run-agent");
  const { stubBrain: sbTrr } = await import("./lib/agent-brain");
  const { approveAction: approveTrr, listPending: listTrr } = await import("./lib/actions-service");
  const orgTrr = await makeOrg("pro");
  const payloadTrr = await makePayload(orgTrr);
  const rTrr = await runAgentTrr({ orgId: orgTrr, payloadId: payloadTrr, role: "tax_rate_reconciliation_agent" }, { db, brain: sbTrr });
  ok("tax_rate_reconciliation_agent run produced an analysis", rTrr.ok && rTrr.proposalCount === 1);
  const pendTrr = await listTrr(orgTrr, { db });
  ok("stub proposal passes validateProposal and returns reconcile_tax_rate", pendTrr.length === 1 && pendTrr[0].kind === "reconcile_tax_rate");
  const apprTrr = await approveTrr(orgTrr, pendTrr[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes tax_rate_reconciliation_runs", apprTrr.ok && apprTrr.recordTable === "tax_rate_reconciliation_runs", JSON.stringify(apprTrr));
  const { data: trrRows } = await db.from("tax_rate_reconciliation_runs").select("org_id").eq("org_id", orgTrr);
  ok("tax rate reconciliation record org-stamped", trrRows?.length === 1 && trrRows[0].org_id === orgTrr);
  const { routePayload: routeTrr } = await import("./lib/manager");
  const routeCheckTrr = await routeTrr({ orgId: orgTrr, payloadId: payloadTrr }, { db, enqueue: () => {} });
  ok("tax_rate_reconciliation_agent routes on the financial route", routeCheckTrr.ok && routeCheckTrr.plan.includes("tax_rate_reconciliation_agent"));
  await db.from("organizations").delete().eq("id", orgTrr);

  console.log("== due diligence checklist agent ==");
  ok("build_due_diligence_checklist accepts good", validateProposal("build_due_diligence_checklist", {
    transaction_type: "fundraising_series_a",
    checklist_categories: [{ category: "Financial", item_count: 18, completion_status: "partial" }],
    total_items: 52,
    critical_items: [{ item: "Audited financials", category: "Financial", status: "open", priority: "p1" }],
    items_evidenced_in_data: [{ item: "ARR trend", evidence_found: "x" }],
    open_items: [{ item: "Cap table", category: "Legal", reason_open: "x" }],
    risk_summary: "Series A process with legal gaps.",
  }).ok);
  ok("build_due_diligence_checklist rejects bad transaction_type", !validateProposal("build_due_diligence_checklist", {
    transaction_type: "yard_sale",
    checklist_categories: [{ category: "Financial", item_count: 18, completion_status: "partial" }],
    total_items: 52, critical_items: [], items_evidenced_in_data: [], open_items: [], risk_summary: "x",
  }).ok);
  ok("build_due_diligence_checklist rejects empty checklist_categories array", !validateProposal("build_due_diligence_checklist", {
    transaction_type: "fundraising_series_a", checklist_categories: [], total_items: 52,
    critical_items: [], items_evidenced_in_data: [], open_items: [], risk_summary: "x",
  }).ok);
  ok("build_due_diligence_checklist filters out critical_items item with bad status", (() => {
    const r = validateProposal("build_due_diligence_checklist", {
      transaction_type: "fundraising_series_a",
      checklist_categories: [{ category: "Financial", item_count: 18, completion_status: "partial" }],
      total_items: 52,
      critical_items: [
        { item: "Good", category: "Financial", status: "open", priority: "p1" },
        { item: "Bad", category: "Legal", status: "maybe", priority: "p1" },
      ],
      items_evidenced_in_data: [], open_items: [], risk_summary: "x",
    });
    return r.ok && (r.payload.critical_items as unknown[]).length === 1;
  })());
  ok("due_diligence_checklist_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("due_diligence_checklist_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentDdc } = await import("./lib/run-agent");
  const { stubBrain: sbDdc } = await import("./lib/agent-brain");
  const { approveAction: approveDdc, listPending: listDdc } = await import("./lib/actions-service");
  const orgDdc = await makeOrg("pro");
  const payloadDdc = await makePayload(orgDdc);
  const rDdc = await runAgentDdc({ orgId: orgDdc, payloadId: payloadDdc, role: "due_diligence_checklist_agent" }, { db, brain: sbDdc });
  ok("due_diligence_checklist_agent run produced an analysis", rDdc.ok && rDdc.proposalCount === 1);
  const pendDdc = await listDdc(orgDdc, { db });
  ok("stub proposal passes validateProposal and returns build_due_diligence_checklist", pendDdc.length === 1 && pendDdc[0].kind === "build_due_diligence_checklist");
  const apprDdc = await approveDdc(orgDdc, pendDdc[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes due_diligence_checklist_runs", apprDdc.ok && apprDdc.recordTable === "due_diligence_checklist_runs", JSON.stringify(apprDdc));
  const { data: ddcRows } = await db.from("due_diligence_checklist_runs").select("org_id").eq("org_id", orgDdc);
  ok("due diligence checklist record org-stamped", ddcRows?.length === 1 && ddcRows[0].org_id === orgDdc);
  const { routePayload: routeDdc } = await import("./lib/manager");
  const routeCheckDdcFin = await routeDdc({ orgId: orgDdc, payloadId: payloadDdc }, { db, enqueue: () => {} });
  const { data: plainPayloadDdc } = await db.from("inbound_payloads").insert({
    org_id: orgDdc, source: "upload", storage_path: `${orgDdc}/ddc/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckDdcNonFin = await routeDdc({ orgId: orgDdc, payloadId: plainPayloadDdc!.id }, { db, enqueue: () => {} });
  ok("due_diligence_checklist_agent routes on BOTH the financial and non-financial route",
    routeCheckDdcFin.ok && routeCheckDdcFin.plan.includes("due_diligence_checklist_agent") &&
    routeCheckDdcNonFin.ok && routeCheckDdcNonFin.plan.includes("due_diligence_checklist_agent"));
  await db.from("organizations").delete().eq("id", orgDdc);

  console.log("== market sizing agent ==");
  ok("analyze_market_sizing accepts good", validateProposal("analyze_market_sizing", {
    market_definition: "AI financial software for SMB", tam_estimate: 12400000000, sam_estimate: 1860000000, som_estimate: 93000000,
    sizing_methodology: "bottom_up", current_market_share_pct: 1.09,
    addressable_customers: { total_addressable_count: 620000, segments: [{ name: "US SMB", count: 420000 }] },
    market_growth_rate_pct: 23.4, competitive_density: "fragmented",
    market_opportunity_summary: "Large fragmented market.",
    assumptions_and_caveats: ["TAM based on ARPU"],
  }).ok);
  ok("analyze_market_sizing rejects empty market_definition", !validateProposal("analyze_market_sizing", {
    market_definition: "", tam_estimate: 12400000000, sam_estimate: 1860000000, som_estimate: 93000000,
    sizing_methodology: "bottom_up", current_market_share_pct: 1.09,
    addressable_customers: { total_addressable_count: 620000, segments: [] },
    market_growth_rate_pct: 23.4, competitive_density: "fragmented", market_opportunity_summary: "x", assumptions_and_caveats: ["x"],
  }).ok);
  ok("analyze_market_sizing rejects bad sizing_methodology", !validateProposal("analyze_market_sizing", {
    market_definition: "x", tam_estimate: 12400000000, sam_estimate: 1860000000, som_estimate: 93000000,
    sizing_methodology: "guesswork", current_market_share_pct: 1.09,
    addressable_customers: { total_addressable_count: 620000, segments: [] },
    market_growth_rate_pct: 23.4, competitive_density: "fragmented", market_opportunity_summary: "x", assumptions_and_caveats: ["x"],
  }).ok);
  ok("analyze_market_sizing rejects current_market_share_pct > 100", !validateProposal("analyze_market_sizing", {
    market_definition: "x", tam_estimate: 12400000000, sam_estimate: 1860000000, som_estimate: 93000000,
    sizing_methodology: "bottom_up", current_market_share_pct: 150,
    addressable_customers: { total_addressable_count: 620000, segments: [] },
    market_growth_rate_pct: 23.4, competitive_density: "fragmented", market_opportunity_summary: "x", assumptions_and_caveats: ["x"],
  }).ok);
  ok("market_sizing_agent → sonnet model",
    (await import("./lib/agent-brain")).modelForRole("market_sizing_agent") === "claude-sonnet-4-6");

  const { runAgent: runAgentMkt } = await import("./lib/run-agent");
  const { stubBrain: sbMkt } = await import("./lib/agent-brain");
  const { approveAction: approveMkt, listPending: listMkt } = await import("./lib/actions-service");
  const orgMkt = await makeOrg("pro");
  const payloadMkt = await makePayload(orgMkt);
  const rMkt = await runAgentMkt({ orgId: orgMkt, payloadId: payloadMkt, role: "market_sizing_agent" }, { db, brain: sbMkt });
  ok("market_sizing_agent run produced an analysis", rMkt.ok && rMkt.proposalCount === 1);
  const pendMkt = await listMkt(orgMkt, { db });
  ok("stub proposal passes validateProposal and returns analyze_market_sizing", pendMkt.length === 1 && pendMkt[0].kind === "analyze_market_sizing");
  const apprMkt = await approveMkt(orgMkt, pendMkt[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes market_sizing_runs", apprMkt.ok && apprMkt.recordTable === "market_sizing_runs", JSON.stringify(apprMkt));
  const { data: mktRows } = await db.from("market_sizing_runs").select("org_id").eq("org_id", orgMkt);
  ok("market sizing record org-stamped", mktRows?.length === 1 && mktRows[0].org_id === orgMkt);
  const { routePayload: routeMkt } = await import("./lib/manager");
  const routeCheckMktFin = await routeMkt({ orgId: orgMkt, payloadId: payloadMkt }, { db, enqueue: () => {} });
  const { data: plainPayloadMkt } = await db.from("inbound_payloads").insert({
    org_id: orgMkt, source: "upload", storage_path: `${orgMkt}/mkt/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckMktNonFin = await routeMkt({ orgId: orgMkt, payloadId: plainPayloadMkt!.id }, { db, enqueue: () => {} });
  ok("market_sizing_agent routes on BOTH the financial and non-financial route",
    routeCheckMktFin.ok && routeCheckMktFin.plan.includes("market_sizing_agent") &&
    routeCheckMktNonFin.ok && routeCheckMktNonFin.plan.includes("market_sizing_agent"));
  await db.from("organizations").delete().eq("id", orgMkt);

  console.log("== invoice extraction agent ==");
  ok("extract_invoice_data accepts good", validateProposal("extract_invoice_data", {
    invoices_extracted: [{ invoice_number: "INV-1", vendor_or_customer: "Acme", invoice_date: "2024-03-01", due_date: "2024-03-31", amount: 12400, currency: "USD", line_items: [], status: "extracted" }],
    extraction_summary: { total_invoices: 87, fully_extracted: 71, partially_extracted: 12, unclear: 4, total_amount: 485200 },
    field_completeness: [{ field_name: "invoice_number", completeness_pct: 100 }],
    validation_issues: [{ invoice_reference: "INV-2", issue_type: "missing_due_date", description: "x", severity: "medium" }],
    duplicate_flags: [{ invoice_reference_a: "INV-1", invoice_reference_b: "INV-3", similarity_reason: "x" }],
    ready_for_posting: false,
  }).ok);
  ok("extract_invoice_data rejects empty invoices_extracted array", !validateProposal("extract_invoice_data", {
    invoices_extracted: [],
    extraction_summary: { total_invoices: 0, fully_extracted: 0, partially_extracted: 0, unclear: 0, total_amount: 0 },
    field_completeness: [], validation_issues: [], duplicate_flags: [], ready_for_posting: false,
  }).ok);
  ok("extract_invoice_data filters out invoices_extracted item with bad status", (() => {
    const r = validateProposal("extract_invoice_data", {
      invoices_extracted: [
        { invoice_number: "INV-1", vendor_or_customer: "Acme", invoice_date: "2024-03-01", due_date: "2024-03-31", amount: 12400, currency: "USD", line_items: [], status: "extracted" },
        { invoice_number: "INV-2", vendor_or_customer: "Beta", invoice_date: "2024-03-02", due_date: null, amount: 500, currency: "USD", line_items: [], status: "mystery" },
      ],
      extraction_summary: { total_invoices: 2, fully_extracted: 1, partially_extracted: 0, unclear: 1, total_amount: 12900 },
      field_completeness: [], validation_issues: [], duplicate_flags: [], ready_for_posting: false,
    });
    return r.ok && (r.payload.invoices_extracted as unknown[]).length === 1;
  })());
  ok("extract_invoice_data rejects invalid ready_for_posting type", !validateProposal("extract_invoice_data", {
    invoices_extracted: [{ invoice_number: "INV-1", vendor_or_customer: "Acme", invoice_date: "2024-03-01", due_date: "2024-03-31", amount: 12400, currency: "USD", line_items: [], status: "extracted" }],
    extraction_summary: { total_invoices: 1, fully_extracted: 1, partially_extracted: 0, unclear: 0, total_amount: 12400 },
    field_completeness: [], validation_issues: [], duplicate_flags: [], ready_for_posting: "yes" as unknown as boolean,
  }).ok);
  ok("invoice_extraction_agent → haiku model",
    (await import("./lib/agent-brain")).modelForRole("invoice_extraction_agent") === "claude-haiku-4-5-20251001");

  const { runAgent: runAgentIvx } = await import("./lib/run-agent");
  const { stubBrain: sbIvx } = await import("./lib/agent-brain");
  const { approveAction: approveIvx, listPending: listIvx } = await import("./lib/actions-service");
  const orgIvx = await makeOrg("pro");
  const payloadIvx = await makePayload(orgIvx);
  const rIvx = await runAgentIvx({ orgId: orgIvx, payloadId: payloadIvx, role: "invoice_extraction_agent" }, { db, brain: sbIvx });
  ok("invoice_extraction_agent run produced an analysis", rIvx.ok && rIvx.proposalCount === 1);
  const pendIvx = await listIvx(orgIvx, { db });
  ok("stub proposal passes validateProposal and returns extract_invoice_data", pendIvx.length === 1 && pendIvx[0].kind === "extract_invoice_data");
  const apprIvx = await approveIvx(orgIvx, pendIvx[0].id, "00000000-0000-0000-0000-000000000000", { db });
  ok("approve writes invoice_extraction_runs", apprIvx.ok && apprIvx.recordTable === "invoice_extraction_runs", JSON.stringify(apprIvx));
  const { data: ivxRows } = await db.from("invoice_extraction_runs").select("org_id").eq("org_id", orgIvx);
  ok("invoice extraction record org-stamped", ivxRows?.length === 1 && ivxRows[0].org_id === orgIvx);
  const { routePayload: routeIvx } = await import("./lib/manager");
  const routeCheckIvxFin = await routeIvx({ orgId: orgIvx, payloadId: payloadIvx }, { db, enqueue: () => {} });
  const { data: plainPayloadIvx } = await db.from("inbound_payloads").insert({
    org_id: orgIvx, source: "upload", storage_path: `${orgIvx}/ivx/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckIvxNonFin = await routeIvx({ orgId: orgIvx, payloadId: plainPayloadIvx!.id }, { db, enqueue: () => {} });
  ok("invoice_extraction_agent routes on BOTH the financial and non-financial route",
    routeCheckIvxFin.ok && routeCheckIvxFin.plan.includes("invoice_extraction_agent") &&
    routeCheckIvxNonFin.ok && routeCheckIvxNonFin.plan.includes("invoice_extraction_agent"));
  await db.from("organizations").delete().eq("id", orgIvx);

  console.log("== bigquery query agent ==");
  ok("query_bigquery accepts good", validateProposal("query_bigquery", {
    natural_language_question: "Show me total revenue by customer for the last 90 days",
    generated_sql: "SELECT customer_name, SUM(invoice_amount) AS total_revenue FROM `{project_id}.finance_data.invoices` GROUP BY customer_name",
    query_rationale: "Used the invoices table to aggregate revenue per customer over the last 90 days.",
    rows_returned: 0, execution_time_ms: 0, result_preview: [], result_schema: [],
    query_status: "pending", error_message: null,
    follow_up_suggestions: ["Break this down by month"],
  }).ok);
  ok("query_bigquery rejects invalid query_status", !validateProposal("query_bigquery", {
    natural_language_question: "q", generated_sql: "SELECT 1", query_rationale: "r",
    rows_returned: 0, execution_time_ms: 0, result_preview: [], result_schema: [],
    query_status: "mystery", error_message: null, follow_up_suggestions: [],
  }).ok);
  ok("query_bigquery rejects empty generated_sql", !validateProposal("query_bigquery", {
    natural_language_question: "q", generated_sql: "", query_rationale: "r",
    rows_returned: 0, execution_time_ms: 0, result_preview: [], result_schema: [],
    query_status: "pending", error_message: null, follow_up_suggestions: [],
  }).ok);
  ok("query_bigquery filters out result_schema item with empty name", (() => {
    const r = validateProposal("query_bigquery", {
      natural_language_question: "q", generated_sql: "SELECT 1", query_rationale: "r",
      rows_returned: 1, execution_time_ms: 10,
      result_preview: [{ customer_name: "Acme", total_revenue: 1000 }],
      result_schema: [{ name: "customer_name", type: "STRING", mode: "NULLABLE" }, { name: "", type: "INT64", mode: "NULLABLE" }],
      query_status: "success", error_message: null, follow_up_suggestions: [],
    });
    return r.ok && (r.payload.result_schema as unknown[]).length === 1;
  })());

  const { stubBrain: sbBgq } = await import("./lib/agent-brain");
  const bgqStub = await sbBgq.propose({ role: "bigquery_query_agent", columns: ["x"], sampleRows: [["y"]], rowCount: 1 });
  ok("stubBrain returns query_bigquery", bgqStub.proposals[0]?.kind === "query_bigquery");
  const stubValidateBgq = validateProposal("query_bigquery", bgqStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateBgq.ok && stubValidateBgq.kind === "query_bigquery");

  const { runAgent: runAgentBgq } = await import("./lib/run-agent");
  const orgBgq = await makeOrg("pro");
  const payloadBgq = await makePayload(orgBgq);
  const rBgq = await runAgentBgq({ orgId: orgBgq, payloadId: payloadBgq, role: "bigquery_query_agent" }, { db, brain: sbBgq });
  const { data: pendBgq } = await db.from("proposed_actions").select("kind").eq("org_id", orgBgq);
  ok("runAgent succeeds and returns correct action kind",
    rBgq.ok && rBgq.proposalCount === 1 && pendBgq?.length === 1 && pendBgq[0].kind === "query_bigquery");

  const { routePayload: routeBgq } = await import("./lib/manager");
  const routeCheckBgqFin = await routeBgq({ orgId: orgBgq, payloadId: payloadBgq }, { db, enqueue: () => {} });
  const { data: plainPayloadBgq } = await db.from("inbound_payloads").insert({
    org_id: orgBgq, source: "upload", storage_path: `${orgBgq}/bgq/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckBgqNonFin = await routeBgq({ orgId: orgBgq, payloadId: plainPayloadBgq!.id }, { db, enqueue: () => {} });
  ok("bigquery_query_agent routes on BOTH the financial and non-financial route",
    routeCheckBgqFin.ok && routeCheckBgqFin.plan.includes("bigquery_query_agent") &&
    routeCheckBgqNonFin.ok && routeCheckBgqNonFin.plan.includes("bigquery_query_agent"));
  await db.from("organizations").delete().eq("id", orgBgq);

  console.log("== cash positioning agent ==");
  ok("position_cash accepts good", validateProposal("position_cash", {
    as_of_date: "2024-01-31", total_cash_balance: 285000,
    cash_by_account: [{ account_name: "Operating", balance: 200000, account_type: "checking" }],
    inflows_next_7d: 45000, outflows_next_7d: 62000, inflows_next_30d: 180000, outflows_next_30d: 195000,
    projected_balance_7d: 268000, projected_balance_30d: 270000,
    minimum_operating_cash: 120000, cash_cushion_days: 43.8,
    liquidity_status: "adequate", alerts: ["x"],
  }).ok);
  ok("position_cash rejects invalid liquidity_status", !validateProposal("position_cash", {
    as_of_date: "2024-01-31", total_cash_balance: 100, cash_by_account: [],
    inflows_next_7d: 0, outflows_next_7d: 0, inflows_next_30d: 0, outflows_next_30d: 0,
    projected_balance_7d: 100, projected_balance_30d: 100,
    minimum_operating_cash: null, cash_cushion_days: null,
    liquidity_status: "mystery", alerts: [],
  }).ok);
  ok("position_cash rejects empty as_of_date", !validateProposal("position_cash", {
    as_of_date: "", total_cash_balance: 100, cash_by_account: [],
    inflows_next_7d: 0, outflows_next_7d: 0, inflows_next_30d: 0, outflows_next_30d: 0,
    projected_balance_7d: 100, projected_balance_30d: 100,
    minimum_operating_cash: null, cash_cushion_days: null,
    liquidity_status: "adequate", alerts: [],
  }).ok);
  ok("position_cash filters out cash_by_account item with empty account_name", (() => {
    const r = validateProposal("position_cash", {
      as_of_date: "2024-01-31", total_cash_balance: 100,
      cash_by_account: [
        { account_name: "Operating", balance: 100, account_type: "checking" },
        { account_name: "", balance: 50, account_type: "savings" },
      ],
      inflows_next_7d: 0, outflows_next_7d: 0, inflows_next_30d: 0, outflows_next_30d: 0,
      projected_balance_7d: 100, projected_balance_30d: 100,
      minimum_operating_cash: null, cash_cushion_days: null,
    ...
    return r.ok && (r.payload.cash_by_account as unknown[]).length === 1;
  })());

  const { stubBrain: sbCpg } = await import("./lib/agent-brain");
  const cpgStub = await sbCpg.propose({ role: "cash_positioning_agent", columns: ["amount"], sampleRows: [["10"]], rowCount: 1 });
  ok("stubBrain returns position_cash", cpgStub.proposals[0]?.kind === "position_cash");
  const stubValidateCpg = validateProposal("position_cash", cpgStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateCpg.ok && stubValidateCpg.kind === "position_cash");

  const { runAgent: runAgentCpg } = await import("./lib/run-agent");
  const orgCpg = await makeOrg("pro");
  const payloadCpg = await makePayload(orgCpg);
  const rCpg = await runAgentCpg({ orgId: orgCpg, payloadId: payloadCpg, role: "cash_positioning_agent" }, { db, brain: sbCpg });
  const { data: pendCpg } = await db.from("proposed_actions").select("kind").eq("org_id", orgCpg);
  ok("runAgent succeeds and returns correct action kind",
    rCpg.ok && rCpg.proposalCount === 1 && pendCpg?.length === 1 && pendCpg[0].kind === "position_cash");

  const { routePayload: routeCpg } = await import("./lib/manager");
  const routeCheckCpg = await routeCpg({ orgId: orgCpg, payloadId: payloadCpg }, { db, enqueue: () => {} });
  ok("cash_positioning_agent routes on the financial route", routeCheckCpg.ok && routeCheckCpg.plan.includes("cash_positioning_agent"));
  await db.from("organizations").delete().eq("id", orgCpg);

  console.log("== debt schedule agent ==");
  ok("build_debt_schedule accepts good", validateProposal("build_debt_schedule", {
    as_of_date: "2024-01-31", total_debt: 3500000,
    debt_facilities: [{ facility_name: "Term Loan A", outstanding_balance: 2000000, interest_rate: 7.5, facility_type: "term_loan", maturity_date: "2026-06-30" }],
    total_annual_interest: 266750, total_annual_principal: 400000,
    weighted_avg_interest_rate: 7.62, debt_service_coverage_ratio: 1.8, interest_coverage_ratio: 3.2,
    maturity_schedule: [{ period: "2024", principal_due: 400000, interest_due: 266750 }],
    covenant_headroom: [{ covenant_name: "Max Leverage", current_value: 2.8, covenant_limit: 3.5, headroom_pct: 20.0 }],
    refinancing_risk: "medium", summary_notes: "x",
  }).ok);
  ok("build_debt_schedule rejects invalid refinancing_risk", !validateProposal("build_debt_schedule", {
    as_of_date: "2024-01-31", total_debt: 100, debt_facilities: [],
    total_annual_interest: 0, total_annual_principal: 0,
    weighted_avg_interest_rate: null, debt_service_coverage_ratio: null, interest_coverage_ratio: null,
    maturity_schedule: [], covenant_headroom: [],
    refinancing_risk: "mystery", summary_notes: "x",
  }).ok);
  ok("build_debt_schedule rejects empty as_of_date", !validateProposal("build_debt_schedule", {
    as_of_date: "", total_debt: 100, debt_facilities: [],
    total_annual_interest: 0, total_annual_principal: 0,
    weighted_avg_interest_rate: null, debt_service_coverage_ratio: null, interest_coverage_ratio: null,
    maturity_schedule: [], covenant_headroom: [],
    refinancing_risk: "low", summary_notes: "x",
  }).ok);
  ok("build_debt_schedule filters out debt_facilities item with empty facility_name", (() => {
    const r = validateProposal("build_debt_schedule", {
      as_of_date: "2024-01-31", total_debt: 100,
      debt_facilities: [
        { facility_name: "Term Loan A", outstanding_balance: 100, interest_rate: 5, facility_type: "term_loan", maturity_date: "2026-01-01" },
        { facility_name: "", outstanding_balance: 50, interest_rate: 5, facility_type: "revolver", maturity_date: "2025-01-01" },
      ],
      total_annual_interest: 0, total_annual_principal: 0,
      weighted_avg_interest_rate: null, debt_service_coverage_ratio: null, interest_coverage_ratio: null,
      maturity_schedule: [], covenant_headroom: [],
    ...
    return r.ok && (r.payload.debt_facilities as unknown[]).length === 1;
  })());

  const { stubBrain: sbDbs } = await import("./lib/agent-brain");
  const dbsStub = await sbDbs.propose({ role: "debt_schedule_agent", columns: ["amount"], sampleRows: [["10"]], rowCount: 1 });
  ok("stubBrain returns build_debt_schedule", dbsStub.proposals[0]?.kind === "build_debt_schedule");
  const stubValidateDbs = validateProposal("build_debt_schedule", dbsStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateDbs.ok && stubValidateDbs.kind === "build_debt_schedule");

  const { runAgent: runAgentDbs } = await import("./lib/run-agent");
  const orgDbs = await makeOrg("pro");
  const payloadDbs = await makePayload(orgDbs);
  const rDbs = await runAgentDbs({ orgId: orgDbs, payloadId: payloadDbs, role: "debt_schedule_agent" }, { db, brain: sbDbs });
  const { data: pendDbs } = await db.from("proposed_actions").select("kind").eq("org_id", orgDbs);
  ok("runAgent succeeds and returns correct action kind",
    rDbs.ok && rDbs.proposalCount === 1 && pendDbs?.length === 1 && pendDbs[0].kind === "build_debt_schedule");

  const { routePayload: routeDbs } = await import("./lib/manager");
  const routeCheckDbs = await routeDbs({ orgId: orgDbs, payloadId: payloadDbs }, { db, enqueue: () => {} });
  ok("debt_schedule_agent routes on the financial route", routeCheckDbs.ok && routeCheckDbs.plan.includes("debt_schedule_agent"));
  await db.from("organizations").delete().eq("id", orgDbs);
  console.log("== equity waterfall agent ==");
  ok("model_equity_waterfall accepts good", validateProposal("model_equity_waterfall", {
    exit_scenarios: [{
      scenario_name: "15M Exit", exit_valuation: 15000000,
      distributions: [{ stakeholder: "Series A", share_class: "Series A Preferred", proceeds: 5000000, ownership_pct: 33.3, multiple_on_invested: 1.0 }],
    }],
    share_classes: [{ class_name: "Series A Preferred", shares_outstanding: 5000000, liquidation_preference: 5000000, preference_multiple: 1.0, is_participating: false }],
    total_invested_capital: 5000000, total_option_pool_shares: 1500000, fully_diluted_shares: 15000000,
    waterfall_notes: "x",
  }).ok);
  ok("model_equity_waterfall rejects empty exit_scenarios array", !validateProposal("model_equity_waterfall", {
    exit_scenarios: [], share_classes: [],
    total_invested_capital: 0, total_option_pool_shares: 0, fully_diluted_shares: 0,
    waterfall_notes: "x",
  }).ok);
  ok("model_equity_waterfall rejects empty waterfall_notes", !validateProposal("model_equity_waterfall", {
    exit_scenarios: [{ scenario_name: "x", exit_valuation: 1000, distributions: [] }],
    share_classes: [],
    total_invested_capital: 0, total_option_pool_shares: 0, fully_diluted_shares: 0,
    waterfall_notes: "",
  }).ok);
  ok("model_equity_waterfall filters out exit_scenarios item with empty scenario_name", (() => {
    const r = validateProposal("model_equity_waterfall", {
      exit_scenarios: [
        { scenario_name: "15M Exit", exit_valuation: 15000000, distributions: [] },
        { scenario_name: "", exit_valuation: 5000000, distributions: [] },
      ],
      share_classes: [],
      total_invested_capital: 0, total_option_pool_shares: 0, fully_diluted_shares: 0,
      waterfall_notes: "x",
    });
    return r.ok && (r.payload.exit_scenarios as unknown[]).length === 1;
  })());

  const { stubBrain: sbEwf } = await import("./lib/agent-brain");
  const ewfStub = await sbEwf.propose({ role: "equity_waterfall_agent", columns: ["amount"], sampleRows: [["10"]], rowCount: 1 });
  ok("stubBrain returns model_equity_waterfall", ewfStub.proposals[0]?.kind === "model_equity_waterfall");
  const stubValidateEwf = validateProposal("model_equity_waterfall", ewfStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateEwf.ok && stubValidateEwf.kind === "model_equity_waterfall");

  const { runAgent: runAgentEwf } = await import("./lib/run-agent");
  const orgEwf = await makeOrg("pro");
  const payloadEwf = await makePayload(orgEwf);
  const rEwf = await runAgentEwf({ orgId: orgEwf, payloadId: payloadEwf, role: "equity_waterfall_agent" }, { db, brain: sbEwf });
  const { data: pendEwf } = await db.from("proposed_actions").select("kind").eq("org_id", orgEwf);
  ok("runAgent succeeds and returns correct action kind",
    rEwf.ok && rEwf.proposalCount === 1 && pendEwf?.length === 1 && pendEwf[0].kind === "model_equity_waterfall");

  const { routePayload: routeEwf } = await import("./lib/manager");
  const routeCheckEwf = await routeEwf({ orgId: orgEwf, payloadId: payloadEwf }, { db, enqueue: () => {} });
  ok("equity_waterfall_agent routes on the financial route", routeCheckEwf.ok && routeCheckEwf.plan.includes("equity_waterfall_agent"));
  await db.from("organizations").delete().eq("id", orgEwf);

  console.log("== payroll analytics agent ==");
  ok("analyze_payroll accepts good", validateProposal("analyze_payroll", {
    pay_period: "January 2024",
    total_gross_payroll: 425000, total_employer_burden: 93500, total_fully_loaded_cost: 518500, employee_count: 22,
    avg_salary_per_employee: 232090,
    payroll_by_department: [{ department: "Engineering", gross_payroll: 210000, headcount: 10 }],
    payroll_by_type: [{ employment_type: "Full-time", gross_payroll: 380000, headcount: 19 }],
    burden_rate_pct: 22.0, payroll_as_pct_revenue: 56.7,
    salary_band_analysis: [{ band_label: "$100k-$150k", employee_count: 10, total_cost: 115000 }],
    overtime_cost: 8500, period_over_period_change_pct: 4.2,
    flags: ["x"],
  }).ok);
  ok("analyze_payroll rejects empty pay_period", !validateProposal("analyze_payroll", {
    pay_period: "", total_gross_payroll: 0, total_employer_burden: 0, total_fully_loaded_cost: 0, employee_count: 0,
    avg_salary_per_employee: null,
    payroll_by_department: [], payroll_by_type: [],
    burden_rate_pct: null, payroll_as_pct_revenue: null,
    salary_band_analysis: [], overtime_cost: 0, period_over_period_change_pct: null, flags: [],
  }).ok);
  ok("analyze_payroll rejects negative employee_count", !validateProposal("analyze_payroll", {
    pay_period: "January 2024", total_gross_payroll: 0, total_employer_burden: 0, total_fully_loaded_cost: 0, employee_count: -1,
    avg_salary_per_employee: null,
    payroll_by_department: [], payroll_by_type: [],
    burden_rate_pct: null, payroll_as_pct_revenue: null,
    salary_band_analysis: [], overtime_cost: 0, period_over_period_change_pct: null, flags: [],
  }).ok);
  ok("analyze_payroll filters out payroll_by_department item with empty department", (() => {
    const r = validateProposal("analyze_payroll", {
      pay_period: "January 2024", total_gross_payroll: 0, total_employer_burden: 0, total_fully_loaded_cost: 0, employee_count: 5,
      avg_salary_per_employee: null,
      payroll_by_department: [
        { department: "Engineering", gross_payroll: 100, headcount: 3 },
        { department: "", gross_payroll: 50, headcount: 2 },
      ],
      payroll_by_type: [],
      burden_rate_pct: null, payroll_as_pct_revenue: null,
      salary_band_analysis: [], overtime_cost: 0, period_over_period_change_pct: null, flags: [],
    });
    return r.ok && (r.payload.payroll_by_department as unknown[]).length === 1;
  })());

  const { stubBrain: sbPya } = await import("./lib/agent-brain");
  const pyaStub = await sbPya.propose({ role: "payroll_analytics_agent", columns: ["x"], sampleRows: [["y"]], rowCount: 1 });
  ok("stubBrain returns analyze_payroll", pyaStub.proposals[0]?.kind === "analyze_payroll");
  const stubValidatePya = validateProposal("analyze_payroll", pyaStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidatePya.ok && stubValidatePya.kind === "analyze_payroll");

  const { runAgent: runAgentPya } = await import("./lib/run-agent");
  const orgPya = await makeOrg("pro");
  const payloadPya = await makePayload(orgPya);
  const rPya = await runAgentPya({ orgId: orgPya, payloadId: payloadPya, role: "payroll_analytics_agent" }, { db, brain: sbPya });
  const { data: pendPya } = await db.from("proposed_actions").select("kind").eq("org_id", orgPya);
  ok("runAgent succeeds and returns correct action kind",
    rPya.ok && rPya.proposalCount === 1 && pendPya?.length === 1 && pendPya[0].kind === "analyze_payroll");

  const { routePayload: routePya } = await import("./lib/manager");
  const routeCheckPyaFin = await routePya({ orgId: orgPya, payloadId: payloadPya }, { db, enqueue: () => {} });
  const { data: plainPayloadPya } = await db.from("inbound_payloads").insert({
    org_id: orgPya, source: "upload", storage_path: `${orgPya}/pya/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckPyaNonFin = await routePya({ orgId: orgPya, payloadId: plainPayloadPya!.id }, { db, enqueue: () => {} });
  ok("payroll_analytics_agent routes on BOTH the financial and non-financial route",
    routeCheckPyaFin.ok && routeCheckPyaFin.plan.includes("payroll_analytics_agent") &&
    routeCheckPyaNonFin.ok && routeCheckPyaNonFin.plan.includes("payroll_analytics_agent"));
  await db.from("organizations").delete().eq("id", orgPya);
  console.log("== vendor concentration agent ==");
  ok("analyze_vendor_concentration accepts good", validateProposal("analyze_vendor_concentration", {
    analysis_period: "FY2023", total_vendor_spend: 1850000, vendor_count: 47,
    top_vendors: [{ vendor_name: "AWS", total_spend: 620000, spend_pct: 33.5, category: "cloud_infrastructure", is_single_source: true }],
    spend_by_category: [{ category: "Cloud Infrastructure", total_spend: 680000, vendor_count: 2 }],
    concentration_hhi: 1285.0, top3_spend_pct: 51.0, top10_spend_pct: 74.3,
    single_source_vendors: [{ vendor_name: "AWS", annual_spend: 620000, category: "cloud_infrastructure" }],
    at_risk_spend: 765000, concentration_risk_level: "high", recommendations: ["x"],
  }).ok);
  ok("analyze_vendor_concentration rejects invalid concentration_risk_level", !validateProposal("analyze_vendor_concentration", {
    analysis_period: "FY2023", total_vendor_spend: 0, vendor_count: 0,
    top_vendors: [], spend_by_category: [],
    concentration_hhi: null, top3_spend_pct: null, top10_spend_pct: null,
    single_source_vendors: [], at_risk_spend: 0, concentration_risk_level: "mystery", recommendations: [],
  }).ok);
  ok("analyze_vendor_concentration rejects empty analysis_period", !validateProposal("analyze_vendor_concentration", {
    analysis_period: "", total_vendor_spend: 0, vendor_count: 0,
    top_vendors: [], spend_by_category: [],
    concentration_hhi: null, top3_spend_pct: null, top10_spend_pct: null,
    single_source_vendors: [], at_risk_spend: 0, concentration_risk_level: "low", recommendations: [],
  }).ok);
  ok("analyze_vendor_concentration filters out top_vendors item with empty vendor_name", (() => {
    const r = validateProposal("analyze_vendor_concentration", {
      analysis_period: "FY2023", total_vendor_spend: 100, vendor_count: 2,
      top_vendors: [
        { vendor_name: "AWS", total_spend: 60, spend_pct: 60, category: "cloud", is_single_source: true },
        { vendor_name: "", total_spend: 40, spend_pct: 40, category: "cloud", is_single_source: false },
      ],
      spend_by_category: [],
      concentration_hhi: null, top3_spend_pct: null, top10_spend_pct: null,
      single_source_vendors: [], at_risk_spend: 0, concentration_risk_level: "low", recommendations: [],
    });
    return r.ok && (r.payload.top_vendors as unknown[]).length === 1;
  })());

  const { stubBrain: sbVcn } = await import("./lib/agent-brain");
  const vcnStub = await sbVcn.propose({ role: "vendor_concentration_agent", columns: ["x"], sampleRows: [["y"]], rowCount: 1 });
  ok("stubBrain returns analyze_vendor_concentration", vcnStub.proposals[0]?.kind === "analyze_vendor_concentration");
  const stubValidateVcn = validateProposal("analyze_vendor_concentration", vcnStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateVcn.ok && stubValidateVcn.kind === "analyze_vendor_concentration");

  const { runAgent: runAgentVcn } = await import("./lib/run-agent");
  const orgVcn = await makeOrg("pro");
  const payloadVcn = await makePayload(orgVcn);
  const rVcn = await runAgentVcn({ orgId: orgVcn, payloadId: payloadVcn, role: "vendor_concentration_agent" }, { db, brain: sbVcn });
  const { data: pendVcn } = await db.from("proposed_actions").select("kind").eq("org_id", orgVcn);
  ok("runAgent succeeds and returns correct action kind",
    rVcn.ok && rVcn.proposalCount === 1 && pendVcn?.length === 1 && pendVcn[0].kind === "analyze_vendor_concentration");

  const { routePayload: routeVcn } = await import("./lib/manager");
  const routeCheckVcnFin = await routeVcn({ orgId: orgVcn, payloadId: payloadVcn }, { db, enqueue: () => {} });
  const { data: plainPayloadVcn } = await db.from("inbound_payloads").insert({
    org_id: orgVcn, source: "upload", storage_path: `${orgVcn}/vcn/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckVcnNonFin = await routeVcn({ orgId: orgVcn, payloadId: plainPayloadVcn!.id }, { db, enqueue: () => {} });
  ok("vendor_concentration_agent routes on BOTH the financial and non-financial route",
    routeCheckVcnFin.ok && routeCheckVcnFin.plan.includes("vendor_concentration_agent") &&
    routeCheckVcnNonFin.ok && routeCheckVcnNonFin.plan.includes("vendor_concentration_agent"));
  await db.from("organizations").delete().eq("id", orgVcn);

  console.log("== investor metrics agent ==");
  ok("compute_investor_metrics accepts good", validateProposal("compute_investor_metrics", {
    as_of_period: "Q4 2023", arr: 8400000, mrr: 700000, arr_growth_rate_yoy: 68.0,
    ...
    gross_margin_pct: 72.0, operating_margin_pct: null, net_magic_number: null, quick_ratio: 3.2,
    summary: "x", rating: "above_average",
  }).ok);
  ok("compute_investor_metrics rejects invalid rating", !validateProposal("compute_investor_metrics", {
    as_of_period: "Q4 2023", arr: null, mrr: null, arr_growth_rate_yoy: null,
    net_revenue_retention: null, gross_revenue_retention: null,
    rule_of_40_score: null, magic_number: null, burn_multiple: null,
    ltv_cac_ratio: null, payback_period_months: null, arr_per_fte: null,
    gross_margin_pct: null, operating_margin_pct: null, net_magic_number: null, quick_ratio: null,
    summary: "x", rating: "mystery",
  }).ok);
  ok("compute_investor_metrics rejects empty as_of_period", !validateProposal("compute_investor_metrics", {
    as_of_period: "", arr: null, mrr: null, arr_growth_rate_yoy: null,
    net_revenue_retention: null, gross_revenue_retention: null,
    rule_of_40_score: null, magic_number: null, burn_multiple: null,
    ltv_cac_ratio: null, payback_period_months: null, arr_per_fte: null,
    gross_margin_pct: null, operating_margin_pct: null, net_magic_number: null, quick_ratio: null,
    summary: "x", rating: "average",
  }).ok);
  ok("compute_investor_metrics rejects net_revenue_retention out of range (>300)", !validateProposal("compute_investor_metrics", {
    as_of_period: "Q4 2023", arr: null, mrr: null, arr_growth_rate_yoy: null,
    net_revenue_retention: 350, gross_revenue_retention: null,
    rule_of_40_score: null, magic_number: null, burn_multiple: null,
    ltv_cac_ratio: null, payback_period_months: null, arr_per_fte: null,
    gross_margin_pct: null, operating_margin_pct: null, net_magic_number: null, quick_ratio: null,
    summary: "x", rating: "average",
  ...
  const { stubBrain: sbImt } = await import("./lib/agent-brain");
  const imtStub = await sbImt.propose({ role: "investor_metrics_agent", columns: ["amount"], sampleRows: [["10"]], rowCount: 1 });
  ok("stubBrain returns compute_investor_metrics", imtStub.proposals[0]?.kind === "compute_investor_metrics");
  const stubValidateImt = validateProposal("compute_investor_metrics", imtStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateImt.ok && stubValidateImt.kind === "compute_investor_metrics");

  const { runAgent: runAgentImt } = await import("./lib/run-agent");
  const orgImt = await makeOrg("pro");
  const payloadImt = await makePayload(orgImt);
  const rImt = await runAgentImt({ orgId: orgImt, payloadId: payloadImt, role: "investor_metrics_agent" }, { db, brain: sbImt });
  const { data: pendImt } = await db.from("proposed_actions").select("kind").eq("org_id", orgImt);
  ok("runAgent succeeds and returns correct action kind",
    rImt.ok && rImt.proposalCount === 1 && pendImt?.length === 1 && pendImt[0].kind === "compute_investor_metrics");

  const { routePayload: routeImt } = await import("./lib/manager");
  const routeCheckImt = await routeImt({ orgId: orgImt, payloadId: payloadImt }, { db, enqueue: () => {} });
  ok("investor_metrics_agent routes on the financial route", routeCheckImt.ok && routeCheckImt.plan.includes("investor_metrics_agent"));
  await db.from("organizations").delete().eq("id", orgImt);
  console.log("== clv agent ==");
  ok("compute_clv accepts good", validateProposal("compute_clv", {
    analysis_period: "FY2023", avg_ltv: 42000, avg_cac: 11200, ltv_cac_ratio: 3.75,
    avg_revenue_per_customer_per_month: 3500, avg_gross_margin_pct: 72.0,
    avg_customer_lifespan_months: 16.7, monthly_churn_rate: 6.0, payback_period_months: 4.4,
    clv_by_segment: [{ segment_name: "Enterprise", avg_ltv: 125000, customer_count: 8, avg_cac: 28000 }],
    top_clv_customers: [{ customer_id: "ENT-001", estimated_ltv: 185000, monthly_revenue: 11500 }],
    bottom_clv_customers: [{ customer_id: "SMB-098", estimated_ltv: 4200, monthly_revenue: 350 }],
    methodology_notes: "x",
  }).ok);
  ok("compute_clv rejects empty analysis_period", !validateProposal("compute_clv", {
    analysis_period: "", avg_ltv: null, avg_cac: null, ltv_cac_ratio: null,
    avg_revenue_per_customer_per_month: null, avg_gross_margin_pct: null,
    avg_customer_lifespan_months: null, monthly_churn_rate: null, payback_period_months: null,
    clv_by_segment: [], top_clv_customers: [], bottom_clv_customers: [],
    methodology_notes: "x",
  }).ok);
  ok("compute_clv rejects monthly_churn_rate > 100", !validateProposal("compute_clv", {
    analysis_period: "FY2023", avg_ltv: null, avg_cac: null, ltv_cac_ratio: null,
    avg_revenue_per_customer_per_month: null, avg_gross_margin_pct: null,
    avg_customer_lifespan_months: null, monthly_churn_rate: 150, payback_period_months: null,
    clv_by_segment: [], top_clv_customers: [], bottom_clv_customers: [],
    methodology_notes: "x",
  }).ok);
  ok("compute_clv filters out clv_by_segment item with empty segment_name", (() => {
    const r = validateProposal("compute_clv", {
      analysis_period: "FY2023", avg_ltv: null, avg_cac: null, ltv_cac_ratio: null,
      avg_revenue_per_customer_per_month: null, avg_gross_margin_pct: null,
      avg_customer_lifespan_months: null, monthly_churn_rate: null, payback_period_months: null,
      clv_by_segment: [
        { segment_name: "Enterprise", avg_ltv: 100, customer_count: 5, avg_cac: 20 },
        { segment_name: "", avg_ltv: 50, customer_count: 3, avg_cac: 10 },
      ],
      top_clv_customers: [], bottom_clv_customers: [],
      methodology_notes: "x",
    });
    return r.ok && (r.payload.clv_by_segment as unknown[]).length === 1;
  })());

  const { stubBrain: sbClv } = await import("./lib/agent-brain");
  const clvStub = await sbClv.propose({ role: "clv_agent", columns: ["x"], sampleRows: [["y"]], rowCount: 1 });
  ok("stubBrain returns compute_clv", clvStub.proposals[0]?.kind === "compute_clv");
  const stubValidateClv = validateProposal("compute_clv", clvStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateClv.ok && stubValidateClv.kind === "compute_clv");

  const { runAgent: runAgentClv } = await import("./lib/run-agent");
  const orgClv = await makeOrg("pro");
  const payloadClv = await makePayload(orgClv);
  const rClv = await runAgentClv({ orgId: orgClv, payloadId: payloadClv, role: "clv_agent" }, { db, brain: sbClv });
  const { data: pendClv } = await db.from("proposed_actions").select("kind").eq("org_id", orgClv);
  ok("runAgent succeeds and returns correct action kind",
    rClv.ok && rClv.proposalCount === 1 && pendClv?.length === 1 && pendClv[0].kind === "compute_clv");

  const { routePayload: routeClv } = await import("./lib/manager");
  const routeCheckClvFin = await routeClv({ orgId: orgClv, payloadId: payloadClv }, { db, enqueue: () => {} });
  const { data: plainPayloadClv } = await db.from("inbound_payloads").insert({
    org_id: orgClv, source: "upload", storage_path: `${orgClv}/clv/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckClvNonFin = await routeClv({ orgId: orgClv, payloadId: plainPayloadClv!.id }, { db, enqueue: () => {} });
  ok("clv_agent routes on BOTH the financial and non-financial route",
    routeCheckClvFin.ok && routeCheckClvFin.plan.includes("clv_agent") &&
    routeCheckClvNonFin.ok && routeCheckClvNonFin.plan.includes("clv_agent"));
  await db.from("organizations").delete().eq("id", orgClv);

  console.log("== payment terms agent ==");
  ok("analyze_payment_terms accepts good", validateProposal("analyze_payment_terms", {
    analysis_period: "Q4 2023",
    customer_terms_analysis: [{ customer_name: "Acme Corp", stated_terms: "Net 30", actual_avg_days: 47.0, outstanding_balance: 85000 }],
    vendor_terms_analysis: [{ vendor_name: "Cloud Provider", stated_terms: "Net 30", actual_avg_days: 28.0, outstanding_balance: 52000 }],
    avg_dso_days: 43.5, avg_dpo_days: 29.0, working_capital_impact: 49875,
    early_payment_discount_opportunities: [{ vendor_name: "Office Supplies Co", discount_pct: 2.0, terms_description: "2/10 Net 30", annual_savings: 1920 }],
    late_payment_flags: ["x"], optimization_opportunities: ["x"],
  }).ok);
  ok("analyze_payment_terms rejects empty analysis_period", !validateProposal("analyze_payment_terms", {
    analysis_period: "", customer_terms_analysis: [], vendor_terms_analysis: [],
    avg_dso_days: null, avg_dpo_days: null, working_capital_impact: null,
    early_payment_discount_opportunities: [], late_payment_flags: [], optimization_opportunities: [],
  }).ok);
  ok("analyze_payment_terms rejects negative avg_dso_days", !validateProposal("analyze_payment_terms", {
    analysis_period: "Q4 2023", customer_terms_analysis: [], vendor_terms_analysis: [],
    avg_dso_days: -5, avg_dpo_days: null, working_capital_impact: null,
    early_payment_discount_opportunities: [], late_payment_flags: [], optimization_opportunities: [],
  }).ok);
  ok("analyze_payment_terms filters out customer_terms_analysis item with empty customer_name", (() => {
    const r = validateProposal("analyze_payment_terms", {
      analysis_period: "Q4 2023",
      customer_terms_analysis: [
        { customer_name: "Acme Corp", stated_terms: "Net 30", actual_avg_days: 47.0, outstanding_balance: 85000 },
        { customer_name: "", stated_terms: "Net 30", actual_avg_days: 20.0, outstanding_balance: 1000 },
      ],
      vendor_terms_analysis: [],
      avg_dso_days: null, avg_dpo_days: null, working_capital_impact: null,
      early_payment_discount_opportunities: [], late_payment_flags: [], optimization_opportunities: [],
    });
    return r.ok && (r.payload.customer_terms_analysis as unknown[]).length === 1;
  })());

  const { stubBrain: sbPtm } = await import("./lib/agent-brain");
  const ptmStub = await sbPtm.propose({ role: "payment_terms_agent", columns: ["amount"], sampleRows: [["10"]], rowCount: 1 });
  ok("stubBrain returns analyze_payment_terms", ptmStub.proposals[0]?.kind === "analyze_payment_terms");
  const stubValidatePtm = validateProposal("analyze_payment_terms", ptmStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidatePtm.ok && stubValidatePtm.kind === "analyze_payment_terms");

  const { runAgent: runAgentPtm } = await import("./lib/run-agent");
  const orgPtm = await makeOrg("pro");
  const payloadPtm = await makePayload(orgPtm);
  const rPtm = await runAgentPtm({ orgId: orgPtm, payloadId: payloadPtm, role: "payment_terms_agent" }, { db, brain: sbPtm });
  const { data: pendPtm } = await db.from("proposed_actions").select("kind").eq("org_id", orgPtm);
  ok("runAgent succeeds and returns correct action kind",
    rPtm.ok && rPtm.proposalCount === 1 && pendPtm?.length === 1 && pendPtm[0].kind === "analyze_payment_terms");

  const { routePayload: routePtm } = await import("./lib/manager");
  const routeCheckPtm = await routePtm({ orgId: orgPtm, payloadId: payloadPtm }, { db, enqueue: () => {} });
  ok("payment_terms_agent routes on the financial route", routeCheckPtm.ok && routeCheckPtm.plan.includes("payment_terms_agent"));
  await db.from("organizations").delete().eq("id", orgPtm);
  console.log("== sensitivity analysis agent ==");
  ok("analyze_sensitivity accepts good", validateProposal("analyze_sensitivity", {
    base_case_metric: "Annual EBITDA", base_case_value: 1200000,
    sensitivity_variables: [{ variable_name: "Revenue Growth Rate (%)", base_value: 25.0, low_value: 10.0, high_value: 40.0, metric_impact_low: 550000, metric_impact_high: 1850000, impact_magnitude: 1300000 }],
    tornado_chart_data: [{ variable_name: "Revenue Growth Rate (%)", downside_impact: -650000, upside_impact: 650000 }],
    scenario_matrix: [{ scenario_label: "Bear Case", variable_combination: "Revenue +10%, GM 65%", resulting_metric_value: 320000 }],
    key_drivers: ["x"],
    break_even_points: [{ variable_name: "Revenue Growth Rate (%)", break_even_value: 5.2, current_value: 25.0, headroom_pct: 79.2 }],
    methodology_notes: "x",
  }).ok);
  ok("analyze_sensitivity rejects empty sensitivity_variables array", !validateProposal("analyze_sensitivity", {
    base_case_metric: "Annual EBITDA", base_case_value: 1200000,
    sensitivity_variables: [], tornado_chart_data: [], scenario_matrix: [],
    key_drivers: [], break_even_points: [], methodology_notes: "x",
  }).ok);
  ok("analyze_sensitivity rejects empty base_case_metric", !validateProposal("analyze_sensitivity", {
    base_case_metric: "", base_case_value: 1200000,
    sensitivity_variables: [{ variable_name: "x", base_value: 1, low_value: 0, high_value: 2, metric_impact_low: 1, metric_impact_high: 2, impact_magnitude: 1 }],
    tornado_chart_data: [], scenario_matrix: [], key_drivers: [], break_even_points: [], methodology_notes: "x",
  }).ok);
  ok("analyze_sensitivity filters out sensitivity_variables item with empty variable_name", (() => {
    const r = validateProposal("analyze_sensitivity", {
      base_case_metric: "Annual EBITDA", base_case_value: 1200000,
      sensitivity_variables: [
        { variable_name: "Revenue Growth Rate (%)", base_value: 25.0, low_value: 10.0, high_value: 40.0, metric_impact_low: 550000, metric_impact_high: 1850000, impact_magnitude: 1300000 },
        { variable_name: "", base_value: 1, low_value: 0, high_value: 2, metric_impact_low: 1, metric_impact_high: 2, impact_magnitude: 1 },
      ],
      tornado_chart_data: [], scenario_matrix: [], key_drivers: [], break_even_points: [], methodology_notes: "x",
    });
    return r.ok && (r.payload.sensitivity_variables as unknown[]).length === 1;
  })());

  const { stubBrain: sbSen } = await import("./lib/agent-brain");
  const senStub = await sbSen.propose({ role: "sensitivity_analysis_agent", columns: ["x"], sampleRows: [["y"]], rowCount: 1 });
  ok("stubBrain returns analyze_sensitivity", senStub.proposals[0]?.kind === "analyze_sensitivity");
  const stubValidateSen = validateProposal("analyze_sensitivity", senStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateSen.ok && stubValidateSen.kind === "analyze_sensitivity");

  const { runAgent: runAgentSen } = await import("./lib/run-agent");
  const orgSen = await makeOrg("pro");
  const payloadSen = await makePayload(orgSen);
  const rSen = await runAgentSen({ orgId: orgSen, payloadId: payloadSen, role: "sensitivity_analysis_agent" }, { db, brain: sbSen });
  const { data: pendSen } = await db.from("proposed_actions").select("kind").eq("org_id", orgSen);
  ok("runAgent succeeds and returns correct action kind",
    rSen.ok && rSen.proposalCount === 1 && pendSen?.length === 1 && pendSen[0].kind === "analyze_sensitivity");

  const { routePayload: routeSen } = await import("./lib/manager");
  const routeCheckSenFin = await routeSen({ orgId: orgSen, payloadId: payloadSen }, { db, enqueue: () => {} });
  const { data: plainPayloadSen } = await db.from("inbound_payloads").insert({
    org_id: orgSen, source: "upload", storage_path: `${orgSen}/sen/plain.csv`, original_filename: "plain.csv",
    mime_type: "text/csv", scan_status: "clean", status: "completed",
    extracted_json: { columns: ["name", "city"], rowCount: 1, rows: [["a", "b"]], truncated: false, parser: "static-mvp" },
  }).select("id").single();
  const routeCheckSenNonFin = await routeSen({ orgId: orgSen, payloadId: plainPayloadSen!.id }, { db, enqueue: () => {} });
  ok("sensitivity_analysis_agent routes on BOTH the financial and non-financial route",
    routeCheckSenFin.ok && routeCheckSenFin.plan.includes("sensitivity_analysis_agent") &&
    routeCheckSenNonFin.ok && routeCheckSenNonFin.plan.includes("sensitivity_analysis_agent"));
  await db.from("organizations").delete().eq("id", orgSen);

  console.log("== working capital optimization agent ==");
  ok("optimize_working_capital accepts good", validateProposal("optimize_working_capital", {
    analysis_period: "Q4 2023", current_ccc_days: 38.5, target_ccc_days: 22.0,
    current_dso: 45.0, current_dpo: 28.0, current_dio: 21.5, cash_release_opportunity: 285000,
    ar_optimization: [{ action: "Automate reminders", estimated_dso_reduction_days: 8.0, estimated_cash_released: 110000, effort: "low" }],
    ap_optimization: [{ action: "Renegotiate terms", estimated_dpo_increase_days: 15.0, estimated_cash_retained: 95000, effort: "medium" }],
    inventory_optimization: [{ action: "Reduce safety stock", estimated_dio_reduction_days: 6.0, estimated_cash_released: 42000, effort: "medium" }],
    priority_actions: [{ rank: 1, action_summary: "Automated AR reminders", cash_impact: 110000, timeline_weeks: 3 }],
    twelve_month_impact: 285000, implementation_complexity: "medium",
  }).ok);
  ok("optimize_working_capital rejects invalid implementation_complexity", !validateProposal("optimize_working_capital", {
    analysis_period: "Q4 2023", current_ccc_days: null, target_ccc_days: null,
    current_dso: null, current_dpo: null, current_dio: null, cash_release_opportunity: null,
    ar_optimization: [], ap_optimization: [], inventory_optimization: [],
    priority_actions: [{ rank: 1, action_summary: "x", cash_impact: 0, timeline_weeks: 0 }],
    twelve_month_impact: null, implementation_complexity: "mystery",
  }).ok);
  ok("optimize_working_capital rejects empty analysis_period", !validateProposal("optimize_working_capital", {
    analysis_period: "", current_ccc_days: null, target_ccc_days: null,
    current_dso: null, current_dpo: null, current_dio: null, cash_release_opportunity: null,
    ar_optimization: [], ap_optimization: [], inventory_optimization: [],
    priority_actions: [{ rank: 1, action_summary: "x", cash_impact: 0, timeline_weeks: 0 }],
    twelve_month_impact: null, implementation_complexity: "low",
  }).ok);
  ok("optimize_working_capital rejects empty priority_actions array", !validateProposal("optimize_working_capital", {
    analysis_period: "Q4 2023", current_ccc_days: null, target_ccc_days: null,
    current_dso: null, current_dpo: null, current_dio: null, cash_release_opportunity: null,
    ar_optimization: [], ap_optimization: [], inventory_optimization: [],
    priority_actions: [],
  ...
  const { stubBrain: sbWco } = await import("./lib/agent-brain");
  const wcoStub = await sbWco.propose({ role: "working_capital_optimization_agent", columns: ["amount"], sampleRows: [["10"]], rowCount: 1 });
  ok("stubBrain returns optimize_working_capital", wcoStub.proposals[0]?.kind === "optimize_working_capital");
  const stubValidateWco = validateProposal("optimize_working_capital", wcoStub.proposals[0].action_payload);
  ok("stub proposal passes validateProposal", stubValidateWco.ok && stubValidateWco.kind === "optimize_working_capital");

  const { runAgent: runAgentWco } = await import("./lib/run-agent");
  const orgWco = await makeOrg("pro");
  const payloadWco = await makePayload(orgWco);
  const rWco = await runAgentWco({ orgId: orgWco, payloadId: payloadWco, role: "working_capital_optimization_agent" }, { db, brain: sbWco });
  const { data: pendWco } = await db.from("proposed_actions").select("kind").eq("org_id", orgWco);
  ok("runAgent succeeds and returns correct action kind",
    rWco.ok && rWco.proposalCount === 1 && pendWco?.length === 1 && pendWco[0].kind === "optimize_working_capital");

  const { routePayload: routeWco } = await import("./lib/manager");
  const routeCheckWco = await routeWco({ orgId: orgWco, payloadId: payloadWco }, { db, enqueue: () => {} });
  ok("working_capital_optimization_agent routes on the financial route", routeCheckWco.ok && routeCheckWco.plan.includes("working_capital_optimization_agent"));
  await db.from("organizations").delete().eq("id", orgWco);
