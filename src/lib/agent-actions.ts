...
export function validateProposal(kind: string, payload: unknown): Ok | Err {
  ...
  if (kind === "draft_investor_update") {
    const subject_line = typeof p.subject_line === "string" && p.subject_line.length > 0 ? p.subject_line.slice(0, 100) : null;
    if (!subject_line) return { ok: false, reason: "missing_subject_line" };

    const headline_metric = str(p.headline_metric);
    if (!headline_metric) return { ok: false, reason: "missing_headline_metric" };

    const rawKpis = Array.isArray(p.kpi_summary) ? (p.kpi_summary as unknown[]).slice(0, 8) : [];
    const kpi_summary: { kpi: string; value: string; vs_last_period: string | null }[] = [];
    for (const item of rawKpis) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const kpi = str(rec.kpi);
      if (!kpi) continue;
      kpi_summary.push({ kpi, value: str(rec.value) ?? "", vs_last_period: str(rec.vs_last_period) });
    }
    if (kpi_summary.length < 3) return { ok: false, reason: "insufficient_kpi_summary" };

    const wins = strArray(p.wins, 6, MAX_STR);
    if (wins.length < 2) return { ok: false, reason: "insufficient_wins" };

    const challenges = strArray(p.challenges, 4, MAX_STR);
    if (challenges.length < 1) return { ok: false, reason: "empty_challenges" };

    const rawAsks = Array.isArray(p.asks) ? (p.asks as unknown[]).slice(0, 4) : [];
    const asks: { ask: string; from_whom: string; context: string }[] = [];
    for (const item of rawAsks) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const ask = str(rec.ask);
      if (!ask) continue;
      asks.push({ ask, from_whom: str(rec.from_whom) ?? "", context: str(rec.context) ?? "" });
    }
    if (asks.length < 1) return { ok: false, reason: "empty_asks" };

    const rawTargets = Array.isArray(p.next_period_targets) ? (p.next_period_targets as unknown[]).slice(0, 5) : [];
    const next_period_targets: { target: string; metric: string; due_date: string }[] = [];
    for (const item of rawTargets) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const target = str(rec.target);
      if (!target) continue;
      next_period_targets.push({ target, metric: str(rec.metric) ?? "", due_date: str(rec.due_date) ?? "" });
    }
    if (next_period_targets.length < 2) return { ok: false, reason: "insufficient_next_period_targets" };

    const full_draft = typeof p.full_draft === "string" && p.full_draft.length > 0 ? p.full_draft.slice(0, 3000) : null;
    if (!full_draft) return { ok: false, reason: "missing_full_draft" };
    ...
    return {
      ...
      payload: { subject_line, headline_metric, kpi_summary, wins, challenges, asks, next_period_targets, full_draft, period },
    };
  }

  if (kind === "orchestrate_agents") {
    const data_summary = typeof p.data_summary === "string" && p.data_summary.length > 0 ? p.data_summary.slice(0, 500) : null;
    if (!data_summary) return { ok: false, reason: "missing_data_summary" };

    const PRIORITIES = ["required", "recommended", "optional"];
    const rawRecommended = Array.isArray(p.recommended_agents) ? (p.recommended_agents as unknown[]).slice(0, 15) : [];
    const recommended_agents: { role_key: string; priority: string; reason: string }[] = [];
    for (const item of rawRecommended) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const priority = typeof rec.priority === "string" && PRIORITIES.includes(rec.priority) ? rec.priority : null;
      if (!priority) continue;
      recommended_agents.push({ role_key: str(rec.role_key) ?? "", priority, reason: str(rec.reason) ?? "" });
    }
    if (recommended_agents.length < 1) return { ok: false, reason: "empty_recommended_agents" };

    const skip_agents = strArray(p.skip_agents, 50, MAX_STR);
    const execution_order = strArray(p.execution_order, 15, MAX_STR);

    const routing_rationale = str(p.routing_rationale);
    if (!routing_rationale) return { ok: false, reason: "missing_routing_rationale" };

    const estimated_insights = strArray(p.estimated_insights, 10, MAX_STR);

    return {
      ok: true,
      kind: "orchestrate_agents",
      payload: { data_summary, recommended_agents, skip_agents, execution_order, routing_rationale, estimated_insights },
    };
  }

  if (kind === "review_confidence") {
    const CONFIDENCE_LEVELS_CR = ["high", "medium", "low"];
    const rawReviewed = Array.isArray(p.reviewed_proposals) ? (p.reviewed_proposals as unknown[]).slice(0, 20) : [];
    const reviewed_proposals: { agent_role: string; action_kind: string; confidence_assessment: string; concerns: string[]; verified_fields: string[] }[] = [];
    for (const item of rawReviewed) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const confidence_assessment = typeof rec.confidence_assessment === "string" && CONFIDENCE_LEVELS_CR.includes(rec.confidence_assessment) ? rec.confidence_assessment : null;
      if (!confidence_assessment) continue;
      reviewed_proposals.push({
        agent_role: str(rec.agent_role) ?? "",
        action_kind: str(rec.action_kind) ?? "",
        confidence_assessment,
        concerns: strArray(rec.concerns, 20, MAX_STR),
        verified_fields: strArray(rec.verified_fields, 20, MAX_STR),
      });
    }

    const overall_confidence = typeof p.overall_confidence === "string" && CONFIDENCE_LEVELS_CR.includes(p.overall_confidence) ? p.overall_confidence : null;
    if (!overall_confidence) return { ok: false, reason: "bad_overall_confidence" };

    const high_confidence_count = numOrNull(p.high_confidence_count, 0);
    if (high_confidence_count === NUM_INVALID || high_confidence_count === null) return { ok: false, reason: "bad_high_confidence_count" };
    const medium_confidence_count = numOrNull(p.medium_confidence_count, 0);
    if (medium_confidence_count === NUM_INVALID || medium_confidence_count === null) return { ok: false, reason: "bad_medium_confidence_count" };
    const low_confidence_count = numOrNull(p.low_confidence_count, 0);
    if (low_confidence_count === NUM_INVALID || low_confidence_count === null) return { ok: false, reason: "bad_low_confidence_count" };

    const flags = strArray(p.flags, 15, MAX_STR);

    const APPROVAL_RECS = ["approve_all", "approve_with_review", "hold_for_review", "reject"];
    const approval_recommendation = typeof p.approval_recommendation === "string" && APPROVAL_RECS.includes(p.approval_recommendation) ? p.approval_recommendation : null;
    if (!approval_recommendation) return { ok: false, reason: "bad_approval_recommendation" };

    return {
      ok: true,
      kind: "review_confidence",
      payload: { reviewed_proposals, overall_confidence, high_confidence_count, medium_confidence_count, low_confidence_count, flags, approval_recommendation },
    };
  }

  if (kind === "reshape_data") {
    const SOURCE_SHAPES = ["wide", "narrow", "pivoted", "unknown"];
    const source_shape = typeof p.source_shape === "string" && SOURCE_SHAPES.includes(p.source_shape) ? p.source_shape : null;
    if (!source_shape) return { ok: false, reason: "bad_source_shape" };

    const TARGET_SHAPES = ["wide", "narrow", "pivoted"];
    const target_shape = typeof p.target_shape === "string" && TARGET_SHAPES.includes(p.target_shape) ? p.target_shape : null;
    if (!target_shape) return { ok: false, reason: "bad_target_shape" };

    const id_columns = strArray(p.id_columns, 20, MAX_STR);
    if (id_columns.length < 1) return { ok: false, reason: "empty_id_columns" };

    const variable_column = typeof p.variable_column === "string" ? p.variable_column.slice(0, MAX_STR) : "";
    const value_column = typeof p.value_column === "string" ? p.value_column.slice(0, MAX_STR) : "";

    const reshaped_preview = Array.isArray(p.reshaped_preview) ? (p.reshaped_preview as unknown[]).slice(0, 10) : [];

    const row_count_before = numOrNull(p.row_count_before, 0);
    if (row_count_before === NUM_INVALID || row_count_before === null) return { ok: false, reason: "bad_row_count_before" };
    const row_count_after = numOrNull(p.row_count_after, 0);
    if (row_count_after === NUM_INVALID || row_count_after === null) return { ok: false, reason: "bad_row_count_after" };
    const column_count_before = numOrNull(p.column_count_before, 0);
    if (column_count_before === NUM_INVALID || column_count_before === null) return { ok: false, reason: "bad_column_count_before" };
    const column_count_after = numOrNull(p.column_count_after, 0);
    if (column_count_after === NUM_INVALID || column_count_after === null) return { ok: false, reason: "bad_column_count_after" };

    const reshape_notes = typeof p.reshape_notes === "string" ? p.reshape_notes.slice(0, 500) : "";

    return {
      ok: true,
      kind: "reshape_data",
      payload: { source_shape, target_shape, id_columns, variable_column, value_column, reshaped_preview, row_count_before, row_count_after, column_count_before, column_count_after, reshape_notes },
    };
  }

  if (kind === "normalize_dates") {
    const rawDetected = Array.isArray(p.detected_formats) ? (p.detected_formats as unknown[]).slice(0, 20) : [];
    const detected_formats: { column_name: string; formats_found: string[]; sample_values: string[]; iso_convertible: boolean; ambiguous_count: number }[] = [];
    for (const item of rawDetected) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      const ambiguousCount = numOrNull(rec.ambiguous_count, 0);
      detected_formats.push({
        column_name,
        formats_found: strArray(rec.formats_found, 5, MAX_STR),
        sample_values: strArray(rec.sample_values, 3, MAX_STR),
        iso_convertible: rec.iso_convertible === true,
        ambiguous_count: ambiguousCount === NUM_INVALID || ambiguousCount === null ? 0 : ambiguousCount,
      });
    }

    const TARGET_FORMATS = ["YYYY-MM-DD", "YYYY-MM-DDTHH:MM:SSZ"];
    const target_format = typeof p.target_format === "string" && TARGET_FORMATS.includes(p.target_format) ? p.target_format : null;
    if (!target_format) return { ok: false, reason: "bad_target_format" };

    const rawMap = Array.isArray(p.normalization_map) ? (p.normalization_map as unknown[]).slice(0, 20) : [];
    const normalization_map: { original_format: string; iso_format: string; example: string }[] = [];
    for (const item of rawMap) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const original_format = str(rec.original_format);
      if (!original_format) continue;
      normalization_map.push({ original_format, iso_format: str(rec.iso_format) ?? "", example: str(rec.example) ?? "" });
    }

    const ambiguous_dates = strArray(p.ambiguous_dates, 50, MAX_STR);
    const timezone_issues = strArray(p.timezone_issues, 20, MAX_STR);

    const rows_affected = numOrNull(p.rows_affected, 0);
    if (rows_affected === NUM_INVALID || rows_affected === null) return { ok: false, reason: "bad_rows_affected" };
    const columns_affected = numOrNull(p.columns_affected, 0);
    if (columns_affected === NUM_INVALID || columns_affected === null) return { ok: false, reason: "bad_columns_affected" };
    ...
    return {
      ...
      payload: { detected_formats, target_format, normalization_map, ambiguous_dates, timezone_issues, rows_affected, columns_affected, recommendations },
    };
  }

  if (kind === "normalize_strings") {
    const rawColumns = Array.isArray(p.columns_analyzed) ? (p.columns_analyzed as unknown[]).slice(0, 30) : [];
    const columns_analyzed: { column_name: string; unique_values_before: number; unique_values_after: number; issues_found: string[] }[] = [];
    for (const item of rawColumns) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      const before = numOrNull(rec.unique_values_before, 0);
      const after = numOrNull(rec.unique_values_after, 0);
      columns_analyzed.push({
        column_name,
        unique_values_before: before === NUM_INVALID || before === null ? 0 : before,
        unique_values_after: after === NUM_INVALID || after === null ? 0 : after,
        issues_found: strArray(rec.issues_found, 20, MAX_STR),
      });
    }

    const rawWhitespace = Array.isArray(p.whitespace_issues) ? (p.whitespace_issues as unknown[]).slice(0, 20) : [];
    const whitespace_issues: { column_name: string; count: number }[] = [];
    for (const item of rawWhitespace) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      const count = numOrNull(rec.count, 0);
      whitespace_issues.push({ column_name, count: count === NUM_INVALID || count === null ? 0 : count });
    }

    const CASES = ["upper", "lower", "title"];
    const rawCases = Array.isArray(p.case_standardization) ? (p.case_standardization as unknown[]).slice(0, 20) : [];
    const case_standardization: { column_name: string; recommended_case: string; example_before: string; example_after: string }[] = [];
    for (const item of rawCases) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const recommended_case = typeof rec.recommended_case === "string" && CASES.includes(rec.recommended_case) ? rec.recommended_case : null;
      if (!recommended_case) continue;
      case_standardization.push({
        column_name: str(rec.column_name) ?? "",
        recommended_case,
        example_before: str(rec.example_before) ?? "",
        example_after: str(rec.example_after) ?? "",
      });
    }

    const rawDedup = Array.isArray(p.entity_dedup_candidates) ? (p.entity_dedup_candidates as unknown[]).slice(0, 50) : [];
    const entity_dedup_candidates: { original: string; canonical: string; similarity_score: number }[] = [];
    for (const item of rawDedup) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const similarity_score = numOrNull(rec.similarity_score, 0, 1);
      if (similarity_score === NUM_INVALID || similarity_score === null) continue;
      entity_dedup_candidates.push({ original: str(rec.original) ?? "", canonical: str(rec.canonical) ?? "", similarity_score });
    }

    const encoding_issues = strArray(p.encoding_issues, 10, MAX_STR);

    const total_values_affected = numOrNull(p.total_values_affected, 0);
    if (total_values_affected === NUM_INVALID || total_values_affected === null) return { ok: false, reason: "bad_total_values_affected" };

    const rawRules = Array.isArray(p.normalization_rules) ? (p.normalization_rules as unknown[]).slice(0, 20) : [];
    const normalization_rules: { rule_type: string; description: string; columns_affected: string[] }[] = [];
    for (const item of rawRules) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const rule_type = str(rec.rule_type);
      if (!rule_type) continue;
      normalization_rules.push({ rule_type, description: str(rec.description) ?? "", columns_affected: strArray(rec.columns_affected, 20, MAX_STR) });
    }

    return {
      ok: true,
      kind: "normalize_strings",
      payload: { columns_analyzed, whitespace_issues, case_standardization, entity_dedup_candidates, encoding_issues, total_values_affected, normalization_rules },
    };
  }

  if (kind === "normalize_currency") {
    const rawCurrencies = Array.isArray(p.currencies_detected) ? (p.currencies_detected as unknown[]).slice(0, 20) : [];
    const currencies_detected: { currency_code: string; symbol: string; row_count: number }[] = [];
    for (const item of rawCurrencies) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const currency_code = str(rec.currency_code);
      if (!currency_code) continue;
      const row_count = numOrNull(rec.row_count, 0);
      currencies_detected.push({ currency_code, symbol: str(rec.symbol) ?? "", row_count: row_count === NUM_INVALID || row_count === null ? 0 : row_count });
    }

    const base_currency = str(p.base_currency);
    if (!base_currency) return { ok: false, reason: "missing_base_currency" };

    if (typeof p.conversion_needed !== "boolean") return { ok: false, reason: "bad_conversion_needed" };
    const conversion_needed = p.conversion_needed;

    const rows_with_mixed_currency = numOrNull(p.rows_with_mixed_currency, 0);
    if (rows_with_mixed_currency === NUM_INVALID || rows_with_mixed_currency === null) return { ok: false, reason: "bad_rows_with_mixed_currency" };

    const rawIssues = Array.isArray(p.normalization_issues) ? (p.normalization_issues as unknown[]).slice(0, 20) : [];
    const normalization_issues: { column_name: string; issue_type: string; description: string; row_count: number }[] = [];
    for (const item of rawIssues) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      const row_count = numOrNull(rec.row_count, 0);
      normalization_issues.push({
        column_name,
        issue_type: str(rec.issue_type) ?? "",
        description: str(rec.description) ?? "",
        row_count: row_count === NUM_INVALID || row_count === null ? 0 : row_count,
      });
    }

    const conversion_recommendations = strArray(p.conversion_recommendations, 10, MAX_STR);
    const columns_affected = strArray(p.columns_affected, 20, MAX_STR);
  ...
  if (kind === "analyze_distribution") {
    const DATA_TYPES = ["numeric", "integer", "currency"];
    const rawCols = Array.isArray(p.columns_analyzed) ? (p.columns_analyzed as unknown[]).slice(0, 30) : [];
    const columns_analyzed: { column_name: string; data_type: string; min_val: number; max_val: number; mean: number; median: number; std_dev: number; null_count: number; row_count: number }[] = [];
    for (const item of rawCols) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      if (typeof rec.data_type !== "string" || !DATA_TYPES.includes(rec.data_type)) continue;
      const min_val = numOrNull(rec.min_val);
      const max_val = numOrNull(rec.max_val);
      const mean = numOrNull(rec.mean);
      const median = numOrNull(rec.median);
      const std_dev = numOrNull(rec.std_dev);
      const null_count = numOrNull(rec.null_count, 0);
      const row_count = numOrNull(rec.row_count, 0);
      if ([min_val, max_val, mean, median, std_dev, null_count, row_count].some((v) => v === NUM_INVALID || v === null)) continue;
      columns_analyzed.push({ column_name, data_type: rec.data_type, min_val: min_val as number, max_val: max_val as number, mean: mean as number, median: median as number, std_dev: std_dev as number, null_count: null_count as number, row_count: row_count as number });
    }
    if (columns_analyzed.length === 0) return { ok: false, reason: "empty_columns_analyzed" };

    const DISTRIBUTION_SHAPES = ["normal", "right_skewed", "left_skewed", "bimodal", "uniform", "unknown"];
    const rawDist = Array.isArray(p.distribution_summary) ? (p.distribution_summary as unknown[]).slice(0, 30) : [];
    const distribution_summary: { column_name: string; distribution_shape: string; percentile_25: number; percentile_75: number; iqr: number }[] = [];
    for (const item of rawDist) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      if (typeof rec.distribution_shape !== "string" || !DISTRIBUTION_SHAPES.includes(rec.distribution_shape)) continue;
      const percentile_25 = numOrNull(rec.percentile_25);
      const percentile_75 = numOrNull(rec.percentile_75);
      const iqr = numOrNull(rec.iqr);
      if ([percentile_25, percentile_75, iqr].some((v) => v === NUM_INVALID || v === null)) continue;
      distribution_summary.push({ column_name, distribution_shape: rec.distribution_shape, percentile_25: percentile_25 as number, percentile_75: percentile_75 as number, iqr: iqr as number });
    }

    const skewness_flags = strArray(p.skewness_flags, 20, MAX_STR);

    const rawOutliers = Array.isArray(p.outlier_summary) ? (p.outlier_summary as unknown[]).slice(0, 30) : [];
    const outlier_summary: { column_name: string; outlier_count: number; outlier_threshold_low: number; outlier_threshold_high: number }[] = [];
    for (const item of rawOutliers) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      const outlier_count = numOrNull(rec.outlier_count, 0);
      const outlier_threshold_low = numOrNull(rec.outlier_threshold_low);
      const outlier_threshold_high = numOrNull(rec.outlier_threshold_high);
      if ([outlier_count, outlier_threshold_low, outlier_threshold_high].some((v) => v === NUM_INVALID || v === null)) continue;
      outlier_summary.push({ column_name, outlier_count: outlier_count as number, outlier_threshold_low: outlier_threshold_low as number, outlier_threshold_high: outlier_threshold_high as number });
    }

    const rawNormality = Array.isArray(p.normality_assessment) ? (p.normality_assessment as unknown[]).slice(0, 30) : [];
    const normality_assessment: { column_name: string; likely_normal: boolean; assessment_note: string }[] = [];
    for (const item of rawNormality) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const column_name = str(rec.column_name);
      if (!column_name) continue;
      if (typeof rec.likely_normal !== "boolean") continue;
      normality_assessment.push({ column_name, likely_normal: rec.likely_normal, assessment_note: str(rec.assessment_note) ?? "" });
    }

    const visualization_recommendations = strArray(p.visualization_recommendations, 10, MAX_STR);

    return {
      ok: true,
      kind: "analyze_distribution",
      payload: { columns_analyzed, distribution_summary, skewness_flags, outlier_summary, normality_assessment, visualization_recommendations },
    };
  }

  if (kind === "analyze_correlation") {
    const STRENGTHS = ["strong", "moderate", "weak", "none"];
    const DIRECTIONS = ["positive", "negative", "none"];

    const rawPairs = Array.isArray(p.correlation_pairs) ? (p.correlation_pairs as unknown[]).slice(0, 200) : [];
    const correlation_pairs: { col_a: string; col_b: string; pearson_r: number; strength: string; direction: string }[] = [];
    for (const item of rawPairs) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const col_a = str(rec.col_a);
      const col_b = str(rec.col_b);
      if (!col_a || !col_b) continue;
      const pearson_r = numOrNull(rec.pearson_r, -1, 1);
      if (pearson_r === NUM_INVALID || pearson_r === null) continue;
      if (typeof rec.strength !== "string" || !STRENGTHS.includes(rec.strength)) continue;
      if (typeof rec.direction !== "string" || !DIRECTIONS.includes(rec.direction)) continue;
      correlation_pairs.push({ col_a, col_b, pearson_r, strength: rec.strength, direction: rec.direction });
    }

    const rawStrong = Array.isArray(p.strong_correlations) ? (p.strong_correlations as unknown[]).slice(0, 20) : [];
    const strong_correlations: { col_a: string; col_b: string; pearson_r: number; interpretation: string }[] = [];
    for (const item of rawStrong) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const col_a = str(rec.col_a);
      if (!col_a) continue;
      const pearson_r = numOrNull(rec.pearson_r, -1, 1);
      if (pearson_r === NUM_INVALID || pearson_r === null) continue;
      strong_correlations.push({ col_a, col_b: str(rec.col_b) ?? "", pearson_r, interpretation: str(rec.interpretation) ?? "" });
    }

    const rawSurprising = Array.isArray(p.surprising_correlations) ? (p.surprising_correlations as unknown[]).slice(0, 10) : [];
    const surprising_correlations: { col_a: string; col_b: string; pearson_r: number; why_surprising: string }[] = [];
    for (const item of rawSurprising) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const col_a = str(rec.col_a);
      if (!col_a) continue;
      const pearson_r = numOrNull(rec.pearson_r, -1, 1);
      if (pearson_r === NUM_INVALID || pearson_r === null) continue;
      surprising_correlations.push({ col_a, col_b: str(rec.col_b) ?? "", pearson_r, why_surprising: str(rec.why_surprising) ?? "" });
    }

    const multicollinearity_flags = strArray(p.multicollinearity_flags, 15, MAX_STR);
    const business_insights = strArray(p.business_insights, 10, MAX_STR);

    const columns_included = numOrNull(p.columns_included, 0);
    if (columns_included === NUM_INVALID || columns_included === null) return { ok: false, reason: "bad_columns_included" };
  ...
  if (kind === "test_hypothesis") {
    const TEST_TYPES = ["t_test", "chi_square", "anova", "mann_whitney", "proportion_z_test"];
    const rawTests = Array.isArray(p.tests_performed) ? (p.tests_performed as unknown[]).slice(0, 20) : [];
    const tests_performed: { test_name: string; test_type: string; null_hypothesis: string; p_value: number; reject_null: boolean; confidence_level: number }[] = [];
    for (const item of rawTests) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const test_name = str(rec.test_name);
      if (!test_name) continue;
      if (typeof rec.test_type !== "string" || !TEST_TYPES.includes(rec.test_type)) continue;
      const p_value = numOrNull(rec.p_value, 0, 1);
      if (p_value === NUM_INVALID || p_value === null) continue;
      const confidence_level = numOrNull(rec.confidence_level, 0, 1);
      if (confidence_level === NUM_INVALID || confidence_level === null) continue;
      if (typeof rec.reject_null !== "boolean") continue;
      tests_performed.push({ test_name, test_type: rec.test_type, null_hypothesis: str(rec.null_hypothesis) ?? "", p_value, reject_null: rec.reject_null, confidence_level });
    }
    if (tests_performed.length === 0) return { ok: false, reason: "empty_tests_performed" };

    const rawSignificant = Array.isArray(p.significant_findings) ? (p.significant_findings as unknown[]).slice(0, 10) : [];
    const significant_findings: { test_name: string; finding: string; p_value: number; practical_significance: string }[] = [];
    for (const item of rawSignificant) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const test_name = str(rec.test_name);
      if (!test_name) continue;
      const p_value = numOrNull(rec.p_value, 0, 1);
      if (p_value === NUM_INVALID || p_value === null) continue;
      significant_findings.push({ test_name, finding: str(rec.finding) ?? "", p_value, practical_significance: str(rec.practical_significance) ?? "" });
    }

    const non_significant_findings = strArray(p.non_significant_findings, 10, MAX_STR);

    const EFFECT_INTERPRETATIONS = ["negligible", "small", "medium", "large"];
    const rawEffects = Array.isArray(p.effect_sizes) ? (p.effect_sizes as unknown[]).slice(0, 20) : [];
    const effect_sizes: { test_name: string; effect_size_measure: string; value: number; interpretation: string }[] = [];
    for (const item of rawEffects) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const test_name = str(rec.test_name);
      if (!test_name) continue;
      if (typeof rec.interpretation !== "string" || !EFFECT_INTERPRETATIONS.includes(rec.interpretation)) continue;
      const value = numOrNull(rec.value);
      if (value === NUM_INVALID || value === null) continue;
      effect_sizes.push({ test_name, effect_size_measure: str(rec.effect_size_measure) ?? "", value, interpretation: rec.interpretation });
    }

    const recommended_actions = strArray(p.recommended_actions, 10, MAX_STR);
    const statistical_caveats = strArray(p.statistical_caveats, 10, MAX_STR);

    return {
      ok: true,
      kind: "test_hypothesis",
      payload: { tests_performed, significant_findings, non_significant_findings, effect_sizes, recommended_actions, statistical_caveats },
    };
  }

  if (kind === "analyze_pareto") {
    const analysis_dimension = str(p.analysis_dimension);
    if (!analysis_dimension) return { ok: false, reason: "missing_analysis_dimension" };

    const value_metric = str(p.value_metric);
    if (!value_metric) return { ok: false, reason: "missing_value_metric" };

    const rawEntries = Array.isArray(p.pareto_entries) ? (p.pareto_entries as unknown[]).slice(0, 100) : [];
    const pareto_entries: { rank: number; dimension_value: string; metric_value: number; cumulative_pct: number; pct_of_total: number }[] = [];
    for (const item of rawEntries) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const rank = numOrNull(rec.rank, 1);
      if (rank === NUM_INVALID || rank === null) continue;
      const dimension_value = str(rec.dimension_value);
      if (!dimension_value) continue;
      const metric_value = numOrNull(rec.metric_value, 0);
      if (metric_value === NUM_INVALID || metric_value === null) continue;
      const cumulative_pct = numOrNull(rec.cumulative_pct, 0, 100);
      if (cumulative_pct === NUM_INVALID || cumulative_pct === null) continue;
      const pct_of_total = numOrNull(rec.pct_of_total, 0, 100);
      if (pct_of_total === NUM_INVALID || pct_of_total === null) continue;
      pareto_entries.push({ rank, dimension_value, metric_value, cumulative_pct, pct_of_total });
    }
    if (pareto_entries.length === 0) return { ok: false, reason: "empty_pareto_entries" };

    function parseEightyTwentyThreshold(v: unknown): { items_in_top_80_pct: number; items_in_top_80_pct_pct: number; actual_80_pct_item_count: number } | null {
      if (typeof v !== "object" || v === null) return null;
      const rec = v as Record<string, unknown>;
      const items_in_top_80_pct = numOrNull(rec.items_in_top_80_pct, 0);
      if (items_in_top_80_pct === NUM_INVALID || items_in_top_80_pct === null) return null;
      const items_in_top_80_pct_pct = numOrNull(rec.items_in_top_80_pct_pct, 0, 100);
      if (items_in_top_80_pct_pct === NUM_INVALID || items_in_top_80_pct_pct === null) return null;
      const actual_80_pct_item_count = numOrNull(rec.actual_80_pct_item_count, 0);
      if (actual_80_pct_item_count === NUM_INVALID || actual_80_pct_item_count === null) return null;
      return { items_in_top_80_pct, items_in_top_80_pct_pct, actual_80_pct_item_count };
    }
    const eighty_twenty_threshold = parseEightyTwentyThreshold(p.eighty_twenty_threshold);
    if (!eighty_twenty_threshold) return { ok: false, reason: "bad_eighty_twenty_threshold" };

    const concentration_score = numOrNull(p.concentration_score, 0, 100);
    if (concentration_score === NUM_INVALID || concentration_score === null) return { ok: false, reason: "bad_concentration_score" };

    const long_tail_count = numOrNull(p.long_tail_count, 0);
    if (long_tail_count === NUM_INVALID || long_tail_count === null) return { ok: false, reason: "bad_long_tail_count" };

    const business_implications = strArray(p.business_implications, 10, MAX_STR);
    if (business_implications.length === 0) return { ok: false, reason: "empty_business_implications" };

    return {
      ok: true,
      kind: "analyze_pareto",
      payload: { analysis_dimension, value_metric, pareto_entries, eighty_twenty_threshold, concentration_score, long_tail_count, business_implications },
    };
  }

  if (kind === "cluster_data") {
    const clustering_dimensions = strArray(p.clustering_dimensions, 10, MAX_STR);
    if (clustering_dimensions.length === 0) return { ok: false, reason: "empty_clustering_dimensions" };

    const cluster_count = numOrNull(p.cluster_count, 2, 10);
    if (cluster_count === NUM_INVALID || cluster_count === null) return { ok: false, reason: "bad_cluster_count" };

    const rawClusters = Array.isArray(p.clusters) ? (p.clusters as unknown[]).slice(0, 10) : [];
    const clusters: { cluster_id: number; label: string; size: number; centroid_description: string; key_characteristics: string[] }[] = [];
    for (const item of rawClusters) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const label = str(rec.label);
      if (!label) continue;
      const cluster_id = numOrNull(rec.cluster_id, 0);
      if (cluster_id === NUM_INVALID || cluster_id === null) continue;
      const size = numOrNull(rec.size, 0);
      if (size === NUM_INVALID || size === null) continue;
      clusters.push({ cluster_id, label, size, centroid_description: str(rec.centroid_description) ?? "", key_characteristics: strArray(rec.key_characteristics, 5, MAX_STR) });
    }
    if (clusters.length < 2) return { ok: false, reason: "too_few_clusters" };

    const CLUSTER_QUALITIES = ["excellent", "good", "fair", "poor"];
    const cluster_quality = typeof p.cluster_quality === "string" && CLUSTER_QUALITIES.includes(p.cluster_quality) ? p.cluster_quality : null;
    if (!cluster_quality) return { ok: false, reason: "bad_cluster_quality" };

    const rawOutliers = Array.isArray(p.outlier_entities) ? (p.outlier_entities as unknown[]).slice(0, 20) : [];
    const outlier_entities: { entity: string; reason: string }[] = [];
    for (const item of rawOutliers) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const entity = str(rec.entity);
      if (!entity) continue;
      outlier_entities.push({ entity, reason: str(rec.reason) ?? "" });
    }

    const rawSegments = Array.isArray(p.business_segments) ? (p.business_segments as unknown[]).slice(0, 10) : [];
    const business_segments: { segment_name: string; description: string; recommended_treatment: string }[] = [];
    for (const item of rawSegments) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const segment_name = str(rec.segment_name);
      if (!segment_name) continue;
      business_segments.push({ segment_name, description: str(rec.description) ?? "", recommended_treatment: str(rec.recommended_treatment) ?? "" });
    }
    ...
    return {
      ok: true,
      kind: "cluster_data",
      payload: { clustering_dimensions, cluster_count, clusters, cluster_quality, outlier_entities, business_segments, recommended_actions },
    };
  }

  if (kind === "analyze_funnel") {
    const funnel_name = str(p.funnel_name);
    if (!funnel_name) return { ok: false, reason: "missing_funnel_name" };

    const rawStages = Array.isArray(p.funnel_stages) ? (p.funnel_stages as unknown[]).slice(0, 10) : [];
    const funnel_stages: { stage_name: string; entity_count: number; conversion_rate_from_previous: number; drop_off_count: number }[] = [];
    for (const item of rawStages) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const stage_name = str(rec.stage_name);
      if (!stage_name) continue;
      const entity_count = numOrNull(rec.entity_count, 0);
      if (entity_count === NUM_INVALID || entity_count === null) continue;
      const conversion_rate_from_previous = numOrNull(rec.conversion_rate_from_previous, 0, 100);
      if (conversion_rate_from_previous === NUM_INVALID || conversion_rate_from_previous === null) continue;
      const drop_off_count = numOrNull(rec.drop_off_count, 0);
      if (drop_off_count === NUM_INVALID || drop_off_count === null) continue;
      funnel_stages.push({ stage_name, entity_count, conversion_rate_from_previous, drop_off_count });
    }
    if (funnel_stages.length < 2) return { ok: false, reason: "too_few_funnel_stages" };

    const overall_conversion_rate = numOrNull(p.overall_conversion_rate, 0, 100);
    if (overall_conversion_rate === NUM_INVALID || overall_conversion_rate === null) return { ok: false, reason: "bad_overall_conversion_rate" };

    const biggest_drop_off_stage = str(p.biggest_drop_off_stage);
    if (!biggest_drop_off_stage) return { ok: false, reason: "missing_biggest_drop_off_stage" };

    const rawDropOff = Array.isArray(p.drop_off_analysis) ? (p.drop_off_analysis as unknown[]).slice(0, 10) : [];
    const drop_off_analysis: { stage: string; drop_off_count: number; drop_off_pct: number; likely_causes: string[] }[] = [];
    for (const item of rawDropOff) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const stage = str(rec.stage);
      if (!stage) continue;
      const drop_off_count = numOrNull(rec.drop_off_count, 0);
      if (drop_off_count === NUM_INVALID || drop_off_count === null) continue;
      const drop_off_pct = numOrNull(rec.drop_off_pct, 0, 100);
      if (drop_off_pct === NUM_INVALID || drop_off_pct === null) continue;
      drop_off_analysis.push({ stage, drop_off_count, drop_off_pct, likely_causes: strArray(rec.likely_causes, 3, MAX_STR) });
    }

    const rawSegments = Array.isArray(p.segment_breakdowns) ? (p.segment_breakdowns as unknown[]).slice(0, 10) : [];
    const segment_breakdowns: { segment: string; overall_conversion_rate: number; notable_difference: string }[] = [];
    for (const item of rawSegments) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const segment = str(rec.segment);
      if (!segment) continue;
      const segConvRate = numOrNull(rec.overall_conversion_rate, 0, 100);
      if (segConvRate === NUM_INVALID || segConvRate === null) continue;
      segment_breakdowns.push({ segment, overall_conversion_rate: segConvRate, notable_difference: str(rec.notable_difference) ?? "" });
    }

    const optimization_recommendations = strArray(p.optimization_recommendations, 10, MAX_STR);
    if (optimization_recommendations.length === 0) return { ok: false, reason: "empty_optimization_recommendations" };

    return {
      ok: true,
      kind: "analyze_funnel",
      payload: { funnel_name, funnel_stages, overall_conversion_rate, biggest_drop_off_stage, drop_off_analysis, segment_breakdowns, optimization_recommendations },
    };
  }

  if (kind === "analyze_retention") {
    const COHORT_TYPES = ["monthly", "weekly", "quarterly", "signup_date", "first_purchase"];
    const cohort_type = typeof p.cohort_type === "string" && COHORT_TYPES.includes(p.cohort_type) ? p.cohort_type : null;
    if (!cohort_type) return { ok: false, reason: "bad_cohort_type" };

    function parseNumArray(v: unknown, maxItems: number): number[] {
      if (!Array.isArray(v)) return [];
      return v
        .map((n) => numOrNull(n, 0, 100))
        .filter((n): n is number => n !== null && n !== NUM_INVALID)
        .slice(0, maxItems);
    }

    const rawCohorts = Array.isArray(p.cohorts) ? (p.cohorts as unknown[]).slice(0, 24) : [];
    const cohorts: { cohort_label: string; initial_count: number; retention_by_period: number[] }[] = [];
    for (const item of rawCohorts) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const initial_count = numOrNull(rec.initial_count, 1);
      if (initial_count === NUM_INVALID || initial_count === null) continue;
      cohorts.push({ cohort_label: str(rec.cohort_label) ?? "", initial_count, retention_by_period: parseNumArray(rec.retention_by_period, 24) });
    }
    if (cohorts.length === 0) return { ok: false, reason: "empty_cohorts" };

    const rawAvgRetention = Array.isArray(p.average_retention_by_period) ? (p.average_retention_by_period as unknown[]).slice(0, 24) : [];
    const average_retention_by_period: { period: string; avg_retention_pct: number }[] = [];
    for (const item of rawAvgRetention) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const avg_retention_pct = numOrNull(rec.avg_retention_pct, 0, 100);
      if (avg_retention_pct === NUM_INVALID || avg_retention_pct === null) continue;
      average_retention_by_period.push({ period: str(rec.period) ?? "", avg_retention_pct });
    }

    const RETENTION_CURVE_SHAPES = ["steep_initial_drop", "gradual_decline", "smile_curve", "flat_high", "flat_low"];
    const retention_curve_shape = typeof p.retention_curve_shape === "string" && RETENTION_CURVE_SHAPES.includes(p.retention_curve_shape) ? p.retention_curve_shape : null;
    if (!retention_curve_shape) return { ok: false, reason: "bad_retention_curve_shape" };

    const critical_drop_off_period = str(p.critical_drop_off_period);
    if (!critical_drop_off_period) return { ok: false, reason: "missing_critical_drop_off_period" };

    const IMPACTS = ["high", "medium", "low"];
    const rawChurnDrivers = Array.isArray(p.churn_drivers) ? (p.churn_drivers as unknown[]).slice(0, 10) : [];
    const churn_drivers: { driver: string; estimated_impact: string; evidence: string }[] = [];
    for (const item of rawChurnDrivers) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      if (typeof rec.estimated_impact !== "string" || !IMPACTS.includes(rec.estimated_impact)) continue;
      churn_drivers.push({ driver: str(rec.driver) ?? "", estimated_impact: rec.estimated_impact, evidence: str(rec.evidence) ?? "" });
    }

    const COMPARISONS = ["above", "below", "at"];
    const rawBenchmarks = Array.isArray(p.retention_benchmarks) ? (p.retention_benchmarks as unknown[]).slice(0, 5) : [];
    const retention_benchmarks: { benchmark_name: string; value: number; comparison: string }[] = [];
    for (const item of rawBenchmarks) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      if (typeof rec.comparison !== "string" || !COMPARISONS.includes(rec.comparison)) continue;
      const value = numOrNull(rec.value, 0, 100);
      if (value === NUM_INVALID || value === null) continue;
      retention_benchmarks.push({ benchmark_name: str(rec.benchmark_name) ?? "", value, comparison: rec.comparison });
    }

    const improvement_recommendations = strArray(p.improvement_recommendations, 10, MAX_STR);
    if (improvement_recommendations.length === 0) return { ok: false, reason: "empty_improvement_recommendations" };

    return {
      ok: true,
      kind: "analyze_retention",
      payload: { cohort_type, cohorts, average_retention_by_period, retention_curve_shape, critical_drop_off_period, churn_drivers, retention_benchmarks, improvement_recommendations },
    };
  }

  if (kind === "analyze_ab_test") {
    const test_name = str(p.test_name);
    if (!test_name) return { ok: false, reason: "missing_test_name" };

    const hypothesis = str(p.hypothesis);
    if (!hypothesis) return { ok: false, reason: "missing_hypothesis" };

    const rawVariants = Array.isArray(p.variants) ? (p.variants as unknown[]).slice(0, 4) : [];
    const variants: { variant_name: string; sample_size: number; primary_metric_value: number; conversion_rate: number }[] = [];
    for (const item of rawVariants) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const sample_size = numOrNull(rec.sample_size, 1);
      if (sample_size === NUM_INVALID || sample_size === null) continue;
      const primary_metric_value = numOrNull(rec.primary_metric_value);
      if (primary_metric_value === NUM_INVALID || primary_metric_value === null) continue;
      const conversion_rate = numOrNull(rec.conversion_rate, 0, 100);
      if (conversion_rate === NUM_INVALID || conversion_rate === null) continue;
      variants.push({ variant_name: str(rec.variant_name) ?? "", sample_size, primary_metric_value, conversion_rate });
    }
    if (variants.length < 2) return { ok: false, reason: "too_few_variants" };

    const primary_metric = str(p.primary_metric);
    if (!primary_metric) return { ok: false, reason: "missing_primary_metric" };

    function parseStatisticalResult(v: unknown): { p_value: number; confidence_level: number; statistically_significant: boolean; test_type: string } | null {
      if (typeof v !== "object" || v === null) return null;
      const rec = v as Record<string, unknown>;
      const p_value = numOrNull(rec.p_value, 0, 1);
      if (p_value === NUM_INVALID || p_value === null) return null;
      const confidence_level = numOrNull(rec.confidence_level, 0, 1);
      if (confidence_level === NUM_INVALID || confidence_level === null) return null;
      if (typeof rec.statistically_significant !== "boolean") return null;
      const test_type = str(rec.test_type);
      if (!test_type) return null;
      return { p_value, confidence_level, statistically_significant: rec.statistically_significant, test_type };
    }
    const statistical_result = parseStatisticalResult(p.statistical_result);
    if (!statistical_result) return { ok: false, reason: "bad_statistical_result" };

    const WINNERS = ["control", "variant_a", "variant_b", "variant_c", "no_winner", "inconclusive"];
    const winner = typeof p.winner === "string" && WINNERS.includes(p.winner) ? p.winner : null;
    if (!winner) return { ok: false, reason: "bad_winner" };

    const EFFECT_SIZES = ["large", "medium", "small", "negligible"];
    const effect_size = typeof p.effect_size === "string" && EFFECT_SIZES.includes(p.effect_size) ? p.effect_size : null;
    if (!effect_size) return { ok: false, reason: "bad_effect_size" };

    const SAMPLE_SIZE_ADEQUACIES = ["adequate", "borderline", "insufficient"];
    const sample_size_adequacy = typeof p.sample_size_adequacy === "string" && SAMPLE_SIZE_ADEQUACIES.includes(p.sample_size_adequacy) ? p.sample_size_adequacy : null;
    if (!sample_size_adequacy) return { ok: false, reason: "bad_sample_size_adequacy" };

    const rawSegmentEffects = Array.isArray(p.segment_effects) ? (p.segment_effects as unknown[]).slice(0, 10) : [];
    const segment_effects: { segment: string; winner_in_segment: string; conversion_lift_pct: number }[] = [];
    for (const item of rawSegmentEffects) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const segment = str(rec.segment);
      if (!segment) continue;
      const conversion_lift_pct = numOrNull(rec.conversion_lift_pct);
      if (conversion_lift_pct === NUM_INVALID || conversion_lift_pct === null) continue;
      segment_effects.push({ segment, winner_in_segment: str(rec.winner_in_segment) ?? "", conversion_lift_pct });
    }
    ...
    return {
      ...
      payload: { test_name, hypothesis, variants, primary_metric, statistical_result, winner, effect_size, sample_size_adequacy, segment_effects, recommendation, caveats },
    };
  }

  if (kind === "analyze_nps") {
    const nps_score = numOrNull(p.nps_score, -100, 100);
    if (nps_score === NUM_INVALID || nps_score === null) return { ok: false, reason: "bad_nps_score" };

    const promoter_count = numOrNull(p.promoter_count, 0);
    if (promoter_count === NUM_INVALID || promoter_count === null) return { ok: false, reason: "bad_promoter_count" };

    const passive_count = numOrNull(p.passive_count, 0);
    if (passive_count === NUM_INVALID || passive_count === null) return { ok: false, reason: "bad_passive_count" };

    const detractor_count = numOrNull(p.detractor_count, 0);
    if (detractor_count === NUM_INVALID || detractor_count === null) return { ok: false, reason: "bad_detractor_count" };

    const total_responses = numOrNull(p.total_responses, 1);
    if (total_responses === NUM_INVALID || total_responses === null) return { ok: false, reason: "bad_total_responses" };

    const promoter_pct = numOrNull(p.promoter_pct, 0, 100);
    if (promoter_pct === NUM_INVALID || promoter_pct === null) return { ok: false, reason: "bad_promoter_pct" };

    const passive_pct = numOrNull(p.passive_pct, 0, 100);
    if (passive_pct === NUM_INVALID || passive_pct === null) return { ok: false, reason: "bad_passive_pct" };

    const detractor_pct = numOrNull(p.detractor_pct, 0, 100);
    if (detractor_pct === NUM_INVALID || detractor_pct === null) return { ok: false, reason: "bad_detractor_pct" };

    function parseThemes(v: unknown): { theme: string; mention_count: number; example_quote: string }[] {
      const raw = Array.isArray(v) ? (v as unknown[]).slice(0, 10) : [];
      const out: { theme: string; mention_count: number; example_quote: string }[] = [];
      for (const item of raw) {
        if (typeof item !== "object" || item === null) continue;
        const rec = item as Record<string, unknown>;
        const theme = str(rec.theme);
        if (!theme) continue;
        const mention_count = numOrNull(rec.mention_count, 0);
        if (mention_count === NUM_INVALID || mention_count === null) continue;
        out.push({ theme, mention_count, example_quote: str(rec.example_quote) ?? "" });
      }
      return out;
    }
    const top_promoter_themes = parseThemes(p.top_promoter_themes);
    const top_detractor_themes = parseThemes(p.top_detractor_themes);

    const rawSegmentNps = Array.isArray(p.segment_nps) ? (p.segment_nps as unknown[]).slice(0, 10) : [];
    const segment_nps: { segment: string; nps_score: number; response_count: number }[] = [];
    for (const item of rawSegmentNps) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const segNpsScore = numOrNull(rec.nps_score, -100, 100);
      if (segNpsScore === NUM_INVALID || segNpsScore === null) continue;
      const response_count = numOrNull(rec.response_count, 0);
      if (response_count === NUM_INVALID || response_count === null) continue;
      segment_nps.push({ segment: str(rec.segment) ?? "", nps_score: segNpsScore, response_count });
    }

    const TREND_ASSESSMENTS = ["improving", "declining", "stable", "insufficient_data"];
    const trend_assessment = typeof p.trend_assessment === "string" && TREND_ASSESSMENTS.includes(p.trend_assessment) ? p.trend_assessment : null;
    if (!trend_assessment) return { ok: false, reason: "bad_trend_assessment" };

    const action_priorities = strArray(p.action_priorities, 10, MAX_STR);
    if (action_priorities.length === 0) return { ok: false, reason: "empty_action_priorities" };
  ...
  if (kind === "score_customer_health") {
    const HEALTH_CATEGORIES = ["green", "yellow", "red"];
    const rawScored = Array.isArray(p.scored_customers) ? (p.scored_customers as unknown[]).slice(0, 500) : [];
    const scored_customers: { customer_name: string; health_score: number; health_category: string; top_risk_factor: string }[] = [];
    for (const item of rawScored) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const health_score = numOrNull(rec.health_score, 0, 100);
      if (health_score === NUM_INVALID || health_score === null) continue;
      if (typeof rec.health_category !== "string" || !HEALTH_CATEGORIES.includes(rec.health_category)) continue;
      scored_customers.push({ customer_name: str(rec.customer_name) ?? "", health_score, health_category: rec.health_category, top_risk_factor: str(rec.top_risk_factor) ?? "" });
    }
    if (scored_customers.length === 0) return { ok: false, reason: "empty_scored_customers" };

    const rawComponents = Array.isArray(p.score_components) ? (p.score_components as unknown[]).slice(0, 10) : [];
    const score_components: { component_name: string; weight_pct: number; description: string }[] = [];
    for (const item of rawComponents) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const weight_pct = numOrNull(rec.weight_pct, 0, 100);
      if (weight_pct === NUM_INVALID || weight_pct === null) continue;
      score_components.push({ component_name: str(rec.component_name) ?? "", weight_pct, description: str(rec.description) ?? "" });
    }
    if (score_components.length === 0) return { ok: false, reason: "empty_score_components" };

    function parseHealthDistribution(v: unknown): { green_count: number; yellow_count: number; red_count: number; avg_score: number } | null {
      if (typeof v !== "object" || v === null) return null;
      const rec = v as Record<string, unknown>;
      const green_count = numOrNull(rec.green_count, 0);
      if (green_count === NUM_INVALID || green_count === null) return null;
      const yellow_count = numOrNull(rec.yellow_count, 0);
      if (yellow_count === NUM_INVALID || yellow_count === null) return null;
      const red_count = numOrNull(rec.red_count, 0);
      if (red_count === NUM_INVALID || red_count === null) return null;
      const avg_score = numOrNull(rec.avg_score, 0, 100);
      if (avg_score === NUM_INVALID || avg_score === null) return null;
      return { green_count, yellow_count, red_count, avg_score };
    }
    const health_distribution = parseHealthDistribution(p.health_distribution);
    if (!health_distribution) return { ok: false, reason: "bad_health_distribution" };

    const rawHighRisk = Array.isArray(p.high_risk_customers) ? (p.high_risk_customers as unknown[]).slice(0, 20) : [];
    const high_risk_customers: { customer_name: string; health_score: number; primary_risk: string; recommended_action: string }[] = [];
    for (const item of rawHighRisk) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const health_score = numOrNull(rec.health_score, 0, 100);
      if (health_score === NUM_INVALID || health_score === null) continue;
      high_risk_customers.push({ customer_name: str(rec.customer_name) ?? "", health_score, primary_risk: str(rec.primary_risk) ?? "", recommended_action: str(rec.recommended_action) ?? "" });
    }

    const rawChampions = Array.isArray(p.champion_customers) ? (p.champion_customers as unknown[]).slice(0, 10) : [];
    const champion_customers: { customer_name: string; health_score: number; expansion_opportunity: string }[] = [];
    for (const item of rawChampions) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const health_score = numOrNull(rec.health_score, 0, 100);
      if (health_score === NUM_INVALID || health_score === null) continue;
      champion_customers.push({ customer_name: str(rec.customer_name) ?? "", health_score, expansion_opportunity: str(rec.expansion_opportunity) ?? "" });
    }

    const MODEL_CONFIDENCES = ["high", "medium", "low"];
    const model_confidence = typeof p.model_confidence === "string" && MODEL_CONFIDENCES.includes(p.model_confidence) ? p.model_confidence : null;
    if (!model_confidence) return { ok: false, reason: "bad_model_confidence" };

    const rawPlaybooks = Array.isArray(p.recommended_playbooks) ? (p.recommended_playbooks as unknown[]).slice(0, 5) : [];
    const recommended_playbooks: { playbook_name: string; trigger_condition: string; target_segment: string }[] = [];
    for (const item of rawPlaybooks) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const playbook_name = str(rec.playbook_name);
      if (!playbook_name) continue;
      recommended_playbooks.push({ playbook_name, trigger_condition: str(rec.trigger_condition) ?? "", target_segment: str(rec.target_segment) ?? "" });
    }
    if (recommended_playbooks.length === 0) return { ok: false, reason: "empty_recommended_playbooks" };
  ...
  if (kind === "analyze_win_loss") {
    const period = str(p.period);
    if (!period) return { ok: false, reason: "missing_period" };

    const total_opportunities = numOrNull(p.total_opportunities, 0);
    if (total_opportunities === NUM_INVALID || total_opportunities === null) return { ok: false, reason: "bad_total_opportunities" };

    const won_count = numOrNull(p.won_count, 0);
    if (won_count === NUM_INVALID || won_count === null) return { ok: false, reason: "bad_won_count" };

    const lost_count = numOrNull(p.lost_count, 0);
    if (lost_count === NUM_INVALID || lost_count === null) return { ok: false, reason: "bad_lost_count" };

    const win_rate_pct = numOrNull(p.win_rate_pct, 0, 100);
    if (win_rate_pct === NUM_INVALID || win_rate_pct === null) return { ok: false, reason: "bad_win_rate_pct" };

    const IMPACTS = ["high", "medium", "low"];
    function parseFactors(v: unknown): { factor: string; mention_frequency_pct: number; impact: string }[] {
      const raw = Array.isArray(v) ? (v as unknown[]).slice(0, 10) : [];
      const out: { factor: string; mention_frequency_pct: number; impact: string }[] = [];
      for (const item of raw) {
        if (typeof item !== "object" || item === null) continue;
        const rec = item as Record<string, unknown>;
        if (typeof rec.impact !== "string" || !IMPACTS.includes(rec.impact)) continue;
        const mention_frequency_pct = numOrNull(rec.mention_frequency_pct, 0, 100);
        if (mention_frequency_pct === NUM_INVALID || mention_frequency_pct === null) continue;
        out.push({ factor: str(rec.factor) ?? "", mention_frequency_pct, impact: rec.impact });
      }
      return out;
    }
    const win_factors = parseFactors(p.win_factors);
    if (win_factors.length === 0) return { ok: false, reason: "empty_win_factors" };
    const loss_factors = parseFactors(p.loss_factors);
    if (loss_factors.length === 0) return { ok: false, reason: "empty_loss_factors" };

    const rawCompetitors = Array.isArray(p.competitor_analysis) ? (p.competitor_analysis as unknown[]).slice(0, 10) : [];
    const competitor_analysis: { competitor: string; lost_to_count: number; won_against_count: number; win_rate_vs_competitor: number }[] = [];
    for (const item of rawCompetitors) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const competitor = str(rec.competitor);
      if (!competitor) continue;
      const lost_to_count = numOrNull(rec.lost_to_count, 0);
      if (lost_to_count === NUM_INVALID || lost_to_count === null) continue;
      const won_against_count = numOrNull(rec.won_against_count, 0);
      if (won_against_count === NUM_INVALID || won_against_count === null) continue;
      const win_rate_vs_competitor = numOrNull(rec.win_rate_vs_competitor, 0, 100);
      if (win_rate_vs_competitor === NUM_INVALID || win_rate_vs_competitor === null) continue;
      competitor_analysis.push({ competitor, lost_to_count, won_against_count, win_rate_vs_competitor });
    }

    const rawDealChar = Array.isArray(p.deal_characteristic_analysis) ? (p.deal_characteristic_analysis as unknown[]).slice(0, 10) : [];
    const deal_characteristic_analysis: { characteristic: string; win_rate_when_present: number; win_rate_when_absent: number }[] = [];
    for (const item of rawDealChar) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const characteristic = str(rec.characteristic);
      if (!characteristic) continue;
      const win_rate_when_present = numOrNull(rec.win_rate_when_present, 0, 100);
      if (win_rate_when_present === NUM_INVALID || win_rate_when_present === null) continue;
      const win_rate_when_absent = numOrNull(rec.win_rate_when_absent, 0, 100);
      if (win_rate_when_absent === NUM_INVALID || win_rate_when_absent === null) continue;
      deal_characteristic_analysis.push({ characteristic, win_rate_when_present, win_rate_when_absent });
    }

    const rawSegments = Array.isArray(p.win_rate_by_segment) ? (p.win_rate_by_segment as unknown[]).slice(0, 10) : [];
    const win_rate_by_segment: { segment: string; win_rate_pct: number; deal_count: number }[] = [];
    for (const item of rawSegments) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const segment = str(rec.segment);
      if (!segment) continue;
      const segWinRatePct = numOrNull(rec.win_rate_pct, 0, 100);
      if (segWinRatePct === NUM_INVALID || segWinRatePct === null) continue;
      const deal_count = numOrNull(rec.deal_count, 0);
      if (deal_count === NUM_INVALID || deal_count === null) continue;
      win_rate_by_segment.push({ segment, win_rate_pct: segWinRatePct, deal_count });
    }
    ...
    return {
      ...
      payload: { period, total_opportunities, won_count, lost_count, win_rate_pct, win_factors, loss_factors, competitor_analysis, deal_characteristic_analysis, win_rate_by_segment, recommendations },
    };
  }

  if (kind === "analyze_forecast_accuracy") {
    const period = str(p.period);
    if (!period) return { ok: false, reason: "missing_period" };

    const rawFva = Array.isArray(p.forecast_vs_actual) ? (p.forecast_vs_actual as unknown[]).slice(0, 36) : [];
    const forecast_vs_actual: { label: string; forecasted: number; actual: number; accuracy_pct: number; variance: number }[] = [];
    for (const item of rawFva) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const label = str(rec.label);
      if (!label) continue;
      const forecasted = numOrNull(rec.forecasted, 0);
      if (forecasted === NUM_INVALID || forecasted === null) continue;
      const actual = numOrNull(rec.actual, 0);
      if (actual === NUM_INVALID || actual === null) continue;
      const accuracy_pct = numOrNull(rec.accuracy_pct, 0);
      if (accuracy_pct === NUM_INVALID || accuracy_pct === null) continue;
      const variance = numOrNull(rec.variance);
      if (variance === NUM_INVALID || variance === null) continue;
      forecast_vs_actual.push({ label, forecasted, actual, accuracy_pct, variance });
    }
    if (forecast_vs_actual.length === 0) return { ok: false, reason: "empty_forecast_vs_actual" };

    const overall_accuracy_pct = numOrNull(p.overall_accuracy_pct, 0);
    if (overall_accuracy_pct === NUM_INVALID || overall_accuracy_pct === null) return { ok: false, reason: "bad_overall_accuracy_pct" };

    const BIAS_DIRECTIONS = ["over_forecast", "under_forecast", "unbiased"];
    const bias_direction = str(p.bias_direction);
    if (!bias_direction || !BIAS_DIRECTIONS.includes(bias_direction)) return { ok: false, reason: "bad_bias_direction" };

    const mape = numOrNull(p.mape, 0);
    if (mape === NUM_INVALID || mape === null) return { ok: false, reason: "bad_mape" };

    const rawTrend = Array.isArray(p.period_accuracy_trend) ? (p.period_accuracy_trend as unknown[]).slice(0, 12) : [];
    const period_accuracy_trend: { period: string; mape: number }[] = [];
    for (const item of rawTrend) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const trendPeriod = str(rec.period);
      if (!trendPeriod) continue;
      const trendMape = numOrNull(rec.mape, 0);
      if (trendMape === NUM_INVALID || trendMape === null) continue;
      period_accuracy_trend.push({ period: trendPeriod, mape: trendMape });
    }

    const rawRep = Array.isArray(p.rep_accuracy_breakdown) ? (p.rep_accuracy_breakdown as unknown[]).slice(0, 50) : [];
    const rep_accuracy_breakdown: { rep_name: string; mape: number; bias: string }[] = [];
    for (const item of rawRep) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const rep_name = str(rec.rep_name);
      if (!rep_name) continue;
      const repMape = numOrNull(rec.mape, 0);
      if (repMape === NUM_INVALID || repMape === null) continue;
      rep_accuracy_breakdown.push({ rep_name, mape: repMape, bias: str(rec.bias) ?? "" });
    }

    const pipeline_quality_flags = strArray(p.pipeline_quality_flags, 15, MAX_STR);
  ...
  if (kind === "query_bigquery") {
    const natural_language_question = typeof p.natural_language_question === "string" && p.natural_language_question.length > 0
      ? p.natural_language_question.slice(0, 1000) : null;
    if (!natural_language_question) return { ok: false, reason: "missing_natural_language_question" };

    const generated_sql = typeof p.generated_sql === "string" && p.generated_sql.length > 0
      ? p.generated_sql.slice(0, 10000) : null;
    if (!generated_sql) return { ok: false, reason: "missing_generated_sql" };

    const query_rationale = str(p.query_rationale);
    if (!query_rationale) return { ok: false, reason: "missing_query_rationale" };

    const rows_returned = typeof p.rows_returned === "number" && Number.isInteger(p.rows_returned) && p.rows_returned >= 0 ? p.rows_returned : null;
    if (rows_returned === null) return { ok: false, reason: "bad_rows_returned" };

    const execution_time_ms = typeof p.execution_time_ms === "number" && Number.isInteger(p.execution_time_ms) && p.execution_time_ms >= 0 ? p.execution_time_ms : null;
    if (execution_time_ms === null) return { ok: false, reason: "bad_execution_time_ms" };

    const result_preview = Array.isArray(p.result_preview) ? (p.result_preview as unknown[]).slice(0, 20) : [];

    const QUERY_STATUSES = ["success", "error", "timeout", "pending"];
    if (typeof p.query_status !== "string" || !QUERY_STATUSES.includes(p.query_status)) {
      return { ok: false, reason: "bad_query_status" };
    }

    const rawSchema = Array.isArray(p.result_schema) ? (p.result_schema as unknown[]).slice(0, 100) : [];
    const result_schema: { name: string; type: string; mode: string }[] = [];
    for (const item of rawSchema) {
      if (typeof item !== "object" || item === null) continue;
      const rec = item as Record<string, unknown>;
      const name = str(rec.name);
      if (!name) continue;
      result_schema.push({ name, type: str(rec.type) ?? "", mode: str(rec.mode) ?? "" });
    }

    const error_message = p.error_message === null ? null : (str(p.error_message) ?? null);
    const follow_up_suggestions = strArray(p.follow_up_suggestions, 10, 500);
