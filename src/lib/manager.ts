...
export async function routePayload(
  ...
  if (financial) {
    plan.push("reconciler", "invoice_matcher", "cash_flow_agent", "tax_categorizer", "budget_analyst", "trend_detector", "accountant");
  } else {
    plan.push("inventory_tracker", "reorder_flagger", "supplier_analyst", "po_agent", "trend_detector", "data_merger"); // non-financial only
  }
