-- ============================================================================
-- U-I-OS Migration 0036 — SQL Analysis Agent
-- ============================================================================
-- Adds role 'sql_analyst', action kind 'analyze_sql', and the
-- sql_analysis_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0035.
-- ============================================================================

-- 1. agent_runs.role += 'sql_analyst'
...
-- 2. proposed_actions.kind += 'analyze_sql'
...
-- 4. sql_analysis_runs — executor target (one row per approved analyze_sql action)
create table sql_analysis_runs (
  ...
  queries_found       int not null default 0,
  issues              jsonb not null,
  optimizations       jsonb not null,
  risk_level          text not null,
  created_at          timestamptz not null default now()
);
create index idx_sql_analysis_runs_org_id on sql_analysis_runs(org_id);
create index idx_sql_analysis_runs_payload_id on sql_analysis_runs(payload_id);

alter table sql_analysis_runs enable row level security;
create policy tenant_isolation_sql_analysis_runs on sql_analysis_runs
