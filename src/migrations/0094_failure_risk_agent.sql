-- ============================================================================
-- U-I-OS Migration 0094 — Failure Risk Agent
-- ============================================================================
-- Adds role 'failure_risk_agent', action kind 'assess_failure_risk', and the
-- failure_risk_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0093.
-- ============================================================================

-- 1. agent_runs.role += 'failure_risk_agent'
...
-- 2. proposed_actions.kind += 'assess_failure_risk'
...
-- 4. failure_risk_runs — executor target (one row per approved assess_failure_risk action)
create table failure_risk_runs (
...
create index idx_failure_risk_runs_org_id on failure_risk_runs(org_id);
create index idx_failure_risk_runs_payload_id on failure_risk_runs(payload_id);

alter table failure_risk_runs enable row level security;
create policy tenant_isolation_failure_risk_runs on failure_risk_runs
