-- ============================================================================
-- U-I-OS Migration 0033 — Audit Trail Summarizer agent
-- ============================================================================
-- Adds role 'audit_summarizer', action kind 'summarize_audit_trail', and the
-- audit_summaries record table (executor target for approved summaries).
-- Run once against the same DB as 0032.
-- ============================================================================

-- 1. agent_runs.role += 'audit_summarizer'
...
-- 2. proposed_actions.kind += 'summarize_audit_trail'
...
-- 4. audit_summaries — executor target (one row per approved summarize_audit_trail action)
create table audit_summaries (
  ...
  events_summarized   int not null default 0,
  summary_paragraphs  jsonb not null,
  key_actions         jsonb not null,
  anomalies_noted     jsonb not null,
  created_at          timestamptz not null default now()
);
create index idx_audit_summaries_org_id on audit_summaries(org_id);
create index idx_audit_summaries_payload_id on audit_summaries(payload_id);

alter table audit_summaries enable row level security;
create policy tenant_isolation_audit_summaries on audit_summaries
