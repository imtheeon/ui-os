-- ============================================================================
-- U-I-OS Migration 0129 — Cohort Analysis Agent
-- ============================================================================
-- Adds role 'cohort_analysis_agent', action kind 'analyze_customer_cohorts',
-- and the customer_cohort_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0128.
-- (Action kind renamed from the batch-20 spec's 'analyze_cohorts' — that
-- string is already used by the pre-existing 'cohort_agent' role with a
-- different payload shape; renamed to 'analyze_customer_cohorts' per user
-- decision to avoid a validateProposal dead-code collision. Table renamed
-- from the spec's 'cohort_analysis_runs' to 'customer_cohort_runs' — that
-- name was already the live executor target of the pre-existing cohort_agent
-- role, migration 0053.)
-- ============================================================================

-- 1. agent_runs.role += 'cohort_analysis_agent'
...
-- 3. agent_accuracy.agent_role += 'cohort_analysis_agent'
...
-- 4. customer_cohort_runs — executor target (one row per approved analyze_customer_cohorts action)
create table customer_cohort_runs (
  ...
  avg_month1_retention      numeric,
  avg_month3_retention      numeric,
  avg_month6_retention      numeric,
  avg_month12_retention     numeric,
  best_cohort               text,
  worst_cohort              text,
  trend                     text not null,
...
create index idx_customer_cohort_runs_org_id on customer_cohort_runs(org_id);
create index idx_customer_cohort_runs_payload_id on customer_cohort_runs(payload_id);

alter table customer_cohort_runs enable row level security;
create policy tenant_isolation_customer_cohort_runs on customer_cohort_runs
