-- ============================================================================
-- U-I-OS Migration 0095 — Unit Economics Agent
-- ============================================================================
-- Adds role 'unit_economics_agent', action kind 'analyze_unit_economics', and
-- the unit_economics_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0094.
-- ============================================================================

-- 1. agent_runs.role += 'unit_economics_agent'
...
-- 2. proposed_actions.kind += 'analyze_unit_economics'
...
-- 4. unit_economics_runs — executor target (one row per approved analyze_unit_economics action)
create table unit_economics_runs (
  ...
  churn_rate_monthly     numeric,
  magic_number           numeric,
  by_channel             jsonb not null,
  data_period            text not null,
  created_at             timestamptz not null default now()
);
create index idx_unit_economics_runs_org_id on unit_economics_runs(org_id);
create index idx_unit_economics_runs_payload_id on unit_economics_runs(payload_id);

alter table unit_economics_runs enable row level security;
create policy tenant_isolation_unit_economics_runs on unit_economics_runs
