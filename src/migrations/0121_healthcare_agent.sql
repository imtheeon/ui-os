-- ============================================================================
-- U-I-OS Migration 0121 — Healthcare Agent
-- ============================================================================
-- Adds role 'healthcare_agent', action kind 'analyze_healthcare_financials',
-- and the healthcare_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0120.
-- ============================================================================

-- 1. agent_runs.role += 'healthcare_agent'
...
-- 4. healthcare_runs — executor target (one row per approved analyze_healthcare_financials action)
create table healthcare_runs (
  ...
  net_patient_revenue       numeric not null default 0,
  gross_charges             numeric,
  contractual_adjustments   numeric,
  bad_debt_expense          numeric,
  payor_mix                 jsonb not null,
  cost_per_patient_encounter numeric,
  days_in_ar                numeric,
  denial_rate               numeric,
  clean_claim_rate          numeric,
  quality_metrics           jsonb not null,
  revenue_cycle_insights    jsonb not null,
  created_at                timestamptz not null default now()
);
create index idx_healthcare_runs_org_id on healthcare_runs(org_id);
create index idx_healthcare_runs_payload_id on healthcare_runs(payload_id);

alter table healthcare_runs enable row level security;
create policy tenant_isolation_healthcare_runs on healthcare_runs
