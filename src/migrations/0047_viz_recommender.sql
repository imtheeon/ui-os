-- ============================================================================
-- U-I-OS Migration 0047 — Visualization Recommender
-- ============================================================================
-- Adds role 'viz_recommender', action kind 'recommend_visualizations', and
-- the viz_recommendation_runs record table (executor target for approved
-- recommendations). Run once against the same DB as 0046.
-- ============================================================================

-- 1. agent_runs.role += 'viz_recommender'
...
-- 2. proposed_actions.kind += 'recommend_visualizations'
...
alter table proposed_actions add constraint proposed_actions_kind_check
  check (kind in ('record_ledger_entry','store_report','flag_anomaly','categorize_items','clean_data','merge_datasets','normalize_units','reconcile_records','match_invoices','project_cash_flow','categorize_tax_items','flag_duplicates','compare_budget_actual','track_inventory','flag_reorders','analyze_suppliers','process_purchase_orders','detect_trends','compare_periods','generate_exec_summary','generate_forecast','generate_report','assess_data_quality','flag_compliance_issues','assess_vendor_risk','generate_onboarding_guidance','request_clarification','analyze_multi_period','summarize_audit_trail','review_code','generate_tests','analyze_sql','validate_analysis','generate_health_score','draft_email','generate_recommendations','extract_patterns','generate_alerts','generate_client_report','generate_narrative','prepare_meeting','build_board_deck','recommend_visualizations'));

-- 3. agent_accuracy.agent_role += 'viz_recommender'
...
-- 4. viz_recommendation_runs — executor target (one row per approved recommend_visualizations action)
create table viz_recommendation_runs (
  ...
  total_recommended   int not null default 0,
  created_at          timestamptz not null default now()
);
create index idx_viz_recommendation_runs_org_id on viz_recommendation_runs(org_id);
create index idx_viz_recommendation_runs_payload_id on viz_recommendation_runs(payload_id);

alter table viz_recommendation_runs enable row level security;
create policy tenant_isolation_viz_recommendation_runs on viz_recommendation_runs
