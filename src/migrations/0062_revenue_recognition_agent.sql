-- ============================================================================
-- U-I-OS Migration 0062 — Revenue Recognition Agent
-- ============================================================================
-- Adds role 'revenue_recognition_agent', action kind 'analyze_revenue_recognition',
-- and the revenue_recognition_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0061.
-- ============================================================================

-- 1. agent_runs.role += 'revenue_recognition_agent'
...
-- 2. proposed_actions.kind += 'analyze_revenue_recognition'
...
-- 4. revenue_recognition_runs — executor target (one row per approved analyze_revenue_recognition action)
create table revenue_recognition_runs (
  ...
  asc_606_notes         text not null,
  created_at            timestamptz not null default now()
);
create index idx_revenue_recognition_runs_org_id on revenue_recognition_runs(org_id);
create index idx_revenue_recognition_runs_payload_id on revenue_recognition_runs(payload_id);

alter table revenue_recognition_runs enable row level security;
create policy tenant_isolation_revenue_recognition_runs on revenue_recognition_runs
