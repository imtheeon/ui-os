-- ============================================================================
-- U-I-OS Migration 0093 — Time Series Decomposition Agent
-- ============================================================================
-- Adds role 'time_series_decomp_agent', action kind 'decompose_time_series',
-- and the time_series_decomp_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0092.
-- ============================================================================

-- 1. agent_runs.role += 'time_series_decomp_agent'
...
-- 2. proposed_actions.kind += 'decompose_time_series'
...
-- 4. time_series_decomp_runs — executor target (one row per approved decompose_time_series action)
create table time_series_decomp_runs (
  ...
  seasonality_period          text,
  cycle_length_periods        int,
  residual_variance_pct       numeric,
  data_points_analyzed        int not null default 0,
  components                  jsonb not null,
  data_period                 text not null,
  created_at                  timestamptz not null default now()
);
create index idx_time_series_decomp_runs_org_id on time_series_decomp_runs(org_id);
create index idx_time_series_decomp_runs_payload_id on time_series_decomp_runs(payload_id);

alter table time_series_decomp_runs enable row level security;
create policy tenant_isolation_time_series_decomp_runs on time_series_decomp_runs
