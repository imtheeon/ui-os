-- ============================================================================
-- U-I-OS Migration 0107 — Bad Debt Provision Agent
-- ============================================================================
-- Adds role 'bad_debt_provision_agent', action kind
-- 'calculate_bad_debt_provision', and the bad_debt_provision_runs record
-- table (executor target for approved analyses). Run once against the same
-- DB as 0106.
-- ============================================================================

-- 1. agent_runs.role += 'bad_debt_provision_agent'
...
-- 4. bad_debt_provision_runs — executor target (one row per approved calculate_bad_debt_provision action)
create table bad_debt_provision_runs (
  ...
  current_provision      numeric,
  recommended_provision  numeric not null,
  provision_methodology  text not null,
  aging_analysis         jsonb not null,
  specific_provisions    jsonb not null,
  provision_adjustment   numeric not null,
  notes                  text not null,
  created_at             timestamptz not null default now()
);
create index idx_bad_debt_provision_runs_org_id on bad_debt_provision_runs(org_id);
create index idx_bad_debt_provision_runs_payload_id on bad_debt_provision_runs(payload_id);

alter table bad_debt_provision_runs enable row level security;
create policy tenant_isolation_bad_debt_provision_runs on bad_debt_provision_runs
