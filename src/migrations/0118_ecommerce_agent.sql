-- ============================================================================
-- U-I-OS Migration 0118 — E-commerce Agent
-- ============================================================================
-- Adds role 'ecommerce_agent', action kind 'analyze_ecommerce', and the
-- ecommerce_runs record table (executor target for approved analyses). Run
-- once against the same DB as 0117.
-- ============================================================================

-- 1. agent_runs.role += 'ecommerce_agent'
...
-- 2. proposed_actions.kind += 'analyze_ecommerce'
...
-- 4. ecommerce_runs — executor target (one row per approved analyze_ecommerce action)
create table ecommerce_runs (
  ...
  fulfillment_metrics   jsonb not null,
  growth_insights       jsonb not null,
  created_at            timestamptz not null default now()
);
create index idx_ecommerce_runs_org_id on ecommerce_runs(org_id);
create index idx_ecommerce_runs_payload_id on ecommerce_runs(payload_id);

alter table ecommerce_runs enable row level security;
create policy tenant_isolation_ecommerce_runs on ecommerce_runs
