-- ============================================================================
-- U-I-OS Migration 0138 — Data Quality Agent
-- ============================================================================
-- Adds role 'data_quality_agent', action kind 'evaluate_data_quality', and
-- the data_quality_runs record table (executor target for approved
-- assessments). First agent of batch-22. Runs on BOTH routes, inserted
-- FIRST in each route's plan (ahead of all other agents).
--
-- NOTE: batch-22.txt spec'd this agent's action kind as 'assess_data_quality',
-- but that name is already used by the pre-existing 'data_quality' role
-- (migration 0027) with a materially different payload shape
-- (quality_score/overall_grade/issues[column,issue_type,affected_rows,severity]
-- vs. this agent's overall_score/completeness_score/consistency_score/
-- issues[issue_type,description,severity,affected_columns]). Renamed to
-- 'evaluate_data_quality' to avoid a validateProposal collision, following
-- the same precedent used for cohort_analysis_agent's analyze_cohorts ->
-- analyze_customer_cohorts rename in migration 0129.
-- ============================================================================

-- 1. agent_runs.role += 'data_quality_agent'
...
-- 3. agent_accuracy.agent_role += 'data_quality_agent'
...
-- 4. data_quality_runs — executor target (one row per approved evaluate_data_quality action)
create table data_quality_runs (
  ...
  usable_for_analysis    boolean not null default true,
  recommended_agents     jsonb not null,
  created_at             timestamptz not null default now()
);
create index idx_data_quality_runs_org_id on data_quality_runs(org_id);
create index idx_data_quality_runs_payload_id on data_quality_runs(payload_id);

alter table data_quality_runs enable row level security;
create policy tenant_isolation_data_quality_runs on data_quality_runs
