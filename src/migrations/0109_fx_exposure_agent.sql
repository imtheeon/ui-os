-- ============================================================================
-- U-I-OS Migration 0109 — FX Exposure Agent
-- ============================================================================
-- Adds role 'fx_exposure_agent', action kind 'analyze_fx_exposure', and the
-- fx_exposure_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0108.
-- ============================================================================

-- 1. agent_runs.role += 'fx_exposure_agent'
...
-- 2. proposed_actions.kind += 'analyze_fx_exposure'
...
-- 4. fx_exposure_runs — executor target (one row per approved analyze_fx_exposure action)
create table fx_exposure_runs (
  ...
  total_transaction_exposure  numeric not null default 0,
  total_translation_exposure  numeric not null default 0,
  net_exposure_usd_equivalent numeric not null default 0,
  sensitivity_analysis        jsonb not null,
  hedging_recommendations     jsonb not null,
  created_at                  timestamptz not null default now()
);
create index idx_fx_exposure_runs_org_id on fx_exposure_runs(org_id);
create index idx_fx_exposure_runs_payload_id on fx_exposure_runs(payload_id);

alter table fx_exposure_runs enable row level security;
create policy tenant_isolation_fx_exposure_runs on fx_exposure_runs
