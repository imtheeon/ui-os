-- ============================================================================
-- U-I-OS Migration 0105 — Maverick Spend Agent
-- ============================================================================
-- Adds role 'maverick_spend_agent', action kind 'detect_maverick_spend', and
-- the maverick_spend_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0104.
-- ============================================================================

-- 1. agent_runs.role += 'maverick_spend_agent'
...
-- 2. proposed_actions.kind += 'detect_maverick_spend'
...
-- 4. maverick_spend_runs — executor target (one row per approved detect_maverick_spend action)
create table maverick_spend_runs (
  ...
  maverick_transactions  jsonb not null,
  total_maverick_amount  numeric not null default 0,
  maverick_percentage    numeric not null default 0,
  total_spend_analyzed   numeric not null default 0,
  categories_affected    jsonb not null,
  root_causes            jsonb not null,
  recommendations        jsonb not null,
  created_at             timestamptz not null default now()
);
create index idx_maverick_spend_runs_org_id on maverick_spend_runs(org_id);
create index idx_maverick_spend_runs_payload_id on maverick_spend_runs(payload_id);

alter table maverick_spend_runs enable row level security;
create policy tenant_isolation_maverick_spend_runs on maverick_spend_runs
