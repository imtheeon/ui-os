-- ============================================================================
-- U-I-OS Migration 0035 — Code Testing Agent
-- ============================================================================
-- Adds role 'code_tester', action kind 'generate_tests', and the
-- test_generation_runs record table (executor target for approved test gens).
-- Run once against the same DB as 0034.
-- ============================================================================

-- 1. agent_runs.role += 'code_tester'
...
-- 2. proposed_actions.kind += 'generate_tests'
...
-- 4. test_generation_runs — executor target (one row per approved generate_tests action)
create table test_generation_runs (
  ...
  framework_suggested  text not null,
  coverage_estimate    int not null default 0,
  created_at           timestamptz not null default now()
);
create index idx_test_generation_runs_org_id on test_generation_runs(org_id);
create index idx_test_generation_runs_payload_id on test_generation_runs(payload_id);

alter table test_generation_runs enable row level security;
create policy tenant_isolation_test_generation_runs on test_generation_runs
