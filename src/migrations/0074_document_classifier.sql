-- ============================================================================
-- U-I-OS Migration 0074 — Document Classifier
-- ============================================================================
-- Adds role 'document_classifier', action kind 'classify_document', and the
-- document_classifier_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0073.
-- ============================================================================

-- 1. agent_runs.role += 'document_classifier'
...
-- 2. proposed_actions.kind += 'classify_document'
...
-- 3. agent_accuracy.agent_role += 'document_classifier'
...
-- 4. document_classifier_runs — executor target (one row per approved classify_document action)
create table document_classifier_runs (
...
create index idx_document_classifier_runs_org_id on document_classifier_runs(org_id);
create index idx_document_classifier_runs_payload_id on document_classifier_runs(payload_id);

alter table document_classifier_runs enable row level security;
create policy tenant_isolation_document_classifier_runs on document_classifier_runs
