-- ============================================================================
-- U-I-OS Migration 0058 — Profitability Agent
-- ============================================================================
-- Adds role 'profitability_agent', action kind 'analyze_profitability', and
-- the profitability_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0057.
-- ============================================================================

-- 1. agent_runs.role += 'profitability_agent'
...
-- 2. proposed_actions.kind += 'analyze_profitability'
...
-- 3. agent_accuracy.agent_role += 'profitability_agent'
...
-- 4. profitability_runs — executor target (one row per approved analyze_profitability action)
create table profitability_runs (
  ...
  most_profitable           text not null,
  least_profitable          text not null,
  recommendations           jsonb not null,
  created_at                timestamptz not null default now()
);
create index idx_profitability_runs_org_id on profitability_runs(org_id);
create index idx_profitability_runs_payload_id on profitability_runs(payload_id);

alter table profitability_runs enable row level security;
create policy tenant_isolation_profitability_runs on profitability_runs
