-- ============================================================================
-- U-I-OS Migration 0119 — Professional Services Agent
-- ============================================================================
-- Adds role 'professional_services_agent', action kind
-- 'analyze_professional_services', and the professional_services_runs record
-- table (executor target for approved analyses). Run once against the same
-- DB as 0118.
-- ============================================================================

-- 1. agent_runs.role += 'professional_services_agent'
...
-- 2. proposed_actions.kind += 'analyze_professional_services'
...
-- 4. professional_services_runs — executor target (one row per approved analyze_professional_services action)
create table professional_services_runs (
  ...
  average_bill_rate        numeric,
  revenue_per_consultant   numeric,
  wip_value                numeric,
  project_profitability    jsonb not null,
  staff_utilization        jsonb not null,
  realization_rate         numeric,
  recommendations          jsonb not null,
  created_at               timestamptz not null default now()
);
create index idx_professional_services_runs_org_id on professional_services_runs(org_id);
create index idx_professional_services_runs_payload_id on professional_services_runs(payload_id);

alter table professional_services_runs enable row level security;
create policy tenant_isolation_professional_services_runs on professional_services_runs
