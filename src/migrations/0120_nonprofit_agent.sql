-- ============================================================================
-- U-I-OS Migration 0120 — Nonprofit Agent
-- ============================================================================
-- Adds role 'nonprofit_agent', action kind 'analyze_nonprofit_financials', and
-- the nonprofit_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0119.
-- ============================================================================

-- 1. agent_runs.role += 'nonprofit_agent'
...
-- 2. proposed_actions.kind += 'analyze_nonprofit_financials'
...
-- 3. agent_accuracy.agent_role += 'nonprofit_agent'
...
-- 4. nonprofit_runs — executor target (one row per approved analyze_nonprofit_financials action)
create table nonprofit_runs (
  ...
  revenue_by_source              jsonb not null,
  total_revenue                  numeric not null default 0,
  program_expenses               numeric not null default 0,
  administrative_expenses        numeric not null default 0,
  fundraising_expenses           numeric not null default 0,
  total_expenses                 numeric not null default 0,
  program_efficiency_ratio       numeric,
  fundraising_efficiency_ratio   numeric,
  months_of_reserves             numeric,
  donor_metrics                  jsonb not null,
  grant_pipeline                 jsonb not null,
  compliance_notes               text not null,
  created_at                     timestamptz not null default now()
);
create index idx_nonprofit_runs_org_id on nonprofit_runs(org_id);
create index idx_nonprofit_runs_payload_id on nonprofit_runs(payload_id);

alter table nonprofit_runs enable row level security;
create policy tenant_isolation_nonprofit_runs on nonprofit_runs
