-- ============================================================================
-- U-I-OS Migration 0051 — SaaS Metrics Agent
-- ============================================================================
-- Adds role 'saas_metrics_agent', action kind 'calculate_saas_metrics', and
-- the saas_metrics_runs record table (executor target for approved metrics).
-- Run once against the same DB as 0050.
-- ============================================================================

-- 1. agent_runs.role += 'saas_metrics_agent'
...
-- 2. proposed_actions.kind += 'calculate_saas_metrics'
...
-- 3. agent_accuracy.agent_role += 'saas_metrics_agent'
...
-- 4. saas_metrics_runs — executor target (one row per approved calculate_saas_metrics action)
create table saas_metrics_runs (
  ...
  metrics_confidence      text not null,
  available_metrics       jsonb not null,
  notes                   text not null,
  created_at              timestamptz not null default now()
);
create index idx_saas_metrics_runs_org_id on saas_metrics_runs(org_id);
create index idx_saas_metrics_runs_payload_id on saas_metrics_runs(payload_id);

alter table saas_metrics_runs enable row level security;
create policy tenant_isolation_saas_metrics_runs on saas_metrics_runs
