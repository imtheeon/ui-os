-- ============================================================================
-- U-I-OS Migration 0097 — Cap Table Agent
-- ============================================================================
-- Adds role 'cap_table_agent', action kind 'analyze_cap_table', and the
-- cap_table_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0096.
-- ============================================================================

-- 1. agent_runs.role += 'cap_table_agent'
...
-- 2. proposed_actions.kind += 'analyze_cap_table'
...
-- 4. cap_table_runs — executor target (one row per approved analyze_cap_table action)
create table cap_table_runs (
  ...
  option_pool_pct               numeric,
  top_holder_concentration_pct  numeric,
  founder_ownership_pct         numeric,
  investor_ownership_pct        numeric,
  employee_pool_pct             numeric,
  holders                       jsonb not null,
  data_period                   text not null,
  created_at                    timestamptz not null default now()
);
create index idx_cap_table_runs_org_id on cap_table_runs(org_id);
create index idx_cap_table_runs_payload_id on cap_table_runs(payload_id);

alter table cap_table_runs enable row level security;
create policy tenant_isolation_cap_table_runs on cap_table_runs
