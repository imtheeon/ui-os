-- ============================================================================
-- U-I-OS Migration 0053 — Cohort Agent
-- ============================================================================
-- Adds role 'cohort_agent', action kind 'analyze_cohorts', and the
-- cohort_analysis_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0052.
-- ============================================================================

-- 1. agent_runs.role += 'cohort_agent'
...
-- 2. proposed_actions.kind += 'analyze_cohorts'
...
-- 4. cohort_analysis_runs — executor target (one row per approved analyze_cohorts action)
create table cohort_analysis_runs (
  ...
  avg_retention_m1    numeric,
  avg_retention_m3    numeric,
  trend               text not null,
  notes               text not null,
  created_at          timestamptz not null default now()
);
create index idx_cohort_analysis_runs_org_id on cohort_analysis_runs(org_id);
create index idx_cohort_analysis_runs_payload_id on cohort_analysis_runs(payload_id);

alter table cohort_analysis_runs enable row level security;
create policy tenant_isolation_cohort_analysis_runs on cohort_analysis_runs
