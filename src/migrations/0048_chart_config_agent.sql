-- ============================================================================
-- U-I-OS Migration 0048 — Chart Config Agent
-- ============================================================================
-- Adds role 'chart_config_agent', action kind 'generate_chart_configs', and
-- the chart_config_runs record table (executor target for approved configs).
-- Run once against the same DB as 0047.
-- ============================================================================

-- 1. agent_runs.role += 'chart_config_agent'
...
-- 2. proposed_actions.kind += 'generate_chart_configs'
...
alter table proposed_actions add constraint proposed_actions_kind_check
  check (kind in ('record_ledger_entry','store_report','flag_anomaly','categorize_items','clean_data','merge_datasets','normalize_units','reconcile_records','match_invoices','project_cash_flow','categorize_tax_items','flag_duplicates','compare_budget_actual','track_inventory','flag_reorders','analyze_suppliers','process_purchase_orders','detect_trends','compare_periods','generate_exec_summary','generate_forecast','generate_report','assess_data_quality','flag_compliance_issues','assess_vendor_risk','generate_onboarding_guidance','request_clarification','analyze_multi_period','summarize_audit_trail','review_code','generate_tests','analyze_sql','validate_analysis','generate_health_score','draft_email','generate_recommendations','extract_patterns','generate_alerts','generate_client_report','generate_narrative','prepare_meeting','build_board_deck','recommend_visualizations','generate_chart_configs'));

-- 3. agent_accuracy.agent_role += 'chart_config_agent'
...
-- 4. chart_config_runs — executor target (one row per approved generate_chart_configs action)
create table chart_config_runs (
  ...
  total_configs       int not null default 0,
  created_at          timestamptz not null default now()
);
create index idx_chart_config_runs_org_id on chart_config_runs(org_id);
create index idx_chart_config_runs_payload_id on chart_config_runs(payload_id);

alter table chart_config_runs enable row level security;
create policy tenant_isolation_chart_config_runs on chart_config_runs
