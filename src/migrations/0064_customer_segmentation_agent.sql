-- ============================================================================
-- U-I-OS Migration 0064 — Customer Segmentation Agent
-- ============================================================================
-- Adds role 'customer_segmentation_agent', action kind 'segment_customers', and
-- the customer_segmentation_runs record table (executor target for approved
-- segmentations). Runs on BOTH the financial and non-financial routes.
-- Run once against the same DB as 0063.
-- ============================================================================

-- 1. agent_runs.role += 'customer_segmentation_agent'
...
-- 2. proposed_actions.kind += 'segment_customers'
...
-- 3. agent_accuracy.agent_role += 'customer_segmentation_agent'
...
-- 4. customer_segmentation_runs — executor target (one row per approved segment_customers action)
create table customer_segmentation_runs (
...
create index idx_customer_segmentation_runs_org_id on customer_segmentation_runs(org_id);
create index idx_customer_segmentation_runs_payload_id on customer_segmentation_runs(payload_id);

alter table customer_segmentation_runs enable row level security;
create policy tenant_isolation_customer_segmentation_runs on customer_segmentation_runs
