-- ============================================================================
-- U-I-OS Migration 0038 — Health Score Agent
-- ============================================================================
-- Adds role 'health_scorer', action kind 'generate_health_score', and the
-- health_score_runs record table (executor target for approved scores).
-- Run once against the same DB as 0037.
-- ============================================================================

-- 1. agent_runs.role += 'health_scorer'
...
-- 2. proposed_actions.kind += 'generate_health_score'
...
alter table proposed_actions add constraint proposed_actions_kind_check
  check (kind in ('record_ledger_entry','store_report','flag_anomaly','categorize_items','clean_data','merge_datasets','normalize_units','reconcile_records','match_invoices','project_cash_flow','categorize_tax_items','flag_duplicates','compare_budget_actual','track_inventory','flag_reorders','analyze_suppliers','process_purchase_orders','detect_trends','compare_periods','generate_exec_summary','generate_forecast','generate_report','assess_data_quality','flag_compliance_issues','assess_vendor_risk','generate_onboarding_guidance','request_clarification','analyze_multi_period','summarize_audit_trail','review_code','generate_tests','analyze_sql','validate_analysis','generate_health_score'));

-- 3. agent_accuracy.agent_role += 'health_scorer'
...
-- 4. health_score_runs — executor target (one row per approved generate_health_score action)
create table health_score_runs (
...
create index idx_health_score_runs_org_id on health_score_runs(org_id);
create index idx_health_score_runs_payload_id on health_score_runs(payload_id);

alter table health_score_runs enable row level security;
create policy tenant_isolation_health_score_runs on health_score_runs
