-- ============================================================================
-- U-I-OS Migration 0044 — Narrative Agent
-- ============================================================================
-- Adds role 'narrator', action kind 'generate_narrative', and the
-- narrative_runs record table (executor target for approved narratives).
-- Run once against the same DB as 0043.
-- ============================================================================

-- 1. agent_runs.role += 'narrator'
...
-- 2. proposed_actions.kind += 'generate_narrative'
...
alter table proposed_actions add constraint proposed_actions_kind_check
  check (kind in ('record_ledger_entry','store_report','flag_anomaly','categorize_items','clean_data','merge_datasets','normalize_units','reconcile_records','match_invoices','project_cash_flow','categorize_tax_items','flag_duplicates','compare_budget_actual','track_inventory','flag_reorders','analyze_suppliers','process_purchase_orders','detect_trends','compare_periods','generate_exec_summary','generate_forecast','generate_report','assess_data_quality','flag_compliance_issues','assess_vendor_risk','generate_onboarding_guidance','request_clarification','analyze_multi_period','summarize_audit_trail','review_code','generate_tests','analyze_sql','validate_analysis','generate_health_score','draft_email','generate_recommendations','extract_patterns','generate_alerts','generate_client_report','generate_narrative'));

-- 3. agent_accuracy.agent_role += 'narrator'
...
-- 4. narrative_runs — executor target (one row per approved generate_narrative action)
...
create index idx_narrative_runs_org_id on narrative_runs(org_id);
create index idx_narrative_runs_payload_id on narrative_runs(payload_id);

alter table narrative_runs enable row level security;
create policy tenant_isolation_narrative_runs on narrative_runs
