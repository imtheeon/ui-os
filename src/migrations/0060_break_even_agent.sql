-- ============================================================================
-- U-I-OS Migration 0060 — Break-Even Agent
-- ============================================================================
-- Adds role 'break_even_agent', action kind 'calculate_break_even', and
-- the break_even_runs record table (executor target for approved calculations).
-- Run once against the same DB as 0059.
-- ============================================================================

-- 1. agent_runs.role += 'break_even_agent'
...
-- 2. proposed_actions.kind += 'calculate_break_even'
...
-- 4. break_even_runs — executor target (one row per approved calculate_break_even action)
create table break_even_runs (
  ...
  current_units_or_revenue        numeric,
  margin_of_safety                numeric,
  margin_of_safety_percentage     numeric,
  contribution_margin_per_unit    numeric,
  contribution_margin_ratio       numeric,
  status                          text not null,
  created_at                      timestamptz not null default now()
);
create index idx_break_even_runs_org_id on break_even_runs(org_id);
create index idx_break_even_runs_payload_id on break_even_runs(payload_id);

alter table break_even_runs enable row level security;
create policy tenant_isolation_break_even_runs on break_even_runs
