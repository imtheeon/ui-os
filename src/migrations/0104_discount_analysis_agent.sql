-- ============================================================================
-- U-I-OS Migration 0104 — Discount Analysis Agent
-- ============================================================================
-- Adds role 'discount_analysis_agent', action kind 'analyze_discounts', and
-- the discount_analysis_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0103.
-- ============================================================================

-- 1. agent_runs.role += 'discount_analysis_agent'
...
-- 2. proposed_actions.kind += 'analyze_discounts'
...
-- 4. discount_analysis_runs — executor target (one row per approved analyze_discounts action)
create table discount_analysis_runs (
  ...
  discount_summary               jsonb not null,
  total_list_price               numeric not null default 0,
  total_discounted_price         numeric not null default 0,
  total_discount_amount          numeric not null default 0,
  average_discount_percentage    numeric not null default 0,
  discount_by_segment            jsonb not null,
  excessive_discounts            jsonb not null,
  revenue_leakage                numeric not null default 0,
  recommendations                jsonb not null,
  created_at                     timestamptz not null default now()
);
create index idx_discount_analysis_runs_org_id on discount_analysis_runs(org_id);
create index idx_discount_analysis_runs_payload_id on discount_analysis_runs(payload_id);

alter table discount_analysis_runs enable row level security;
create policy tenant_isolation_discount_analysis_runs on discount_analysis_runs
