-- ============================================================================
-- U-I-OS Migration 0030 — Onboarding agent
-- ============================================================================
-- Adds role 'onboarding_agent', action kind 'generate_onboarding_guidance',
-- and the onboarding_guidance_runs record table (executor target for
-- approved guidance).
-- Run once against the same DB as 0029.
-- ============================================================================

-- 1. agent_runs.role += 'onboarding_agent'
...
-- 2. proposed_actions.kind += 'generate_onboarding_guidance'
...
-- 4. onboarding_guidance_runs — executor target (one row per approved generate_onboarding_guidance action)
create table onboarding_guidance_runs (
  ...
  data_type_detected      text not null,
  guidance_steps          jsonb not null,
  next_upload_suggestion  text not null,
  confidence              text not null,
  created_at              timestamptz not null default now()
);
create index idx_onboarding_guidance_runs_org_id on onboarding_guidance_runs(org_id);
create index idx_onboarding_guidance_runs_payload_id on onboarding_guidance_runs(payload_id);

alter table onboarding_guidance_runs enable row level security;
create policy tenant_isolation_onboarding_guidance_runs on onboarding_guidance_runs
