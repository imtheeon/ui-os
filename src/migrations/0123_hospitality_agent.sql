-- ============================================================================
-- U-I-OS Migration 0123 — Hospitality Agent
-- ============================================================================
-- Adds role 'hospitality_agent', action kind 'analyze_hospitality_financials',
-- and the hospitality_runs record table (executor target for approved
-- analyses). Run once against the same DB as 0122.
-- ============================================================================

-- 1. agent_runs.role += 'hospitality_agent'
...
-- 2. proposed_actions.kind += 'analyze_hospitality_financials'
...
-- 3. agent_accuracy.agent_role += 'hospitality_agent'
...
-- 4. hospitality_runs — executor target (one row per approved analyze_hospitality_financials action)
create table hospitality_runs (
  ...
  goppar                        numeric,
  cost_per_occupied_room        numeric,
  channel_mix                   jsonb not null,
  performance_vs_stly           jsonb not null,
  revenue_management_insights   jsonb not null,
  created_at                    timestamptz not null default now()
);
create index idx_hospitality_runs_org_id on hospitality_runs(org_id);
create index idx_hospitality_runs_payload_id on hospitality_runs(payload_id);

alter table hospitality_runs enable row level security;
create policy tenant_isolation_hospitality_runs on hospitality_runs
