-- ============================================================================
-- U-I-OS Migration 0063 — Churn Risk Agent
-- ============================================================================
-- Adds role 'churn_risk_agent', action kind 'analyze_churn_risk', and the
-- churn_risk_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0062.
-- ============================================================================

-- 1. agent_runs.role += 'churn_risk_agent'
...
-- 2. proposed_actions.kind += 'analyze_churn_risk'
...
-- 3. agent_accuracy.agent_role += 'churn_risk_agent'
...
-- 4. churn_risk_runs — executor target (one row per approved analyze_churn_risk action)
create table churn_risk_runs (
...
create index idx_churn_risk_runs_org_id on churn_risk_runs(org_id);
create index idx_churn_risk_runs_payload_id on churn_risk_runs(payload_id);

alter table churn_risk_runs enable row level security;
create policy tenant_isolation_churn_risk_runs on churn_risk_runs
