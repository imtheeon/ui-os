-- ============================================================================
-- U-I-OS Migration 0055 — AP Agent
-- ============================================================================
-- Adds role 'ap_agent', action kind 'analyze_accounts_payable', and the
-- ap_analysis_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0054.
-- ============================================================================

-- 1. agent_runs.role += 'ap_agent'
...
-- 2. proposed_actions.kind += 'analyze_accounts_payable'
...
-- 3. agent_accuracy.agent_role += 'ap_agent'
...
-- 4. ap_analysis_runs — executor target (one row per approved analyze_accounts_payable action)
create table ap_analysis_runs (
  ...
  due_this_week                 numeric not null default 0,
  due_this_month                numeric not null default 0,
  overdue_amount                numeric not null default 0,
  vendors                       jsonb not null,
  early_payment_opportunities   jsonb not null,
  cash_required_30_days         numeric not null default 0,
  created_at                    timestamptz not null default now()
);
create index idx_ap_analysis_runs_org_id on ap_analysis_runs(org_id);
create index idx_ap_analysis_runs_payload_id on ap_analysis_runs(payload_id);

alter table ap_analysis_runs enable row level security;
create policy tenant_isolation_ap_analysis_runs on ap_analysis_runs
