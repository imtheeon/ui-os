-- ============================================================================
-- U-I-OS Migration 0061 — COGS Analysis Agent
-- ============================================================================
-- Adds role 'cogs_analysis_agent', action kind 'analyze_cogs', and the
-- cogs_analysis_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0060.
-- ============================================================================

-- 1. agent_runs.role += 'cogs_analysis_agent'
...
-- 2. proposed_actions.kind += 'analyze_cogs'
...
-- 4. cogs_analysis_runs — executor target (one row per approved analyze_cogs action)
create table cogs_analysis_runs (
  ...
  gross_margin_percentage       numeric not null default 0,
  cogs_components               jsonb not null,
  cogs_trend                    text not null,
  cost_drivers                  jsonb not null,
  optimization_opportunities    jsonb not null,
  created_at                    timestamptz not null default now()
);
create index idx_cogs_analysis_runs_org_id on cogs_analysis_runs(org_id);
create index idx_cogs_analysis_runs_payload_id on cogs_analysis_runs(payload_id);

alter table cogs_analysis_runs enable row level security;
create policy tenant_isolation_cogs_analysis_runs on cogs_analysis_runs
