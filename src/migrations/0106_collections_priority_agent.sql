-- ============================================================================
-- U-I-OS Migration 0106 — Collections Priority Agent
-- ============================================================================
-- Adds role 'collections_priority_agent', action kind 'prioritize_collections',
-- and the collections_priority_runs record table (executor target for
-- approved analyses). Run once against the same DB as 0105.
-- ============================================================================

-- 1. agent_runs.role += 'collections_priority_agent'
...
-- 2. proposed_actions.kind += 'prioritize_collections'
...
-- 4. collections_priority_runs — executor target (one row per approved prioritize_collections action)
create table collections_priority_runs (
  ...
  priority_1_amount     numeric not null default 0,
  priority_2_amount     numeric not null default 0,
  priority_3_amount     numeric not null default 0,
  collection_actions    jsonb not null,
  estimated_collectible numeric,
  created_at            timestamptz not null default now()
);
create index idx_collections_priority_runs_org_id on collections_priority_runs(org_id);
create index idx_collections_priority_runs_payload_id on collections_priority_runs(payload_id);

alter table collections_priority_runs enable row level security;
create policy tenant_isolation_collections_priority_runs on collections_priority_runs
