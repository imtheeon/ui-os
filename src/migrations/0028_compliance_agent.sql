-- ============================================================================
-- U-I-OS Migration 0028 — Compliance agent
-- ============================================================================
-- Adds role 'compliance_agent', action kind 'flag_compliance_issues', and the
-- compliance_flags record table (executor target for approved flags).
-- Run once against the same DB as 0027.
-- ============================================================================

-- 1. agent_runs.role += 'compliance_agent'
...
-- 2. proposed_actions.kind += 'flag_compliance_issues'
...
-- 4. compliance_flags — executor target (one row per approved flag_compliance_issues action)
...
create index idx_compliance_flags_org_id on compliance_flags(org_id);
create index idx_compliance_flags_payload_id on compliance_flags(payload_id);

alter table compliance_flags enable row level security;
create policy tenant_isolation_compliance_flags on compliance_flags
