-- ============================================================================
-- U-I-OS Migration 0040 — Recommendation Agent
-- ============================================================================
-- Adds role 'recommender', action kind 'generate_recommendations', and the
-- recommendation_runs record table (executor target for approved recs).
-- Run once against the same DB as 0039.
-- ============================================================================

-- 1. agent_runs.role += 'recommender'
...
-- 2. proposed_actions.kind += 'generate_recommendations'
...
alter table proposed_actions add constraint proposed_actions_kind_check
  check (kind in ('record_ledger_entry','store_report','flag_anomaly','categorize_items','clean_data','merge_datasets','normalize_units','reconcile_records','match_invoices','project_cash_flow','categorize_tax_items','flag_duplicates','compare_budget_actual','track_inventory','flag_reorders','analyze_suppliers','process_purchase_orders','detect_trends','compare_periods','generate_exec_summary','generate_forecast','generate_report','assess_data_quality','flag_compliance_issues','assess_vendor_risk','generate_onboarding_guidance','request_clarification','analyze_multi_period','summarize_audit_trail','review_code','generate_tests','analyze_sql','validate_analysis','generate_health_score','draft_email','generate_recommendations'));

-- 3. agent_accuracy.agent_role += 'recommender'
...
-- 4. recommendation_runs — executor target (one row per approved generate_recommendations action)
create table recommendation_runs (
  ...
  next_upload_type    text not null,
  priority            text not null,
  created_at          timestamptz not null default now()
);
create index idx_recommendation_runs_org_id on recommendation_runs(org_id);
create index idx_recommendation_runs_payload_id on recommendation_runs(payload_id);

alter table recommendation_runs enable row level security;
create policy tenant_isolation_recommendation_runs on recommendation_runs
