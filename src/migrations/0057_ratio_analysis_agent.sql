-- ============================================================================
-- U-I-OS Migration 0057 — Ratio Analysis Agent
-- ============================================================================
-- Adds role 'ratio_analysis_agent', action kind 'analyze_financial_ratios', and
-- the ratio_analysis_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0056.
-- ============================================================================

-- 1. agent_runs.role += 'ratio_analysis_agent'
...
-- 2. proposed_actions.kind += 'analyze_financial_ratios'
...
-- 3. agent_accuracy.agent_role += 'ratio_analysis_agent'
...
-- 4. ratio_analysis_runs — executor target (one row per approved analyze_financial_ratios action)
create table ratio_analysis_runs (
  ...
  liquidity_ratios          jsonb not null,
  profitability_ratios      jsonb not null,
  leverage_ratios           jsonb not null,
  efficiency_ratios         jsonb not null,
  overall_health            text not null,
  notes                     text not null,
  created_at                timestamptz not null default now()
);
create index idx_ratio_analysis_runs_org_id on ratio_analysis_runs(org_id);
create index idx_ratio_analysis_runs_payload_id on ratio_analysis_runs(payload_id);

alter table ratio_analysis_runs enable row level security;
create policy tenant_isolation_ratio_analysis_runs on ratio_analysis_runs
