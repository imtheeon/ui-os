-- ============================================================================
-- U-I-OS Migration 0096 — Valuation Agent
-- ============================================================================
-- Adds role 'valuation_agent', action kind 'estimate_valuation', and the
-- valuation_runs record table (executor target for approved analyses).
-- Run once against the same DB as 0095.
-- ============================================================================

-- 1. agent_runs.role += 'valuation_agent'
...
-- 2. proposed_actions.kind += 'estimate_valuation'
...
-- 4. valuation_runs — executor target (one row per approved estimate_valuation action)
create table valuation_runs (
  ...
  dcf_value                numeric,
  comparable_low           numeric,
  comparable_high          numeric,
  estimated_valuation_low  numeric,
  estimated_valuation_high numeric,
  primary_method           text not null,
  valuation_notes          text not null,
  data_period              text not null,
  created_at               timestamptz not null default now()
);
create index idx_valuation_runs_org_id on valuation_runs(org_id);
create index idx_valuation_runs_payload_id on valuation_runs(payload_id);

alter table valuation_runs enable row level security;
create policy tenant_isolation_valuation_runs on valuation_runs
